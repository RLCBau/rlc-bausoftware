...
/* DXF_DWG_UNIVERSAL_IMPORT_V15_27 */
/* =========================================================
...
export default function CADViewer() {
  ...
  const importCadFile = async (
    ...
    try {
      ...
      if (extension === "dxf" && file.size <= 12 * 1024 * 1024) {
        try {
          ...
          return;
        } catch (localDxfError: any) {
          setStatus(
            `DXF wird serverseitig verarbeitet: ${String(
              localDxfError?.message || "lokaler Parser nicht ausreichend"
            )}`
          );
        }
      } else if (extension === "dxf") {
        setStatus(`Große DXF-Datei wird serverseitig verarbeitet (${(file.size / 1024 / 1024).toFixed(1)} MB)`);
      }
      ...
      if (!["dxf", "dwg", "dgn", "xml", "landxml", "pdf"].includes(extension)) {
        throw new Error(
