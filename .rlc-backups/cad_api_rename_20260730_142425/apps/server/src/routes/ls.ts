...
function lsPdfInputFromSource(opts: {
  pdfPath: string;
  fsKey: string;
  source: any;
  company?: any;
  ...
  const rows = Array.isArray(source.rows) && source.rows.length
    ? source.rows
    : Array.isArray(source.items?.lieferscheine) && source.items.lieferscheine.length
  ...
  return {
    ...
    projectName: String(
      source.projectName || source.projectTitle || source.baustelle || source.site || opts.fsKey
    ),
    date: safeDate(source.date || first.date),
    lieferscheinNummer: String(
      source.lieferscheinNummer || first.lieferscheinNummer || source.number || ""
    ),
    supplier: String(source.supplier || source.lieferant || first.supplier || first.lieferant || ""),
    site: String(source.site || source.baustelle || first.site || first.baustelle || ""),
    driver: String(source.driver || source.fahrer || first.driver || first.fahrer || ""),
    material: String(source.material || first.material || ""),
    quantity: source.quantity ?? source.qty ?? first.quantity ?? first.qty ?? 0,
    unit: String(source.unit || first.unit || ""),
    kostenstelle: String(source.kostenstelle || source.costCenter || first.kostenstelle || ""),
    lvItemPos: source.lvItemPos || source.lvPos || first.lvItemPos || first.lvPos || null,
    comment: String(source.comment || source.text || first.comment || first.text || ""),
    bemerkungen: String(source.bemerkungen || source.notes || first.bemerkungen || ""),
    attachments,
    rows,
...
router.post("/inbox/approve", async (req, res) => {
  try {
    ...
    // Kompatibilitätskopie für bestehende Clients; die offizielle Ablage ist direkt Verwaltung.
    const { dir: freigabeDir } = await ensureFreigabeDir(fsKey);
    const dst = path.join(freigabeDir, `${body.docId}.json`);

    const { dir: officialDir } = await ensureHistoryDirs(fsKey);
    const date = safeDate(freigabeObj.date || new Date().toISOString().slice(0, 10));
    const { jsonName, reportId } = nextLsFile(officialDir, date);
    const pdfName = jsonName.replace(/\.json$/i, ".pdf");
    const jsonPath = path.join(officialDir, jsonName);
    const pdfPath = path.join(officialDir, pdfName);
    const company = await loadRlcPdfCompanyFromRequest(req);

    const pdfResult = await createLieferscheinPdf(
      lsPdfInputFromSource({
        pdfPath,
        fsKey,
        source: { ...freigabeObj, reportId },
        company,
      })
    );
    ...
    const items = fs
      ...
      .map((f) => {
        ...
        return {
          ...
          pdfUrl:
            meta?.pdfUrl ||
            (fs.existsSync(path.join(dir, f.replace(/\.json$/i, ".pdf")))
              ? `/projects/${encodeURIComponent(fsKey)}/lieferscheine/${encodeURIComponent(
                  f.replace(/\.json$/i, ".pdf")
                )}`
              : null),
        };
      })
...
router.post("/preview", async (req, res) => {
  try {
    ...
    const fsKey = await resolveProjectFsKey(projectId);
    const previewDir = path.join(lieferscheineDir(fsKey), "preview");
    ...
    const safe = safeDate(req.body?.date || new Date().toISOString().slice(0, 10));
    const fileName = `Lieferschein_Vorschau_${safe}_${Date.now()}.pdf`;
    const pdfPath = path.join(previewDir, fileName);
    const company = await loadRlcPdfCompanyFromRequest(req);
    const result = await createLieferscheinPdf(
      lsPdfInputFromSource({
        pdfPath,
        fsKey,
        source: req.body,
    ...
    console.error("POST /api/ls/preview failed:", e);
