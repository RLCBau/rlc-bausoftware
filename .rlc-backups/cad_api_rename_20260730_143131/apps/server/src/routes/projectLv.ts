...
async function parseGaebXmlImport(xmlText: string) {
  ...
  function normalizeOneItem(item: any, prefix: string, fallbackNo: number) {
    ...
    const position = joinOz(prefix, rNo);
  ...
  function readItemsFromBoQ(root: any): any[] {
    ...
    function walkAny(node: any, prefix = "") {
        ...
        /*
         * Prima vengono elaborate le categorie.
         * Ogni livello BoQCtgy estende la OZ corrente.
         */
        for (const ctgy of arr(node?.BoQCtgy)) {
          ...
          /*
           * La categoria viene attraversata una sola volta con il
           * prefix corretto, per esempio 01.01.
           */
          for (const [key, value] of Object.entries(ctgy)) {
            if (
              key === "RNoPart" ||
              key === "RNo" ||
              key === "Nr" ||
            ...
            walkAny(value, nextPrefix);
          }
        }

        /*
         * Posizioni direttamente contenute nel nodo corrente.
         */
        ...
        /*
         * Altri contenitori GAEB: GAEB, Award, BoQ, BoQBody ecc.
         * BoQCtgy, Item e Itemlist sono esclusi perché già elaborati.
         */
