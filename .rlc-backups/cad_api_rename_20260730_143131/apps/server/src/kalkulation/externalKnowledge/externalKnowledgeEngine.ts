...
export type ExternalKnowledgeMatch = ExternalKnowledgeItem & { score: number };
...
export function findExternalKnowledgeMatches(input: {
  ...
  return RLC_EXTERNAL_KNOWLEDGE_V1.map((item) => {
    let score = 0;
    const family = norm(item.family);
    if (family && hay.includes(family)) score += 35;
    for (const kw of item.keywords) if (hay.includes(norm(kw))) score += 12;
