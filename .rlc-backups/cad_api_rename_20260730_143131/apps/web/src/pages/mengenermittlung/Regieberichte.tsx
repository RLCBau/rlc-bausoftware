...
export default function Regieberichte() {
  const q = useQuery();
  const routeReportType: ReportType = window.location.pathname.includes("/tagesberichte")
    ? "TAGESBERICHT"
    : window.location.pathname.includes("/bautagebuch")
    ? "BAUTAGEBUCH"
    : "REGIE";
  ...
  const loadApproved = React.useCallback(async () => {
    ...
    if (primary?.ok) {
      setApprovedItems(
        (primary.items || []).filter((item) => String(item.reportType || "REGIE") === routeReportType)
      );
      return;
    }
    ...
    if (aliasA?.ok) {
      setApprovedItems(
        (aliasA.items || []).filter((item) => String(item.reportType || "REGIE") === routeReportType)
      );
      return;
    }
    ...
    setApprovedItems(
      (legacy?.items || []).filter((item) => String(item.reportType || "REGIE") === routeReportType)
    );
  }, [projectKey, routeReportType]);
  ...
  return (
    <div className="page">
      <MengPageHeader title="Regieberichte" subtitle="Regieberichte prüfen, bearbeiten und mit Aufmaßdaten verknüpfen." />
