...
export function getCadImportPaths(projectId: string): CadImportPaths {
  const PROJECTS_ROOT = getProjectsRoot();
  const projectRoot = path.join(PROJECTS_ROOT, projectId);
  const cadDir = path.join(projectRoot, "cad");

  return {
    projectRoot,
    cadDir,
    utmCsvPath: path.join(cadDir, "utm.csv"),
    takeoffJsonPath: path.join(cadDir, "takeoff.json"),
    snapshotPngPath: path.join(cadDir, "snapshot.png"), // ✅ NEW
  };
}
  ...
  if (!exists(paths.utmCsvPath)) {
    return {
      ...
      message:
        ...
        `Erwarteter Pfad:\n${paths.utmCsvPath}\n\n` +
        `Hinweis: Plugin muss die Datei unter data/projects/<projectId>/cad/utm.csv schreiben.`,
      paths,
    };
  }
  ...
  if (!exists(paths.takeoffJsonPath)) {
    return {
      ...
      message:
        ...
        `Hinweis: Plugin muss die Datei unter data/projects/<projectId>/cad/takeoff.json schreiben.`,
      paths,
    };
  }
  ...
  if (!exists(paths.snapshotPngPath)) {
    return {
      ...
      message:
        ...
        `Hinweis: Plugin muss Snapshot nach data/projects/<projectId>/cad/snapshot.png exportieren.`,
      paths,
    };
  }
...
export function openCadImport(projectIdRaw: string): {
  ok: boolean;
  projectId: string;
  projectRoot: string;
  exe: string;
  ...
  if (!isWindows()) {
    throw new Error("RLC CAD öffnen ist aktuell nur für Windows vorgesehen.");
  }

  const { projectId, projectRoot } = assertProjectDirExists(projectIdRaw);

  const exe =
    (process.env.RLC_CAD_EXE || "").trim() ||
    "C:\\Programme\\Bricsys\\RLC CAD V24 de_DE\\bricscad.exe";

  if (!exists(exe)) {
    throw new Error(
      `RLC CAD EXE nicht gefunden.\n` +
        `Setze RLC_CAD_EXE in apps/server/.env\n` +
        `Aktuell: ${exe}`
    );
  }
  ...
  return {
    ...
    openedFile: dwg,
    message: dwg
      ? `RLC CAD gestartet mit DWG: ${path.basename(dwg)}`
      : "RLC CAD gestartet (kein DWG gefunden, nur App geöffnet).",
  };
}
