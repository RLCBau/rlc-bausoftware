...
function parseLsFromText(txt, defaults) {
    const date = (txt.match(/Datum[:\s]*([0-9]{2}\.[0-9]{2}\.[0-9]{4}|[0-9]{4}-[0-9]{2}-[0-9]{2})/i)?.[1] ?? today()).replace(/(\d{2})\.(\d{2})\.(\d{4})/, "$3-$2-$1");
...
export default function Lieferscheine() {
    ...
    */
    ...
    async function exportPdf(list, preview = false) {
        ...
        for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
            ...
            pageRows.forEach((r, idx) => {
                ...
                const mengeStr = r.quantity != null && r.quantity !== 0 ? `${num(r.quantity)} ${r.unit || ""}`.trim() : "";
                let txCol = margin + 2;
                if (r.kostenstelle)
                    safeText(doc, r.kostenstelle, txCol, textY, colKosten - 4);
                txCol += colKosten;
                const lfText = [r.supplier || "", r.driver || ""].filter(Boolean).join(" / ");
                if (lfText)
                    safeText(doc, lfText, txCol + 2, textY, colLieferant - 4);
                txCol += colLieferant;
                if (r.site)
                    safeText(doc, r.site, txCol + 2, textY, colBaustelle - 4);
                txCol += colBaustelle;
                if (r.material)
                    safeText(doc, r.material, txCol + 2, textY, colMaterial - 4);
                txCol += colMaterial;
                if (mengeStr)
                    doc.text(mengeStr, txCol + colMenge / 2, textY, { align: "center" });
                txCol += colMenge;
                if (r.comment)
                    safeText(doc, r.comment, txCol + 2, textY, colBem - 4);
            });
            ...
            const firstImg = pageRows.flatMap((r) => r.photos || r.attachments || []).find((p) => isImg(p.type)) || null;
            if (firstImg) {
    ...
    async function exportRowPdf(row, projectName) {
        const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
        const margin = 10;
    ...
    const activeList = tab === "INBOX" ? inboxRows : tab === "FREIGEGEBEN" ? freigegebenRows : finalPreview?.rows || [];
    const rightTitle = tab === "INBOX" ? "Inbox" : tab === "FREIGEGEBEN" ? "Freigegeben" : "Final (Historie)";
    const rightCount = tab === "INBOX" ? inboxRows.length : tab === "FREIGEGEBEN" ? freigegebenRows.length : history.length;
    const canEditFinal = (r) => true;
    function openAndEditFromFinal(r) {
        selectRow(r);
        setTab("FREIGEGEBEN");
    }
    return (_jsxs("div", { className: "page", children: [_jsxs("div", { className: "page-header", children: [_jsxs("div", { children: [_jsx("h2", { className: "page-title", style: { marginBottom: 4 }, children: "Lieferscheine" }), _jsx("p", { className: "page-subtitle", style: { margin: 0 }, children: "Inbox \u2192 Freigabe \u2192 Final (Historie)." })] }), _jsxs("div", { className: "page-header-actions", style: { display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }, children: [_jsx("button", { className: "btn", onClick: () => navigate(PATH_BUERO), disabled: !projectId, children: "B\u00FCro / Verwaltung" }), _jsx("button", { className: "btn", onClick: () => navigate(PATH_BUCHHALTUNG), disabled: !projectId, children: "Buchhaltung" }), _jsx("button", { className: "btn", onClick: transferToAufmassEditor, disabled: !activeList.length || !projectId, children: "Ins Aufma\u00DFeditor \u00FCbertragen" }), _jsx("button", { className: "btn", onClick: () => loadHistory(), disabled: !projectId, children: "Final aktualisieren" }), _jsx("button", { className: "btn", onClick: () => loadAll(), disabled: !projectId, children: "Inbox/Freigabe aktualisieren" }), tab === "FREIGEGEBEN" && (_jsx("button", { className: "btn", onClick: saveFreigegebenToFinal, disabled: !projectId || !freigegebenRows.length || loading, children: "Speichern \u2192 Final" }))] })] }), _jsxs("div", { className: "card", style: {
                    padding: 8,
                    marginBottom: 10,
                    display: "flex",
                    gap: 8,
                    alignItems: "center",
                    flexWrap: "wrap",
                }, children: [_jsx(TabBtn, { active: tab === "INBOX", onClick: () => setTab("INBOX"), label: `Inbox (${inboxRows.length})` }), _jsx(TabBtn, { active: tab === "FREIGEGEBEN", onClick: () => setTab("FREIGEGEBEN"), label: `Freigegeben (${freigegebenRows.length})` }), _jsx(TabBtn, { active: tab === "FINAL", onClick: () => setTab("FINAL"), label: `Final (Historie) (${history.length})` }), tab === "FINAL" && (_jsx("span", { style: { fontSize: 12, color: "var(--muted)" }, children: "Final zeigt nur gespeicherte (freigegebene) Dateien." }))] }), _jsxs("div", { className: "page-body", style: {
                    display: "grid",
                    gridTemplateColumns: "minmax(290px, 330px) 1fr",
                    gap: 16,
                    alignItems: "flex-start",
                }, children: [_jsxs("div", { className: "card", style: { padding: 10 }, children: [_jsx("h3", { style: { marginTop: 0, marginBottom: 6 }, children: "Lieferschein erfassen" }), _jsxs("div", { style: { display: "grid", gridTemplateColumns: "1fr", gap: 6 }, children: [_jsx(L, { label: "Projekt-ID", children: _jsx("input", { value: projectId, onChange: (e) => setProjectId(e.target.value), placeholder: "z. B. BA-2025-001" }) }), _jsxs("div", { style: { display: "flex", gap: 6 }, children: [_jsx(L, { label: "Datum", style: { flex: 1 }, children: _jsx("input", { type: "date", value: form.date ?? "", onChange: (e) => setField("date", e.target.value) }) }), _jsx(L, { label: "LS-Nr.", style: { width: 120 }, children: _jsx("input", { value: form.lieferscheinNummer ?? "", onChange: (e) => setField("lieferscheinNummer", e.target.value), placeholder: "z. B. LS-001" }) })] }), _jsx(L, { label: "Lieferant", children: _jsx("input", { value: form.supplier ?? "", onChange: (e) => setField("supplier", e.target.value) }) }), _jsx(L, { label: "Baustelle", children: _jsx("input", { value: form.site ?? "", onChange: (e) => setField("site", e.target.value) }) }), _jsx(L, { label: "Fahrer", children: _jsx("input", { value: form.driver ?? "", onChange: (e) => setField("driver", e.target.value) }) }), _jsx(L, { label: "Material", children: _jsx("input", { value: form.material ?? "", onChange: (e) => setField("material", e.target.value) }) }), _jsxs("div", { style: { display: "flex", gap: 6 }, children: [_jsx(L, { label: "Menge", style: { flex: 1 }, children: _jsx("input", { type: "number", step: "0.01", value: form.quantity ?? 0, onChange: (e) => setField("quantity", Number(e.target.value)) }) }), _jsx(L, { label: "Einheit", style: { width: 100 }, children: _jsx("input", { value: form.unit ?? "", onChange: (e) => setField("unit", e.target.value) }) })] }), _jsx(L, { label: "Kostenstelle", children: _jsx("input", { value: form.kostenstelle ?? "", onChange: (e) => setField("kostenstelle", e.target.value), placeholder: "z. B. 100-BA-01" }) }), _jsx(L, { label: "LV-Position", children: _jsx("input", { value: String(form.lvItemPos ?? ""), onChange: (e) => setField("lvItemPos", e.target.value), placeholder: "z. B. 001.002" }) }), _jsx(L, { label: "Text / Beschreibung", full: true, children: _jsx("textarea", { value: form.comment ?? "", onChange: (e) => setField("comment", e.target.value), style: { height: 70, resize: "vertical" }, placeholder: "z. B. 2 Fahrten, Zufahrt Nord\u2026" }) }), _jsx(L, { label: "Bemerkungen (PDF-Feld unten)", full: true, children: _jsx("textarea", { value: form.bemerkungen ?? "", onChange: (e) => setField("bemerkungen", e.target.value), style: { height: 60, resize: "vertical" }, placeholder: "Bemerkungen f\u00FCr das Feld im Lieferschein-PDF\u2026" }) }), _jsx(L, { label: "Foto/Anhang (Bilder & PDF)", children: _jsxs("div", { style: { display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }, children: [_jsx("input", { id: "lsPhotos", type: "file", multiple: true, accept: "image/*,.pdf,.heic,.heif", onChange: (e) => addPhotos(e.target.files), style: { display: "none" } }), _jsx("label", { htmlFor: "lsPhotos", className: "btn", children: "Dateien w\u00E4hlen" }), _jsx("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" }, children: (form.photos || form.attachments || []).map((ph) => (_jsxs("div", { style: {
                                                            position: "relative",
                                                            ...
                                                            background: "#fafafa",
                                                                ...
                                                                }, children: ph.name })) : (_jsx("a", { href: ph.url, target: "_blank", rel: "noreferrer", style: {
                                                                    display: "grid",
                                                                    placeItems: "center",
                                                                    width: "100%",
                                                                    height: "100%",
                                                                    fontSize: 11,
                                                                }, children: "FILE" })), _jsx("button", { onClick: () => removePhoto(ph.id), className: "btn", style: { position: "absolute", top: 4, right: 4, padding: "0 6px" }, children: "\u2715" })] }, ph.id))) })] }) }), _jsxs("div", { style: { display: "flex", gap: 6, marginTop: 2, flexWrap: "wrap" }, children: [_jsx("button", { className: "btn", onClick: saveToServerDraft, disabled: loading, children: form.id ? "Änderungen speichern" : "Eintrag anlegen" }), _jsx("button", { className: "btn", onClick: clearForm, disabled: loading, children: "Formular leeren" })] }), _jsxs("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", marginTop: 4 }, children: [_jsx("button", { className: "btn", onClick: () => exportXlsx(activeList), disabled: loading, children: "Export XLSX" }), _jsx("button", { className: "btn", onClick: () => exportPdf(activeList, false), disabled: loading, children: "Export PDF" }), _jsx("button", { className: "btn", onClick: () => exportPdf(activeList, true), disabled: loading, children: "PDF Vorschau" })] }), _jsx("button", { className: "btn", style: { marginTop: 4 }, onClick: submitRowServer, disabled: !form.id || !projectId || loading, children: "Einreichen (Inbox)" }), _jsxs("div", { style: { display: "flex", gap: 6, marginTop: 6, alignItems: "center", flexWrap: "wrap" }, children: [_jsx("input", { id: "lsImport", type: "file", accept: "application/pdf", onChange: (e) => importPdfLs(e.target.files), style: { display: "none" } }), _jsx("label", { htmlFor: "lsImport", className: "btn", children: "Import PDF" }), _jsx("span", { style: { fontSize: 12, color: "var(--muted)" }, children: "Estrae: Datum, Lieferant, Baustelle, Fahrer, Material, Menge, Einheit, Pos." })] }), error && _jsx("div", { style: { color: "crimson", marginTop: 4 }, children: error }), loading && _jsx("div", { style: { fontSize: 12, color: "var(--muted)", marginTop: 4 }, children: "Lade\u2026" })] })] }), _jsxs("div", { style: { display: "grid", gap: 10 }, children: [_jsxs("div", { style: { display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 10, alignItems: "stretch" }, children: [_jsxs("div", { className: "card", style: { padding: 8 }, children: [_jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }, children: [_jsx("strong", { children: "PDF Vorschau" }), _jsx("span", { style: { fontSize: 11, color: "var(--muted)" }, children: pdfUrl ? "" : "Noch kein PDF geladen" })] }), pdfUrl ? (_jsx("iframe", { src: pdfUrl, style: {
                                                    width: "100%",
                                                    height: 230,
                                                    border: "1px solid var(--line)",
                                                    borderRadius: 8,
                                                    ...
                                                    fontSize: 12,
                                                    color: "var(--muted)",
                                                }, children: "Lieferschein w\u00E4hlen oder erzeugen\u2026" }))] }), _jsxs("div", { className: "card", style: { padding: 8 }, children: [_jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }, children: [_jsx("strong", { children: rightTitle }), _jsxs("span", { style: { fontSize: 11, color: "var(--muted)" }, children: [rightCount, " ", tab === "FINAL" ? "Datei(en)" : "Eintrag(e)"] })] }), _jsx("div", { style: { maxHeight: 230, overflowY: "auto", paddingRight: 4 }, children: tab === "FINAL" ? (history.length === 0 ? (_jsx("div", { style: { fontSize: 12, color: "var(--muted)", padding: "4px 2px" }, children: "Noch keine freigegebenen Lieferscheine gespeichert." })) : (history.map((h) => (_jsxs("div", { style: {
                                                        padding: "6px 4px",
                                                        borderBottom: "1px solid var(--line)",
                                                        display: "flex",
                                                        gap: 6,
                                                        alignItems: "flex-start",
                                                    }, children: [_jsxs("div", { style: { flex: 1, fontSize: 12 }, children: [_jsxs("div", { style: { fontWeight: 600 }, children: [h.date, " ", h.savedAt
                                                                            ? `, ${new Date(h.savedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`
                                                                            : ""] }), _jsxs("div", { style: { color: "var(--muted)", marginTop: 2 }, children: [h.rows, " Position(en)"] })] }), _jsxs("div", { style: { display: "flex", flexDirection: "column", gap: 4 }, children: [_jsx("button", { className: "btn", style: { fontSize: 11, padding: "2px 6px" }, onClick: () => loadSavedLsItem(h), children: "Laden" }), h.pdfUrl && (_jsx("a", { className: "btn", style: { fontSize: 11, padding: "2px 6px", textAlign: "center" }, href: h.pdfUrl, target: "_blank", rel: "noreferrer", children: "PDF" }))] })] }, h.filename))))) : activeList.length === 0 ? (_jsx("div", { style: { fontSize: 12, color: "var(--muted)", padding: "4px 2px" }, children: "Keine Eintr\u00E4ge." })) : (activeList.map((r, idx) => (_jsxs("div", { style: {
                                                        padding: "6px 4px",
                                                        borderBottom: "1px solid var(--line)",
                                                        display: "flex",
                                                        gap: 6,
                                                        alignItems: "flex-start",
                                                    }, children: [_jsxs("div", { style: { flex: 1, fontSize: 12 }, children: [_jsxs("div", { style: { fontWeight: 600 }, children: [(r.date || "").slice(0, 10), " ", r.lieferscheinNummer ? `– ${r.lieferscheinNummer}` : ""] }), _jsxs("div", { style: { color: "var(--muted)", marginTop: 2 }, children: [(r.supplier || "-"), " ", r.material ? `• ${r.material}` : ""] })] }), _jsx("div", { style: { display: "flex", flexDirection: "column", gap: 4 }, children: _jsx("button", { className: "btn", style: { fontSize: 11, padding: "2px 6px" }, onClick: () => selectRow(r), children: "Bearbeiten" }) })] }, r.id ?? `r-${idx}`)))) })] })] }), _jsxs("div", { className: "card", style: { padding: 0 }, children: [_jsxs("div", { style: {
                                            padding: "8px 10px",
                                            borderBottom: "1px solid var(--line)",
                                            display: "flex",
                                            alignItems: "center",
                                            justifyContent: "space-between",
                                        }, children: [_jsx("div", { style: { fontWeight: 600, fontSize: 14 }, children: tab === "INBOX" ? "Inbox" : tab === "FREIGEGEBEN" ? "Freigegeben" : "Final (Preview)" }), _jsx("div", { style: { fontSize: 12, color: "var(--muted)" }, children: tab === "FINAL"
                                                    ? finalPreview
                                                        ? `${finalPreview.rows.length} Position(en)`
                                                        : "Keine Datei geladen"
                                                    : `${activeList.length} Eintrag(e)` })] }), tab === "FINAL" && !finalPreview ? (_jsx("div", { style: { padding: 12, fontSize: 12, color: "var(--muted)" }, children: "W\u00E4hle eine Datei in der Historie rechts oben (\u201ELaden\u201C), um den Inhalt anzuzeigen." })) : (_jsx("div", { style: { overflowX: "auto" }, children: _jsxs("table", { style: { width: "100%", borderCollapse: "collapse" }, children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx(Th, { children: "Datum" }), _jsx(Th, { children: "LS-Nr." }), _jsx(Th, { children: "Lieferant" }), _jsx(Th, { children: "Baustelle" }), _jsx(Th, { children: "Fahrer" }), _jsx(Th, { children: "Material" }), _jsx(Th, { children: "Menge" }), _jsx(Th, { children: "Einheit" }), _jsx(Th, { children: "Kostenstelle" }), _jsx(Th, { children: "LV-Pos" }), _jsx(Th, { children: "Status" }), _jsx(Th, { children: "Text" }), _jsx(Th, {})] }) }), _jsx("tbody", { children: activeList.length === 0 ? (_jsx("tr", { children: _jsx(Td, { colSpan: 13, style: { textAlign: "center" }, children: projectId ? "Keine Einträge" : "Projekt-ID eingeben" }) })) : (activeList.map((r, i) => (_jsxs("tr", { style: {
                                                            background: selKey != null && String(selKey) === String(r.id) ? "rgba(0,0,0,0.04)" : undefined,
                                                        }, children: [_jsx(Td, { children: r.date }), _jsx(Td, { children: r.lieferscheinNummer }), _jsx(Td, { children: r.supplier }), _jsx(Td, { children: r.site }), _jsx(Td, { children: r.driver }), _jsx(Td, { children: r.material }), _jsx(Td, { style: { textAlign: "right", whiteSpace: "nowrap" }, children: num(r.quantity) }), _jsx(Td, { children: r.unit }), _jsx(Td, { children: r.kostenstelle }), _jsx(Td, { children: r.lvItemPos ?? "" }), _jsx(Td, { children: normalizeStatus(r.workflowStatus) }), _jsx(Td, { style: { maxWidth: 320, whiteSpace: "pre-wrap" }, children: r.comment }), _jsx(Td, { children: tab === "FINAL" ? (_jsxs("div", { style: { display: "flex", gap: 4, flexWrap: "wrap" }, children: [_jsx("button", { className: "btn", onClick: () => exportRowPdf(r, selectedProject?.name), style: { fontSize: 11, padding: "2px 6px" }, children: "PDF" }), canEditFinal(r) && (_jsx("button", { className: "btn", onClick: () => openAndEditFromFinal(r), style: { fontSize: 11, padding: "2px 6px" }, children: "\u00D6ffnen & bearbeiten" }))] })) : (_jsxs("div", { style: { display: "flex", gap: 4, flexWrap: "wrap" }, children: [_jsx("button", { className: "btn", onClick: () => selectRow(r), style: { fontSize: 11, padding: "2px 6px" }, children: "Bearbeiten" }), tab === "INBOX" && normalizeStatus(r.workflowStatus) === "DRAFT" && (_jsx("button", { className: "btn", onClick: () => {
                                                                                selectRow(r);
                                                                                setTimeout(() => submitRowServer(), 0);
                                                                            }, style: { fontSize: 11, padding: "2px 6px" }, disabled: loading, children: "Einreichen" })), tab === "INBOX" && normalizeStatus(r.workflowStatus) === "EINGEREICHT" && (_jsxs(_Fragment, { children: [_jsx("button", { className: "btn", onClick: () => approveRowServer(r), style: { fontSize: 11, padding: "2px 6px" }, disabled: loading, children: "Freigeben" }), _jsx("button", { className: "btn", onClick: () => requestReject(r), style: { fontSize: 11, padding: "2px 6px" }, disabled: loading, children: "Ablehnen" })] })), tab === "INBOX" && normalizeStatus(r.workflowStatus) === "ABGELEHNT" && (_jsx("button", { className: "btn", onClick: () => {
                                                                                selectRow(r);
                                                                                setTimeout(() => submitRowServer(), 0);
                                                                            }, style: { fontSize: 11, padding: "2px 6px" }, disabled: loading, children: "Erneut einreichen" })), tab === "FREIGEGEBEN" && (_jsxs(_Fragment, { children: [_jsx("button", { className: "btn", onClick: () => selectRow(r), style: { fontSize: 11, padding: "2px 6px" }, disabled: loading, children: "\u00D6ffnen" }), _jsx("button", { className: "btn", onClick: () => commitOneFreigegebenToFinal(r), style: { fontSize: 11, padding: "2px 6px" }, disabled: loading, children: "\u00C4nderungen speichern" })] }))] })) })] }, r.id ?? `ls-${i}`)))) })] }) }))] })] })] }), rejectOpen && (_jsx("div", { onClick: () => setRejectOpen(false), style: {
                    position: "fixed",
                    inset: 0,
                    ...
                    zIndex: 9999,
                    padding: 12,
                }, children: _jsxs("div", { onClick: (e) => e.stopPropagation(), className: "card", style: { width: "min(520px, 98vw)", padding: 12 }, children: [_jsx("div", { style: { fontWeight: 700, marginBottom: 6 }, children: "Ablehnen \u2013 Grund" }), _jsx("textarea", { value: rejectText, onChange: (e) => setRejectText(e.target.value), style: { width: "100%", height: 110, resize: "vertical" }, placeholder: "Warum wird der Lieferschein abgelehnt?" }), _jsxs("div", { style: { display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 10 }, children: [_jsx("button", { className: "btn", onClick: () => setRejectOpen(false), disabled: loading, children: "Abbrechen" }), _jsx("button", { className: "btn", onClick: confirmReject, disabled: loading, children: "Ablehnen" })] })] }) })), previewUrl && (_jsx("div", { onClick: () => setPreviewUrl(null), style: {
                    position: "fixed",
                    inset: 0,
