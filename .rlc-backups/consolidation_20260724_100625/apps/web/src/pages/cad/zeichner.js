...
const shell = { height: "calc(100vh - 0px)", display: "grid", gridTemplateRows: "44px 1fr", fontFamily: "Inter, system-ui, Arial", color: "#0f172a" };
const bar = { display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", borderBottom: "1px solid #e2e8f0" };
const btn = { padding: "6px 10px", border: "1px solid #cbd5e1", background: "#fff", borderRadius: 6, fontSize: 13, cursor: "pointer" };
const input = { border: "1px solid #cbd5e1", borderRadius: 6, padding: "6px 8px" };
const canvasWrap = { background: "#ffffff", position: "relative" };
const toolOn = (on) => ({ ...btn, background: on ? "#eef2ff" : "#fff", borderColor: on ? "#6366f1" : "#cbd5e1" });
export default function Zeichner2D() {
        ...
        if (tool === "line") {
            if (temp.current.length === 0) {
                ...
                if (distance(a, b) > 0) {
                    ...
                    const le = { id: CadAPI.newid(), type: "line", layerId, a, b };
        ...
        if (tool === "polyline") {
            if (e.detail === 2) {
                ...
                if (temp.current.length >= 2) {
                    const layerId = CadAPI.getActiveLayerId(doc);
                    const pe = { id: CadAPI.newid(), type: "polyline", layerId, points: [...temp.current, snapW(p)], closed: false };
                ...
                const np = snapW(p);
                if (!last || distance(last, np) > 0)
                    temp.current.push(np);
            }
            draw();
            return;
        }
    ...
    const measureInfo = useMemo(() => {
        if (tool !== "measure" || temp.current.length < 2)
            return "";
        let len = 0;
        for (let i = 0; i < temp.current.length - 1; i++)
            len += distance(temp.current[i], temp.current[i + 1]);
    ...
    return (_jsxs("div", { style: shell, children: [_jsxs("div", { style: bar, children: [_jsx("button", { style: toolOn(tool === "select"), onClick: () => setTool("select"), children: "Auswahl" }), _jsx("button", { style: toolOn(tool === "line"), onClick: () => setTool("line"), children: "Linie" }), _jsx("button", { style: toolOn(tool === "polyline"), onClick: () => setTool("polyline"), children: "Polylinie" }), _jsx("button", { style: toolOn(tool === "measure"), onClick: () => { temp.current = []; setTool("measure"); }, children: "Messen" }), _jsx("button", { style: toolOn(tool === "pan"), onClick: () => setTool("pan"), children: "Pan" }), _jsx("span", { style: { marginLeft: 12 } }), _jsxs("label", { children: ["Grid (m): ", _jsx("input", { style: { ...input, width: 70, marginLeft: 6 }, type: "number", step: "0.1", value: grid, onChange: e => setGrid(Number(e.target.value) || 0) })] }), _jsxs("label", { style: { display: "flex", alignItems: "center", gap: 6 }, children: [_jsx("input", { type: "checkbox", checked: snap, onChange: e => setSnap(e.target.checked) }), "Snap"] }), _jsx("span", { style: { marginLeft: "auto" } }), _jsx("button", { style: { ...btn, color: "#b91c1c" }, onClick: deleteSel, disabled: !selId, children: "L\u00F6schen" }), _jsx("span", { style: { color: "#64748b", fontSize: 12, marginLeft: 12 }, children: measureInfo })] }), _jsx("div", { style: canvasWrap, children: _jsx("canvas", { ref: cnvRef, width: width, height: height, onMouseDown: onDown, onMouseMove: onMove, onMouseUp: onUp, onWheel: onWheel, style: { display: "block", cursor: tool === "pan" ? "grab" : "crosshair" } }) })] }));
}
...
function fmt(n) { return new Intl.NumberFormat("de-DE", { maximumFractionDigits: 3 }).format(n || 0); }
