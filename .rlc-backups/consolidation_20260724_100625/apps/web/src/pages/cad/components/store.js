...
export const useCadStore = create()((set, get) => ({
    ...
    redo: () => {
        const { _histIdx, _history, canvas } = get();
        if (!canvas || _histIdx >= _history.length - 1)
            return;
        const idx = _histIdx + 1;
        canvas.loadFromJSON(_history[idx], () => {
            applyGridPattern(canvas);
    ...
    importAny: async ({ name, text }) => {
        ...
        if (lower.endsWith(".svg")) {
            const { objects, options } = await new Promise((resolve, reject) => {
                fabric.loadSVGFromString(text, (objs, opts) => resolve({ objects: objs, options: opts }), (err) => reject(err));
            });
...
function applyGridPattern(c, step = 50) {
