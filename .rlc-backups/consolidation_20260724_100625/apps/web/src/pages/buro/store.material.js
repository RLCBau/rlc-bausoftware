...
export const MaterialDB = {
    ...
    create() {
        const it = { id: crypto.randomUUID(), name: "", code: "", unit: "Stk", stock: 0, minStock: 0, priceNet: 0,
    ...
    upsert(it) { const all = load(); const i = all.findIndex(x => x.id === it.id); if (i >= 0)
        all[i] = it;
    else
        all.push(it); save(all); },
    ...
    async attach(itemId, f) {
        ...
        if (!it)
            return;
        it.attachments = [a, ...(it.attachments || [])];
        it.updatedAt = Date.now();
        save(all);
    },
    exportCSV(rows) {
    ...
    importCSV(txt) {
        ...
        const all = load();
        for (const r of rows) {
            const it = { id: r[0] || crypto.randomUUID(), name: unesc(r[1] || ""), code: unesc(r[2] || ""), projectId: r[3] || "",
