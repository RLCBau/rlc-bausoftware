...
export const ProjekteDB = {
    list() { return read(); },
    get(id) { return read().find(p => p.id === id); },
    upsert(p) {
        const all = read();
        const i = all.findIndex(x => x.id === p.id);
        if (i >= 0)
            all[i] = { ...p, updatedAt: new Date().toISOString() };
        else
            all.unshift({ ...p, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() });
        write(all);
        return p.id;
    },
    create(partial) {
