...
function parseCsv(text) {
    ...
    const hasHeader = header.includes("pos") || header.includes("kurz") || header.includes("einheit") || header.includes("status");
...
export default function NachtraegePage() {
    ...
    async function load() {
        ...
        setLoading(true);
        try {
            const data = await apiJson(`/api/verknuepfung/nachtraege/${encodeURIComponent(projectKey)}`);
    ...
    async function persist(nextRows) {
        ...
        let existing = [];
        try {
            const d = await apiJson(`/api/verknuepfung/nachtraege/${encodeURIComponent(projectKey)}`);
    ...
    const viewRows = useMemo(() => {
        ...
        if (q.trim()) {
            ...
            r = r.filter((x) => (x.posNr || "").toLowerCase().includes(s) ||
...
03.0005;"Mehrlänge Speedpipe";m;85;36.5;Entwurf;"Auftraggeber wünscht zusätzliche Trasse"`;
    ...
    return (_jsxs("div", { style: { padding: 16 }, children: [!currentProject && (_jsxs("div", { style: {
                    ...
                    whiteSpace: "pre-wrap",
                                    ...
                                    return (_jsxs("tr", { style: { background: idx % 2 === 1 ? "#fcfcfc" : "#fff" }, children: [_jsx("td", { style: tdChk, children: _jsx("input", { type: "checkbox", checked: isSel, onChange: (e) => setSelected((s) => ({ ...s, [r.id]: e.target.checked })) }) }), _jsx("td", { style: td, children: _jsx("input", { value: r.posNr || "", onChange: (e) => save({ id: r.id, posNr: e.target.value }), style: inp(110) }) }), _jsx("td", { style: td, children: _jsx("input", { value: r.kurztext, onChange: (e) => save({ id: r.id, kurztext: e.target.value }), style: inp(500) }) }), _jsx("td", { style: td, children: _jsx("input", { value: r.einheit || "m", onChange: (e) => save({ id: r.id, einheit: e.target.value }), style: inp(60) }) }), _jsx("td", { style: tdNum, children: _jsx("input", { type: "number", value: r.mengeDelta, onChange: (e) => save({ id: r.id, mengeDelta: Number(e.target.value || 0) }), style: {
                                                        ...inp(120, "right"),
