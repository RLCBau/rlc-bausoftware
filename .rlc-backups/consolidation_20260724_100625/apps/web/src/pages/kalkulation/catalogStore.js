...
function saveChunks(json) {
    // pulisci vecchi
    Object.keys(localStorage).forEach(k => { if (k.startsWith(KEY + "_"))
...
function loadChunks() {
    ...
    let s = "";
    for (let i = 0; i < n; i++)
        s += localStorage.getItem(`${KEY}_${i}`) || "";
    return s;
}
function toNumber(v) {
    if (v == null || v === "")
...
export const Catalog = {
    ...
    clear() {
        Object.keys(localStorage).forEach(k => { if (k.startsWith(KEY))
            localStorage.removeItem(k); });
    },
    importCSV(text) {
        ...
        for (const l of lines) {
            ...
            const p = l.split(";");
            const posNr = p[0] ?? "";
            const kurztext = (() => { const s = p[1] ?? ""; try {
                return JSON.parse(s);
            }
            catch {
                return s;
