...
function toRPN(tokens) {
    ...
    const isFunc = (t, idx) => /[A-Z_][A-Z0-9_]*/.test(t) && tokens[idx + 1] === "(";
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (/^[0-9.]+$/.test(t) || /^[A-Z_][A-Z0-9_]*$/.test(t) && !isFunc(t, i)) {
            ...
            while (st.length && st[st.length - 1] !== "(")
                out.push(st.pop());
            if (!st.length)
                throw new Error("Separatore ',' fuori posto");
        }
        else if (t in prec) {
            while (st.length &&
...
function fnApply(name, stack) {
    const pop1 = () => { const v = stack.pop(); if (v == null)
        throw new Error("Argomenti insufficienti"); return v; };
    switch (name) {
        case "SUM": {
            const b = pop1(), a = pop1();
            stack.push(a + b);
...
export function evaluateExpression(expr, vars = {}) {
    ...
    for (const t of rpn) {
        if (/^[0-9.]+$/.test(t)) {
            ...
            if (a == null || b == null)
                throw new Error("Argomenti operatori insufficienti");
            switch (t) {
                case "+":
                    stack.push(a + b);
                    break;
            ...
            if (!hasVar && !hasConst)
                throw new Error(`Variabile sconosciuta: ${t}`);
