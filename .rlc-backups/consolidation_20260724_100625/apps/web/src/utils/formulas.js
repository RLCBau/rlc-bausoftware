...
function toRpn(tokens) {
    ...
    const isFunc = (t) => /^[A-Z_]+$/.test(t); // SUM, MIN, MAX
    const isNumOrVar = (t) => /^[0-9.]+$/.test(t) || /^[A-Z_][A-Z0-9_]*$/.test(t);
...
function evalRpn(rpn, ctx) {
    const st = [];
    const funcArgs = []; // collettore temporaneo
    const readValue = (t) => {
        if (/^[0-9.]+$/.test(t))
            return parseFloat(t);
        const fromCtx = ctx[t];
        if (typeof fromCtx === 'number' && !Number.isNaN(fromCtx))
            return fromCtx;
