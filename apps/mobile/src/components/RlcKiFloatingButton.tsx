...
import { Dimensions, Image, Keyboard, KeyboardAvoidingView, Platform, Modal, Pressable, Text, TextInput, View, ScrollView } from "react-native";
...
const RLC_KI_AVATAR_IMAGE = require("../../assets/rlc-copilot-avatar-3d.png");
...
function shouldOpenCopilotInstead(input: string) {
  ...
  if (!s) return false;
  const looksLikeHelpQuestion = /^(wie|was|warum|wieso|kannst|kann ich|erkläre|erklaere|hilfe|hilf|come|cosa|perché|perche|aiutami|spiegami)\b/i.test(s) || s.includes("wie kann ich") || s.includes("wie funktioniert") || s.includes("was bedeutet") || s.includes("come funziona");
  const looksLikeStructuredFormData = /(datum|baustelle|lieferschein|lieferant|rechnung|angebot|menge|material|fahrer|kennzeichen|mitarbeiter|gerät|geraet|tätigkeit|taetigkeit|wetter|foto|mangel|lv|position)\s*[:=]/i.test(s) || /\b\d+([,.]\d+)?\s*(m|m²|m2|m³|m3|stk|st|h|std|t|kg|psch)\b/i.test(s);
...
export default function RlcKiFloatingButton({
  ...
  const runKiWithMessage = async (message?: string) => {
    ...
    const forceCopilot = String(screen || "").toLowerCase().includes("mengen");
  ...
  return <>
      <Pressable style={styles.fab} onPress={openKi} hitSlop={10}>
        <Image source={RLC_KI_AVATAR_IMAGE} style={styles.avatar} resizeMode="cover" />
        <View style={styles.dot} />
      </Pressable>

      <Modal visible={panelOpen} transparent animationType="slide" onRequestClose={closePanel}>
        <KeyboardAvoidingView style={styles.modalAvoid} behavior={Platform.OS === "ios" ? "padding" : undefined} keyboardVerticalOffset={12}>
          <Pressable style={styles.overlay} onPress={closePanel}>
            <Pressable style={styles.panel} onPress={() => {}}>
            <View style={styles.panelHead}>
              <Image source={RLC_KI_AVATAR_IMAGE} style={styles.panelAvatar} resizeMode="cover" />
            ...
            {quickActions.length > 0 ? <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.quickScroll} contentContainerStyle={styles.quickContent}>
                {quickActions.map(a => <Pressable key={a} style={[styles.quickChip, running && {
                opacity: 0.65
              }]} onPress={() => runKiWithMessage(a)} disabled={running}>
            ...
            <TextInput value={input} onChangeText={setInput} placeholder="Schreiben oder sprechen: Was möchten Sie machen?" placeholderTextColor={COLORS.sub} style={styles.input} multiline />
            ...
            <View style={styles.actionRow}>
              ...
              <Pressable style={[styles.primaryBtn, running && {
                opacity: 0.65
              }]} onPress={openFullKi} disabled={running}>
...
const styles = createRlcStyles("RlcKiFloatingButton", {
  modalAvoid: {
    flex: 1
  },
  fab: {
