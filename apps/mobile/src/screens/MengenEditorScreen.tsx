...
function normalizeAngebotToMengen(a: any, params: {
  projectId: string;
  projectCode: string;
}) {
  ...
  return {
    ...
    title: String(a?.title || a?.angebotTitle || a?.angebotNr || "Mengenermittlung"),
    datum: new Date().toISOString().slice(0, 10),
    pdfUri: "",
    rows: angebotRows.length ? angebotRows.map((r: any, i: number) => normalizeMengenRow({
    ...
    }, i)) : [normalizeMengenRow({}, 0)]
  };
}
function normalizeStoredDoc(input: any, params: {
  projectId: string;
  projectCode: string;
  angebotId?: string | null;
}) {
  return {
    ...
    sourceType: String(input?.sourceType || (input?.angebotId ? "ANGEBOT" : "FREE")),
    sourceLocked: typeof input?.sourceLocked === "boolean" ? input.sourceLocked : !!input?.angebotId,
    angebotId: input?.angebotId ? String(input.angebotId) : null,
    angebotSnapshot: input?.angebotSnapshot ? deepClone(input.angebotSnapshot) : null,
    ...
    rows: Array.isArray(input?.rows) && input.rows.length ? input.rows.map((r: any, i: number) => normalizeMengenRow(r, i)) : [normalizeMengenRow({}, 0)]
  };
}
...
function pickBestOfferForMengen(list: any[]) {
  ...
  const accepted = rows.filter((x: any) => String(x?.status || "").trim().toLowerCase() === "angenommen");
...
export default function MengenEditorScreen({
  route,
  navigation
}: Props) {
  const {
  ...
  async function readMengenList() {
    ...
    try {
      ...
      const server = await api.getMengen(projectCode);
      const combined = [...(Array.isArray(server) ? server : []), ...(Array.isArray(local) ? local : [])];
      const safe = combined.filter((item: any, index: number, all: any[]) => all.findIndex((candidate: any) => String(candidate?.id) === String(item?.id)) === index);
  ...
  async function load() {
    try {
      ...
      if (mengenId) {
        const found = mengenList.find((x: any) => String(x?.id) === String(mengenId));
        if (found) {
          setDoc(normalizeStoredDoc(found, {
            projectId,
            projectCode,
      ...
      if (angebotId) {
        const existingFromAngebot = mengenList.find((x: any) => String(x?.angebotId || "") === String(angebotId));
        if (existingFromAngebot) {
          setDoc(normalizeStoredDoc(existingFromAngebot, {
            projectId,
            projectCode,
            angebotId
          }));
        ...
        const angebot = offerList.find((x: any) => String(x?.id) === String(angebotId)) || null;
        if (angebot) {
          setDoc(normalizeAngebotToMengen(angebot, {
            projectId,
            projectCode
          }));
          return;
      ...
      if (bestOffer?.id) {
        const existingFromBestOffer = mengenList.find((x: any) => String(x?.angebotId || "") === String(bestOffer.id));
        if (existingFromBestOffer) {
          setDoc(normalizeStoredDoc(existingFromBestOffer, {
            projectId,
            projectCode,
        ...
        setDoc(normalizeAngebotToMengen(bestOffer, {
          projectId,
          projectCode
        }));
        return;
      ...
      if (!angebotId && !mengenId && mengenList.length > 0) {
        ...
        if (latestMengen) {
          setDoc(normalizeStoredDoc(latestMengen, {
            projectId,
            projectCode,
  ...
  function addRow() {
    setDoc((prev: any) => ({
      ...prev,
      rows: [...prev.rows, normalizeMengenRow({
  ...
  function removeRow(i: number) {
    setDoc((prev: any) => {
      const rows = [...prev.rows];
      rows.splice(i, 1);
      const normalizedRows = rows.length > 0 ? rows.map((r: any, idx: number) => normalizeMengenRow({
        ...r,
        pos: String(idx + 1)
      }, idx)) : [normalizeMengenRow({}, 0)];
      return {
        ...prev,
        rows: normalizedRows,
        updatedAt: Date.now()
  ...
  const netto = useMemo(() => {
    return (Array.isArray(doc?.rows) ? doc.rows : []).reduce((s: number, r: any) => s + num(r.qty) * num(r.ep), 0);
  }, [doc.rows]);
  async function persistDoc(customDoc?: any) {
    ...
    const normalized = {
      ...
      sourceType: String(base.sourceType || (base.angebotId ? "ANGEBOT" : "FREE")),
      sourceLocked: typeof base.sourceLocked === "boolean" ? base.sourceLocked : !!base.angebotId,
      angebotId: base.angebotId || null,
      angebotSnapshot: base.angebotSnapshot ? deepClone(base.angebotSnapshot) : null,
      pdfUri: String(base.pdfUri || ""),
      createdAt: Number(base.createdAt || Date.now()),
      updatedAt: Date.now(),
      rows: Array.isArray(base.rows) && base.rows.length ? base.rows.map((r: any, i: number) => normalizeMengenRow(r, i)) : [normalizeMengenRow({}, 0)]
    };
    const raw = await AsyncStorage.getItem(KEY + projectCode);
    const list = raw ? JSON.parse(raw) : [];
    const next = Array.isArray(list) ? list : [];
    const idx = next.findIndex((x: any) => String(x.id) === String(normalized.id));
    if (idx >= 0) next[idx] = normalized;else next.unshift(normalized);
  ...
  async function exportPdf() {
    try {
      ...
      const fileNameBase = String(saved.title || "mengenermittlung").trim().replace(/[\\/:*?"<>|]/g, "_").replace(/\s+/g, "_");
      const {
        pdfUri
        ...
        title: "MENGENERMITTLUNG",
        subTitle: saved?.sourceType === "ANGEBOT" ? "Basierend auf angenommenem Angebot" : "Freie Mengenermittlung",
        date: saved?.datum || "",
        ...
        totals: {
          netto: (saved.rows || []).reduce((sum: number, r: any) => sum + num(r.qty) * num(r.ep), 0)
        },
        extraBlocks: [{
          title: "Dokument",
          lines: [`Titel: ${String(saved.title || "Mengenermittlung")}`, `Quelle: ${saved.sourceType === "ANGEBOT" ? "Angebot" : "Freie Mengenermittlung"}`, ...(saved.angebotId ? [`Basis-Angebot: ${String(saved.angebotId)}`] : [])]
        }],
      ...
      if (pdfUri) {
        navigation.navigate("PdfViewer", {
          uri: pdfUri,
          title: saved.title || "Mengenermittlung",
          projectId,
          projectCode,
  ...
  async function createRechnung() {
    try {
      ...
      Alert.alert("Fehler", "Die Mengenermittlung konnte vor dem Wechsel zur Rechnung nicht gespeichert werden.");
    } finally {
      setBusy(false);
    }
  }
    ...
    return registerRlcKiModuleHandler("MengenEditor", async (payload: any) => {
      ...
      setDoc((prev: any) => {
        ...
        return {
          ...
          rows: parsedRows,
          note: parsed.warnings?.length ? `RLC KI Hinweise:
${parsed.warnings.map((w: string) => `- ${w}`).join("\n")}` : prev?.note,
  ...
  return <SafeAreaView style={s.safe}>
      <ScrollView contentContainerStyle={s.wrap}>
        ...
        <View style={s.infoBox}>
          ...
          <Text style={s.infoText}>
            {doc.sourceType === "ANGEBOT" ? "Diese Mengenermittlung basiert auf einem Angebot." : "Freie Mengenermittlung."}
          </Text>
          {doc.angebotId ? <Text style={s.infoSmall}>Angebot-ID: {String(doc.angebotId)}</Text> : null}
          {doc.sourceLocked ? <Text style={s.infoSmall}>Quelle ist als Snapshot gespeichert.</Text> : null}
          {doc.pdfUri ? <Text style={s.infoSmall}>PDF vorhanden.</Text> : null}
        </View>

        <Text style={s.label}>Titel</Text>
        <TextInput value={String(doc.title || "")} onChangeText={v => setDoc((p: any) => ({
        ...p,
        title: v,
        updatedAt: Date.now()
      }))} style={s.input} placeholder="Titel" placeholderTextColor={COLORS.sub} />

        <Text style={s.label}>Datum</Text>
        <TextInput value={String(doc.datum || "")} onChangeText={v => setDoc((p: any) => ({
        ...p,
        datum: v,
        updatedAt: Date.now()
        ...
        return <View key={r.id} style={s.rowCard}>
              ...
              <TextInput placeholder="Beschreibung" value={String(r.text || "")} onChangeText={v => updateRow(i, "text", v)} style={s.input} placeholderTextColor={COLORS.sub} />

              <View style={s.row}>
                <TextInput placeholder="Einheit" value={String(r.unit || "")} onChangeText={v => updateRow(i, "unit", v)} style={s.inputSmall} placeholderTextColor={COLORS.sub} />

                <TextInput placeholder="Menge" value={String(r.qty || "")} onChangeText={v => updateRow(i, "qty", v)} style={s.inputSmall} placeholderTextColor={COLORS.sub} keyboardType="decimal-pad" />

                <TextInput placeholder="EP" value={String(r.ep || "")} onChangeText={v => updateRow(i, "ep", v)} style={s.inputSmall} placeholderTextColor={COLORS.sub} keyboardType="decimal-pad" />
              </View>

              <TextInput placeholder="Formel (z.B. 10*2*1.2)" value={String(r.formula || "")} onChangeText={v => updateFormula(i, v)} style={s.input} placeholderTextColor={COLORS.sub} />
        ...
        <Pressable style={s.pdfBtn} onPress={exportPdf} disabled={busy}>
          <Text style={s.btnTxt}>
            {busy ? "Bitte warten..." : doc.pdfUri ? "PDF" : "PDF"}
          </Text>
        </Pressable>
      ...
      <RlcKiFloatingButton projectId={projectId} projectCode={projectCode} title="Mengenermittlung" screen="MengenEditor" initialMessage="Ich bin in der Mengenermittlung. Hilf mir konkret mit Mengen, Formeln, Positionen, Aufmaß und PDF." />
    </SafeAreaView>;
}
const s = createRlcStyles("MengenEditorScreen", {
  safe: {
    flex: 1,
