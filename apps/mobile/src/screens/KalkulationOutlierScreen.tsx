...
async function loadOutliers(projectId: string, projectCode?: string): Promise<OutlierRow[]> {
  ...
  const apiCandidates = [
    () => anyApi.getMobileKalkulationOutliers?.(projectId, projectCode),
    () => anyApi.getProjectKalkulationOutliers?.(projectId, projectCode),
    () => anyApi.getKalkulationOutliers?.(projectId, projectCode),
  ...
  const urls = [
    `${base}/api/mobile/projects/${encodedId}/kalkulation/outliers`,
    `${base}/api/projects/${encodedId}/kalkulation/outliers`,
    `${base}/api/project-lv/${encodedId}`,
  ];
...
export default function KalkulationOutlierScreen() {
  ...
  const { projectId, projectCode, title: projectTitle } = route.params;
  ...
  const askCopilot = (row?: OutlierRow) => {
    navigation.navigate("RlcCopilot", {
      ...
      initialMessage: row
        ? `Prüfe diesen Outlier fachlich: Position ${pos(row)} ${title(row)}. Hinweis: ${warning(row)}`
        : "Analysiere den Outlier Report dieses Projekts und priorisiere die wichtigsten Risiken.",
    });
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.actions}>
        ...
        <Pressable style={styles.secondary} onPress={() => navigation.navigate("Kalkulation", { projectId, projectCode, title: projectTitle, filter: "ALL" })}><Text style={styles.secondaryText}>Kalkulation</Text></Pressable>
      </View>
      ...
      <FlatList
        data={rows}
        keyExtractor={(item, index) => String(item.id || item.positionId || `${pos(item)}-${index}`)}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(); }} />}
        contentContainerStyle={rows.length ? styles.list : styles.emptyList}
        ListEmptyComponent={<Text style={styles.empty}>Keine Outlier gefunden.</Text>}
        renderItem={({ item }) => (
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: COLORS.bg, padding: 16, paddingBottom: 10 },
  title: { color: COLORS.text, fontWeight: "900", fontSize: 28, lineHeight: 34 },
  sub: { color: COLORS.sub, marginTop: 4, fontWeight: "800" },
  statsRow: { flexDirection: "row", gap: 10, marginTop: 14 },
  stat: { flex: 1, backgroundColor: COLORS.card, borderRadius: 14, padding: 12, borderWidth: 1, borderColor: COLORS.border, minHeight: 74 },
  ...
  pos: { color: COLORS.text, fontWeight: "900" },
  gp: { color: COLORS.text, fontWeight: "900" },
