...
async function resolveApiBase(): Promise<string> {
  try {
    ...
    if (
      stored &&
      /^https:\/\/.+/i.test(stored) &&
      !/localhost|127\.0\.0\.1/i.test(stored)
    ) {
  ...
  if (
    ...
    !/localhost|127\.0\.0\.1/i.test(envBase)
  ) {
...
function normalizeImportPayload(raw: unknown): ImportedPayload {
  ...
  const items: NormalizedImportedItem[] = itemsRaw
    .map((it: ImportedItem, i: number) => {
      const pos = String(it?.pos ?? it?.position ?? i + 1).trim();
      const text = String(it?.text ?? it?.kurztext ?? "").trim();
      const langtext =
        it?.langtext !== undefined && it?.langtext !== null
          ? String(it.langtext).trim()
          : "";
      const unit = String(it?.unit ?? it?.einheit ?? "").trim();

      const quantity =
        it?.quantity !== undefined && it?.quantity !== null
          ? toSafeNumber(it.quantity, 0)
          : it?.menge !== undefined && it?.menge !== null
          ? toSafeNumber(it.menge, 0)
          : 0;

      const ep =
        it?.ep !== undefined && it?.ep !== null
          ? toSafeNumber(it.ep, 0)
          : it?.einzelpreis !== undefined && it?.einzelpreis !== null
          ? toSafeNumber(it.einzelpreis, 0)
          : 0;
...
function parseXmlLikeToPayload(text: string, filename?: string): ImportedPayload {
  ...
  const title =
    getTag(clean, ["title", "titel", "boqname", "name", "projekt", "project"]) ||
    filename ||
    "Importiertes LV";

  const itemChunks =
    clean.match(/<(Item|Position|BoQCtgy|BoQItem|Ausschreibungsposition)\b[\s\S]*?<\/\1>/gi) || [];

  const items: NormalizedImportedItem[] = itemChunks
    .map((chunk: string, i: number) => {
      const pos =
        getTag(chunk, ["OZ", "Ordnungszahl", "PosNr", "Positionsnummer", "Position", "Pos"]) ||
        String(i + 1);
...
function extractLooseXmlRows(text: string): NormalizedImportedItem[] {
  ...
  const ozMatches = Array.from(
    text.matchAll(
      /<(OZ|Ordnungszahl|PosNr|Positionsnummer|Position|Pos)[^>]*>([\s\S]*?)<\/\1>/gi
    )
  );
...
function buildProjectRef(params: Record<string, unknown>): {
  ...
  const rawProjectId = String(
    params?.projectId ?? params?.id ?? (params as any)?.project?.id ?? ""
  ).trim();

  const rawProjectCode = String(
    params?.projectCode ?? params?.code ?? (params as any)?.project?.code ?? ""
  ).trim();

  const fallbackRef = rawProjectId || rawProjectCode || "local-project";

  const displayCode =
    rawProjectCode || (rawProjectId.toUpperCase().startsWith("BA-") ? rawProjectId : rawProjectId || "—");
...
function buildCacheKeys(projectId: string, projectCode: string): string[] {
  const refs = Array.from(
    new Set([projectId, projectCode].map((v) => String(v || "").trim()).filter(Boolean))
  );
  ...
  const prefixes = [
    ...
    "project_lv_cache:",
    "rlc_project_lv:",
    "rlc_lv_readonly_cache:",
  ];

  return prefixes.flatMap((prefix) => refs.map((ref) => `${prefix}${ref}`));
}

async function copyFileToLocalLvFolder(
  projectRef: string,
...
async function saveOfflineLvCache(params: {
  ...
  const { projectId, projectCode, payload, sourceType, file } = params;

  const fileUri =
    file && file.uri ? await copyFileToLocalLvFolder(projectCode || projectId, file) : undefined;
  ...
  await Promise.all([
    ...keys.map((key) => AsyncStorage.setItem(key, richPayload)),
    AsyncStorage.setItem(`rlc_lv_payload:${projectId}`, flatPayload),
    AsyncStorage.setItem(`rlc_lv_payload:${projectCode}`, flatPayload),
    AsyncStorage.setItem(`rlc_lv_last_import:${projectId}`, richPayload),
    AsyncStorage.setItem(`rlc_lv_last_import:${projectCode}`, richPayload),
  ]);

  return cache;
}

function buildSyntheticPayloadForBinaryFile(
  type: SelectedType,
  file: DocumentPicker.DocumentPickerAsset
): ImportedPayload {
    ...
    items: [
      {
        ...
        langtext:
          `Datei "${name}" wurde lokal gespeichert. ` +
          `Im Offline-Modus wurde ein Platzhalter-LV angelegt, damit das Projekt direkt weiterarbeiten kann.`,
        unit: "",
        quantity: 1,
        ep: 0,
      },
...
async function buildOfflinePayloadFromFile(
  file: DocumentPicker.DocumentPickerAsset,
  type: SelectedType
...
async function uploadImportFile(params: {
  ...
  const { projectCode, file, sourceType } = params;
  ...
  try {
    ...
    throw new Error(
      `Ungültige Server-Antwort (${res.status}). URL: ${url}\n\nAntwort: ${
        rawText?.slice(0, 400) || "leer"
      }`
    );
  }
...
export default function LvImportScreen({ route, navigation }: Props) {
    ...
    try {
      ...
      const version =
        typeof result?.version === "number"
          ? result.version
          : typeof result?.lv?.version === "number"
          ? result.lv.version
          : undefined;
    ...
    <SafeAreaView style={s.safe}>
      <ScrollView contentContainerStyle={s.wrap} showsVerticalScrollIndicator={false}>
        <View style={s.headerCard}>
          ...
          <Text style={s.sub}>
            {offlineMode
              ? "Leistungsverzeichnis lokal in den Projekt-Cache übernehmen."
              : "Leistungsverzeichnis aus Datei laden und direkt ins Projekt übernehmen."}
          </Text>
        </View>
        ...
        <View style={s.card}>
          ...
          <View style={s.infoBox}>
            <Text style={s.infoText}>
              {offlineMode
                ? `${projectMeta.displayCode} • ID: ${projectId || "—"}`
                : projectMeta.displayCode || "—"}
            </Text>
          </View>
        ...
        {preview ? (
          <View style={s.card}>
            ...
            <View style={s.previewList}>
              {preview.items.slice(0, 3).map((it: NormalizedImportedItem, idx: number) => (
                <View key={`${it.pos}-${idx}`} style={s.previewRow}>
            ...
            {preview.items.length > 3 ? (
              <Text style={s.previewMore}>+ {preview.items.length - 3} weitere Positionen</Text>
            ) : null}
          </View>
        ) : null}
        ...
        <Text style={s.hint}>
          {offlineMode
            ? "Im Modus ohne Server wird die Datei lokal gespeichert und als LV-Cache für das Projekt abgelegt."
            : "Dieser Screen lädt direkt zu /api/project-lv/:projectCode/import-file hoch."}
        </Text>
      </ScrollView>
    </SafeAreaView>
  );
