...
function rowEp(r: KiRow): number {
  return n(r?.rlcKiUnitPrice ?? r?.kiUnitPrice ?? r?.calculatedUnitPrice ?? r?.finalUnitPrice ?? r?.suggestedUnitPrice ?? r?.unitPriceNet ?? r?.unitPrice ?? r?.ep ?? r?.preis);
}
...
function payloadTotal(payload: any): number {
  return n(payload?.summary?.rlcKiNet ?? payload?.summary?.rlcKiTotalNet ?? payload?.summary?.totalNet ?? payload?.summary?.netto ?? payload?.kalkulation?.summary?.rlcKiNet ?? payload?.kalkulation?.summary?.totalNet ?? payload?.calculation?.summary?.rlcKiNet ?? payload?.calculation?.summary?.totalNet ?? payload?.data?.summary?.rlcKiNet ?? payload?.data?.summary?.totalNet ?? payload?.data?.kalkulation?.summary?.rlcKiNet ?? payload?.data?.kalkulation?.summary?.totalNet ?? payload?.totalNet ?? payload?.netto);
}
...
function rowHasCalculatedMoney(r: KiRow): boolean {
  return n(r?.rlcKiUnitPrice) > 0 || n(r?.kiUnitPrice) > 0 || n(r?.calculatedUnitPrice) > 0 || n(r?.finalUnitPrice) > 0 || n(r?.suggestedUnitPrice) > 0 || n(r?.unitPriceNet) > 0 || n(r?.ep) > 0 || n(r?.rlcKiTotal) > 0 || n(r?.kiTotal) > 0 || n(r?.calculatedTotal) > 0 || n(r?.totalNet) > 0 || n(r?.gp) > 0;
}
...
function normalizeSummary(payload: any, loadedOnly = false): KiSummary | null {
  ...
  const warningCount = n(summary?.warningCount ?? summary?.warnings) || rows.filter((r: any) => r.warning || r.pruefHinweis || Array.isArray(r.warnings) && r.warnings.length).length;
  const reviewCount = n(summary?.reviewCount ?? summary?.needsReview ?? summary?.review) || rows.filter((r: any) => String(r.calculationStatus || r.status || "").toLowerCase().includes("review")).length;
  const outlierCount = n(summary?.outlierCount ?? summary?.outliers) || rows.filter((r: any) => String(r.warning || r.pruefHinweis || "").toLowerCase().includes("outlier")).length;
  return {
    ok: payload?.ok ?? true,
...
async function readExistingCalculation(projectId: string, projectCode?: string): Promise<KiSummary> {
  ...
  const urls = ids.flatMap(id => [`${base}/api/mobile/projects/${id}/kalkulation`, `${base}/api/projects/${id}/kalkulation`, `${base}/api/kalkulation/projects/${id}`, `${base}/api/kalkulation/project/${id}`, `${base}/api/project-kalkulation/${id}`]);
  let lastError = "";
...
async function runKiCalculation(projectId: string, projectCode?: string, forceRecalculate?: boolean): Promise<KiSummary> {
  const anyApi = api as any;
  const apiCandidates = [() => anyApi.startKiCalculationJob?.({
    projectId,
    projectCode,
    forceRecalculate
  }), () => anyApi.runMobileKiCalculation?.({
    projectId,
    projectCode,
    forceRecalculate
  }), () => anyApi.runProjectKiCalculation?.(projectId, {
    projectCode,
    forceRecalculate
  ...
  for (const fn of apiCandidates) {
    try {
      ...
      return {
        ...
        source: "server",
        engine: "RLC-KI",
        message: "Serverprozess gestartet. Bitte vorhandene Berechnung später laden."
      };
    } catch {
      // fallback
    }
  ...
  const urls = [`${base}/api/mobile/projects/${encodedId}/kalkulation/ki/run`, `${base}/api/projects/${encodedId}/kalkulation/ki/run`, `${base}/api/kalkulation/ki/project/${encodedId}/run`];
  let lastError = "";
  for (const url of urls) {
    try {
      const payload = await withTimeout(postJson(url, {
        projectId,
        projectCode,
...
export default function KiCalculationScreen() {
  ...
  const confirmRecalculate = useCallback(() => {
    if (summary?.hasSavedCalculation || summary?.count || summary?.totalNet) {
      Alert.alert("KI neu berechnen?", "Es existiert bereits eine Kalkulation für dieses Projekt. Willst du sie wirklich neu mit KI berechnen? Die bisherige Server-Kalkulation kann überschrieben werden.", [{
        text: "Abbrechen",
        style: "cancel"
      }, {
        text: "Neu berechnen",
        style: "destructive",
        onPress: startRecalculate
      }]);
      return;
    }
    Alert.alert("KI-Kalkulation starten?", "Mobile startet nur den Serverprozess. Das Handy berechnet die  Positionen nicht lokal.", [{
      text: "Abbrechen",
      style: "cancel"
    }, {
      text: "Starten",
  ...
  const openKalkulation = () => navigation.navigate("Kalkulation", {
    projectId,
    projectCode,
    title
  });
  ...
  const openCopilot = () => navigation.navigate("RlcCopilot", {
    ...
    initialMessage: "Analysiere die KI-Kalkulation dieses Projekts."
  });
  return <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {summary && <View style={styles.resultBox}>
...
const styles = createRlcStyles("KiCalculationScreen", {
  container: {
    flex: 1,
