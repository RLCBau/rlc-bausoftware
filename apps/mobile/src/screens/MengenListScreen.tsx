...
import { View, Text, Pressable, SafeAreaView, FlatList, Alert, TextInput, Platform } from "react-native";
...
export default function MengenListScreen({
  route,
  navigation
}: Props) {
  const {
    ...
    Alert.alert("Löschen", "Mengenermittlung wirklich löschen?", [{
      ...
      onPress: async () => {
        try {
          const raw = await AsyncStorage.getItem(KEY + projectCode);
          const list = raw ? JSON.parse(raw) : [];
          const next = Array.isArray(list) ? list.filter((x: any) => String(x?.id) !== String(id)) : [];
          await AsyncStorage.setItem(KEY + projectCode, JSON.stringify(next));
          setItems(next.map(normalizeItem));
        } catch (e: any) {
          Alert.alert("Fehler", String(e?.message || "Mengenermittlung konnte nicht gelöscht werden."));
        }
      }
    }]);
  }
  const filtered = useMemo(() => {
    ...
    return items.filter(item => {
      ...
      const hay = [item?.title, item?.datum, item?.angebotId, sourceLabel(item), money(total)].join(" ").toLowerCase();
      return hay.includes(term);
    });
  }, [items, q]);
  const totalDocs = filtered.length;
  const totalRows = useMemo(() => filtered.reduce((sum, item) => sum + (Array.isArray(item?.rows) ? item.rows.length : 0), 0), [filtered]);
  const totalValue = useMemo(() => filtered.reduce((sum, item) => sum + calcTotal(item?.rows || []), 0), [filtered]);
  return <SafeAreaView style={s.safe}>
      <FlatList data={filtered} keyExtractor={(item, idx) => String(item?.id || `mengen-${idx}-${Date.now()}`)} contentContainerStyle={filtered.length === 0 ? s.emptyListContent : s.listContent} showsVerticalScrollIndicator={false} ListHeaderComponent={<View>
            ...
            screen: "MengenList"
              ...
              <Pressable style={s.primaryBtn} onPress={() => navigation.navigate("MengenEditor", {
          projectId,
          projectCode,
          title: "Mengenermittlung"
        })}>
              ...
              <Pressable style={s.reloadBtn} onPress={() => void load()} disabled={busy}>
                <Text style={s.reloadBtnTxt}>
                  {busy ? "Lädt..." : "Liste neu laden"}
                </Text>
              </Pressable>

              <TextInput value={q} onChangeText={setQ} placeholder="Suchen (Titel, Datum, Angebot, Wert)…" placeholderTextColor={COLORS.sub} style={s.search} autoCorrect={false} autoCapitalize="none" />
      ...
      return <View style={s.card}>
              ...
              {!!item?.angebotId ? <Text style={s.infoLine}>
                  Angebot: <Text style={s.infoStrong}>{item.angebotId}</Text>
                </Text> : <Text style={s.infoLine}>
                  Quelle: <Text style={s.infoStrong}>Frei</Text>
                </Text>}

              <View style={s.rowBtns}>
                <Pressable style={[s.btn, s.btnPrimary]} onPress={() => navigation.navigate("MengenEditor", {
            projectId,
            projectCode,
            title: "Mengenermittlung",
            ...
            angebotId: item?.angebotId || undefined
                ...
                <Pressable style={[s.btn, s.btnOrange]} onPress={() => navigation.navigate("RechnungEditor", {
            projectId,
            projectCode,
            title: "Rechnung",
            fromMengen: true,
            mengenId: String(item.id)
              ...
              <View style={s.rowBtns}>
                <Pressable style={[s.btn, s.btnGhost]} onPress={() => navigation.navigate("RechnungList", {
            projectId,
            projectCode,
            title: "Rechnungen"
          })}>
                ...
                <Pressable style={[s.btn, s.btnDanger]} onPress={() => removeItem(String(item.id))}>
            ...
            <Text style={s.emptyTitle}>
              {q.trim() ? "Keine passende Mengenermittlung gefunden" : "Noch keine Mengenermittlung"}
            </Text>
            <Text style={s.emptySub}>
              {q.trim() ? "Passe den Suchbegriff an oder lade die Liste neu." : "Erstelle die erste Mengenermittlung für dieses Projekt."}
            </Text>
          </View>} />
    </SafeAreaView>;
}
...
const s = createRlcStyles("MengenListScreen", {
  safe: {
    flex: 1,
