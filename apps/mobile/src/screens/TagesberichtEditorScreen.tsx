...
function summarizeWorkDone(lines?: TagesberichtLine[]) {
  ...
  return arr
    .map((x) => {
      const parts = [
        String(x?.taetigkeit || "").trim(),
        String(x?.ort || "").trim() ? `Ort: ${String(x.ort).trim()}` : "",
        String(x?.mitarbeiter || "").trim()
          ? `Mitarbeiter: ${String(x.mitarbeiter).trim()}`
          : "",
        String(x?.maschine || "").trim()
...
async function saveInboxRows(projectKey: string, rows: TagesberichtRow[]) {
  await AsyncStorage.setItem(
    INBOX_KEY_TAGESBERICHT(projectKey),
    JSON.stringify(rows)
  );
}
...
export default function TagesberichtEditorScreen({
  ...
  const { projectId, projectCode, title, tagesberichtId } = route.params as any;

  const projectKey = useMemo(
    () => String(projectCode || projectId || "").trim(),
  ...
  const updateLine = useCallback(
    (lineId: string, key: keyof TagesberichtLine, value: string) => {
      setData((prev) => {
        const nextLines = (prev.lines || []).map((line) => {
          if (line.id !== lineId) return line;

          const nextLine: TagesberichtLine = { ...line };
          ...
          const pause =
            key === "pauseMin"
              ? Number(digitsOnly(value) || 0)
              : Number(nextLine.pauseMin || 0);
  ...
  const validate = useCallback(() => {
    ...
    if (!hasValidLine && !String(data.workDone || "").trim()) {
      Alert.alert(
        "Pflichtfeld",
        "Bitte mindestens eine Tätigkeitszeile ausfüllen."
      );
      return false;
    }
  ...
  const onSave = useCallback(async () => {
    try {
      ...
      const [list, inboxList, modeRaw] = await Promise.all([
        loadRows(projectKey),
        loadInboxRows(projectKey),
        AsyncStorage.getItem(KEY_MODE),
      ]);

      const mode = String(modeRaw || "SERVER_SYNC").trim() as
        | "NUR_APP"
        | "SERVER_SYNC";

      const now = Date.now();

      const normalizedLines = (Array.isArray(data.lines) ? data.lines : [])
        .map((line) => {
          ...
          const stunden =
            von && bis
              ? calcHours(von, bis, pauseMin)
              : Number(line.stunden || 0);
      ...
      const nextWorkDone =
        summarizeWorkDone(normalizedLines) || String(data.workDone || "").trim();
      ...
      const inboxIdx = inboxList.findIndex(
        (x) => String(x.id) === String(inboxRow.id)
      );
      ...
      if (serverSyncError) {
        Alert.alert(
          "Lokal gespeichert",
          "Tagesbericht wurde lokal gespeichert und in die lokale Eingang / Prüfung gelegt.\n\n" +
      ...
      Alert.alert(
        ...
        mode === "SERVER_SYNC"
          ? isEdit
            ? "Tagesbericht wurde aktualisiert und an Eingang / Prüfung übertragen."
            : "Tagesbericht wurde gespeichert und an Eingang / Prüfung übertragen."
          : isEdit
          ? "Tagesbericht wurde lokal aktualisiert."
          : "Tagesbericht wurde lokal gespeichert.",
        [
          {
            text: "OK",
            onPress: () => navigation.goBack(),
    ...
    <SafeAreaView style={s.safe}>
      ...
      >
        <View style={s.headerCard}>
          <View style={s.headerTopRow}>
            <View style={s.headerTitleWrap}>
              ...
              <Text style={s.h1}>Tagesbericht</Text>
            ...
            <View key={line.id} style={s.lineCard}>
              ...
              <View style={s.twoColRow}>
                <View style={s.col}>
                  ...
                  <TextInput
                    value={
                      line.pauseMin == null || Number(line.pauseMin) === 0
                        ? ""
                        : String(line.pauseMin)
                    }
              ...
              <TextInput
                value={String(line.taetigkeit || "")}
                onChangeText={(v) => updateLine(line.id, "taetigkeit", v)}
              ...
              <TextInput
                value={String(line.notiz || "")}
                onChangeText={(v) => updateLine(line.id, "notiz", v)}
        ...
        <View style={s.card}>
          ...
          <TextInput
            value={String(data.issues || "")}
            onChangeText={(v) => update("issues", v)}
            style={[s.input, s.textarea]}
            multiline
        ...
        <View style={s.actionRow}>
          <Pressable
            style={[s.secondaryBtn, saving ? s.btnDisabled : null]}
            onPress={onReset}
            disabled={saving}
          >
          ...
          <Pressable
            style={[s.saveBtn, saving ? s.btnDisabled : null]}
            onPress={onSave}
            disabled={saving}
          >
