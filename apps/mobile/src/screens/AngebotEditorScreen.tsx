...
import { View, Text, TextInput, Pressable, ScrollView, SafeAreaView, KeyboardAvoidingView, Platform, Alert, Linking } from "react-native";
...
function normalizeOfferDoc(input: any): AngebotDoc {
  return {
    ...
    rows: Array.isArray(input?.rows) && input.rows.length ? input.rows.map((r: any, i: number) => ({
...
async function loadOfferSmart(projectCode: string, angebotId: string) {
  ...
  try {
    ...
    const found = Array.isArray(list) ? list.find((x: any) => String(x?.id) === String(angebotId)) : null;
...
async function saveOfferSmart(doc: AngebotDoc) {
  ...
  // Il documento ufficiale sul server viene creato soltanto dopo Freigabe.
...
async function syncAngebotToMengenAuto(saved: any) {
  ...
  const rows = Array.isArray(saved?.rows) ? saved.rows.map((r: any, i: number) => ({
  ...
  if (existingIdx >= 0) next[existingIdx] = mengenDoc;else next.unshift(mengenDoc);
...
async function exportOfferPdf(params: {
  ...
  const out = await buildDocumentPdf({
    ...
    extraBlocks: [{
      ...
      lines: [doc.baustelle || "", doc.validUntil ? `Gültig bis: ${doc.validUntil}` : ""].filter(Boolean)
    }, {
      title: "Zusammenfassung",
      lines: [`Netto: ${fmtMoney(netto)} €`, `Rabatt: ${fmtMoney(rabattValue)} €`, `Zuschlag: ${fmtMoney(zuschlagValue)} €`, `Netto gesamt: ${fmtMoney(nettoFinal)} €`, `MwSt.: ${fmtMoney(mwstValue)} €`, `Brutto: ${fmtMoney(brutto)} €`]
    }],
    note: doc.note || "",
...
async function exportOfferExcel(params: {
  ...
  const summary = [{
    ...
    Feld: "Angebotstitel",
    Wert: doc.angebotTitle || ""
  }, {
    Feld: "Status",
    Wert: doc.status || "Entwurf"
  }, {
    Feld: "Kunde",
    Wert: doc.customerName || ""
  }, {
    Feld: "Adresse",
    Wert: doc.customerAddress || ""
  }, {
    Feld: "E-Mail",
    Wert: doc.customerEmail || ""
  }, {
    Feld: "Telefon",
    Wert: doc.customerPhone || ""
  }, {
    Feld: "Baustelle",
    Wert: doc.baustelle || ""
  }, {
    ...
    Feld: "Rabatt %",
    Wert: toNum(doc.rabattPct)
  }, {
    Feld: "Zuschlag %",
    Wert: toNum(doc.zuschlagPct)
  }, {
    Feld: "MwSt %",
    Wert: toNum(doc.mwstPct)
  }, {
    Feld: "Zwischensumme netto",
    Wert: netto
  }, {
    Feld: "Rabatt Wert",
    Wert: rabattValue
...
export default function AngebotEditorScreen({
  route,
  navigation
}: Props) {
  const {
  ...
  const [doc, setDoc] = useState<AngebotDoc>(() => createEmptyOffer({
    projectId,
    projectCode,
    title
  }));
  ...
  const hydrate = useCallback(async () => {
    try {
      ...
      Alert.alert("Angebot", String(e?.message || "Angebot konnte nicht geladen werden"));
      setLoaded(true);
    } finally {
      setBusy(false);
    }
  ...
  function showLockedAlert() {
    Alert.alert("Gesperrt", "Dieses Angebot ist angenommen und kann nicht mehr bearbeitet werden.");
  }
  ...
  const netto = useMemo(() => calcRows.reduce((sum, r) => sum + r.gp, 0), [calcRows]);
  ...
  const nettoFinal = useMemo(() => afterRabatt + zuschlagValue, [afterRabatt, zuschlagValue]);
  ...
  async function onSave() {
    try {
      setBusy(true);
      if (isAccepted) {
        Alert.alert("Gesperrt", "Dieses Angebot ist angenommen und kann nicht mehr geändert werden.");
        return;
      }
      ...
      Alert.alert("Fehler", String(e?.message || "Angebot konnte nicht gespeichert werden"));
    } finally {
      setBusy(false);
    }
  }
  async function onExportPdf() {
    try {
      ...
      const withPdf = await saveOfferSmart({
        ...(saved as any),
        pdfUri
      } as any);
  ...
  async function onCreateMengen() {
    try {
      ...
      if (saved.status !== "Angenommen") {
        Alert.alert("Mengenermittlung", "Eine Mengenermittlung soll hier nur aus einem angenommenen Angebot gestartet werden.");
        return;
      }
      ...
      Alert.alert("Mengenermittlung", String(e?.message || "Mengenermittlung konnte nicht vorbereitet werden"));
    }
  }
  async function onCreateRechnung() {
    try {
      const saved = await normalizeAndSaveCurrentDoc();
      if (saved.status !== "Angenommen") {
        Alert.alert("Rechnung", "Eine Rechnung kann nur aus einem angenommenen Angebot erstellt werden.");
        return;
      }
      ...
      Alert.alert("Rechnung", String(e?.message || "Rechnung konnte nicht vorbereitet werden"));
    }
  }
      ...
      <KeyboardAvoidingView style={s.flex} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView contentContainerStyle={s.wrap} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View style={s.hero}>
            <View style={s.heroTop}>
              <View style={s._inline1}>
                ...
                {isAccepted ? <Text style={s.lockInfo}>
          ...
          <View style={s.card}>
            ...
            <TextInput value={doc.angebotNr} onChangeText={v => setField("angebotNr", v)} style={[s.input, isAccepted && s.inputLocked]} placeholder="ANG-2026-0001" placeholderTextColor={COLORS.sub} editable={!isAccepted} />

            <Label text="Angebotstitel" />
            <TextInput value={doc.angebotTitle} onChangeText={v => setField("angebotTitle", v)} style={[s.input, isAccepted && s.inputLocked]} placeholder="Angebot Tiefbauarbeiten" placeholderTextColor={COLORS.sub} editable={!isAccepted} />
            ...
            <View style={s.statusRow}>
              ...
              return <Pressable key={status} style={[s.statusChip, active && s.statusChipActive, chipLocked && s.disabled]} onPress={() => {
            ...
            <Row2>
              <View style={s.col}>
                <Label text="Datum" />
                <TextInput value={doc.datum} onChangeText={v => setField("datum", v)} style={[s.input, isAccepted && s.inputLocked]} placeholder="08.04.2026" placeholderTextColor={COLORS.sub} editable={!isAccepted} />
              </View>
              <View style={s.col}>
                <Label text="Gültig bis" />
                <TextInput value={doc.validUntil} onChangeText={v => setField("validUntil", v)} style={[s.input, isAccepted && s.inputLocked]} placeholder="22.04.2026" placeholderTextColor={COLORS.sub} editable={!isAccepted} />
              </View>
            </Row2>

            <Label text="Baustelle / Projekt" />
            <TextInput value={doc.baustelle} onChangeText={v => setField("baustelle", v)} style={[s.input, isAccepted && s.inputLocked]} placeholder="Projekt / Baustelle" placeholderTextColor={COLORS.sub} editable={!isAccepted} />
          </View>
          ...
          <View style={s.card}>
            ...
            <TextInput value={doc.customerName} onChangeText={v => setField("customerName", v)} style={[s.input, isAccepted && s.inputLocked]} placeholder="Max Mustermann" placeholderTextColor={COLORS.sub} editable={!isAccepted} />

            <Label text="Adresse" />
            <TextInput value={doc.customerAddress} onChangeText={v => setField("customerAddress", v)} style={[s.input, s.inputArea, isAccepted && s.inputLocked]} multiline placeholder="Straße, PLZ Ort" placeholderTextColor={COLORS.sub} editable={!isAccepted} />

            <Row2>
              <View style={s.col}>
                <Label text="E-Mail" />
                <TextInput value={doc.customerEmail} onChangeText={v => setField("customerEmail", v)} style={[s.input, isAccepted && s.inputLocked]} placeholder="kunde@email.de" placeholderTextColor={COLORS.sub} autoCapitalize="none" editable={!isAccepted} />
              </View>
              <View style={s.col}>
                <Label text="Telefon" />
                <TextInput value={doc.customerPhone} onChangeText={v => setField("customerPhone", v)} style={[s.input, isAccepted && s.inputLocked]} placeholder="+49 ..." placeholderTextColor={COLORS.sub} editable={!isAccepted} />
              </View>
            </Row2>
          </View>
          ...
          <View style={s.card}>
            <View style={s.sectionRow}>
              ...
              <Pressable style={[s.addBtn, isAccepted && s.disabled]} onPress={addRow}>
                <Text style={s.addBtnTxt}>+ Position</Text>
              </Pressable>
            </View>

            {calcRows.map((row, idx) => <View key={row.id} style={s.rowCard}>
                <Row2>
                  <View style={[s.col, {
                flex: 0.9
              }]}>
                    <Label text="Pos." />
                    <TextInput value={row.pos} onChangeText={v => setRow(row.id, {
                  pos: v
                })} style={[s.input, isAccepted && s.inputLocked]} placeholder={String(idx + 1)} placeholderTextColor={COLORS.sub} editable={!isAccepted} />
                  </View>
                ...
                flex: 1.1
                    ...
                    <Label text="Einheit" />
                    <TextInput value={row.unit} onChangeText={v => setRow(row.id, {
                  unit: v
                })} style={[s.input, isAccepted && s.inputLocked]} placeholder="m / m² / Stk" placeholderTextColor={COLORS.sub} editable={!isAccepted} />
                  </View>
                </Row2>

                <Label text="Beschreibung" />
                <TextInput value={row.text} onChangeText={v => setRow(row.id, {
              text: v
            })} style={[s.input, s.inputArea, isAccepted && s.inputLocked]} multiline placeholder="Leistungsbeschreibung" placeholderTextColor={COLORS.sub} editable={!isAccepted} />

                <Row3>
                  <View style={s.col}>
                    <Label text="Menge" />
                    <TextInput value={row.quantity} onChangeText={v => setRow(row.id, {
                  quantity: cleanNumInput(v)
                })} style={[s.input, isAccepted && s.inputLocked]} keyboardType="decimal-pad" placeholder="0" placeholderTextColor={COLORS.sub} editable={!isAccepted} />
                  </View>

                  <View style={s.col}>
                    <Label text="EP" />
                    <TextInput value={row.ep} onChangeText={v => setRow(row.id, {
                  ep: cleanNumInput(v)
                })} style={[s.input, isAccepted && s.inputLocked]} keyboardType="decimal-pad" placeholder="0,00" placeholderTextColor={COLORS.sub} editable={!isAccepted} />
                  </View>
                ...
                <Pressable style={[s.removeBtn, isAccepted && s.disabled]} onPress={() => removeRow(row.id)}>
          ...
          <View style={s.card}>
            ...
            <Row3>
              <View style={s.col}>
                <Label text="Rabatt %" />
                <TextInput value={doc.rabattPct} onChangeText={v => setField("rabattPct", cleanNumInput(v))} style={[s.input, isAccepted && s.inputLocked]} keyboardType="decimal-pad" placeholder="0" placeholderTextColor={COLORS.sub} editable={!isAccepted} />
              </View>

              <View style={s.col}>
                <Label text="Zuschlag %" />
                <TextInput value={doc.zuschlagPct} onChangeText={v => setField("zuschlagPct", cleanNumInput(v))} style={[s.input, isAccepted && s.inputLocked]} keyboardType="decimal-pad" placeholder="0" placeholderTextColor={COLORS.sub} editable={!isAccepted} />
              </View>

              <View style={s.col}>
                <Label text="MwSt %" />
                <TextInput value={doc.mwstPct} onChangeText={v => setField("mwstPct", cleanNumInput(v))} style={[s.input, isAccepted && s.inputLocked]} keyboardType="decimal-pad" placeholder="19" placeholderTextColor={COLORS.sub} editable={!isAccepted} />
              </View>
            </Row3>

            <Label text="Bemerkung / Hinweis" />
            <TextInput value={doc.note} onChangeText={v => setField("note", v)} style={[s.input, s.inputAreaLarge, isAccepted && s.inputLocked]} multiline placeholder="Zahlungsbedingungen, Ausführungshinweise, Sonstiges..." placeholderTextColor={COLORS.sub} editable={!isAccepted} />
          </View>
          ...
          <View style={s.summary}>
            ...
            <SumRow label={`Rabatt (${toNum(doc.rabattPct)} %)`} value={`- ${fmtMoney(rabattValue)}`} />
            <SumRow label={`Zuschlag (${toNum(doc.zuschlagPct)} %)`} value={`+ ${fmtMoney(zuschlagValue)}`} />
            <SumRow label="Netto gesamt" value={fmtMoney(nettoFinal)} />
            <SumRow label={`MwSt (${toNum(doc.mwstPct)} %)`} value={fmtMoney(mwstValue)} />
            <View style={s.sumDivider} />
            <SumRow label="Endsumme brutto" value={fmtMoney(brutto)} strong />
          </View>

          {doc.status === "Angenommen" ? <View style={s.flowButtons}>
              <Pressable style={[s.flowBtnOrange, busy && s.disabled]} onPress={onCreateMengen} disabled={busy}>
                <Text style={s.flowBtnTxt}>Mengenermittlung erstellen</Text>
              </Pressable>

              <Pressable style={[s.createInvoiceBtn, busy && s.disabled]} onPress={onCreateRechnung} disabled={busy}>
                <Text style={s.createInvoiceBtnTxt}>Rechnung erstellen</Text>
              </Pressable>
            </View> : null}

          <View style={s.actionGrid}>
            <Pressable style={[s.actionBtn, s.actionBtnSecondary, busy && s.disabled]} onPress={onExportPdf} disabled={busy}>
              <Text style={s.actionBtnSecondaryTxt}>PDF</Text>
            </Pressable>

            <Pressable style={[s.actionBtn, s.actionBtnSecondary, busy && s.disabled]} onPress={onExportExcel} disabled={busy}>
              <Text style={s.actionBtnSecondaryTxt}>Excel exportieren</Text>
            </Pressable>
          </View>

          <Pressable style={[s.saveBtn, (busy || isAccepted) && s.disabled]} onPress={onSave} disabled={busy || isAccepted}>
            <Text style={s.saveBtnTxt}>
              {busy ? "Speichert..." : isAccepted ? "Angebot gesperrt" : "Angebot speichern"}
            </Text>
          </Pressable>
        </ScrollView>
