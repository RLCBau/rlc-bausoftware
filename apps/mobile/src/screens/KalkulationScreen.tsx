...
const FILTERS: { key: KalkulationFilterType; label: string }[] = [
  { key: "ALL", label: "Alle" },
  { key: "KRITISCH", label: "Kritisch" },
  { key: "PRUEFHINWEISE", label: "Hinweise" },
  { key: "PRUEFPFLICHTIG", label: "Prüfpflichtig" },
  { key: "OUTLIER", label: "Outlier" },
  { key: "OHNE_PREIS", label: "Ohne Preis" },
  { key: "DOPPELTE", label: "Doppelte" },
];
...
function isCritical(row: KalkRow): boolean {
  ...
  return (
    ...
    warning.includes("kritisch") ||
    warning.includes("outlier") ||
    warning.includes("prüfen") ||
...
function hasCalculatedMoney(row: KalkRow): boolean {
  return (
    n(row.rlcKiUnitPrice) > 0 ||
    n(row.kiUnitPrice) > 0 ||
    n(row.calculatedUnitPrice) > 0 ||
    n(row.finalUnitPrice) > 0 ||
    n(row.suggestedUnitPrice) > 0 ||
    n(row.unitPriceNet) > 0 ||
    n(row.ep) > 0 ||
    n(row.rlcKiTotal) > 0 ||
    n(row.kiTotal) > 0 ||
    n(row.calculatedTotal) > 0 ||
    n(row.totalNet) > 0 ||
    n(row.gp) > 0
  );
}
...
function normalizeMoneyScale(rows: KalkRow[], explicitTotal?: number): { rows: KalkRow[]; totalNet?: number; scale: number } {
  ...
  if (scale === 1) return { rows, totalNet: explicitTotal, scale };
  return {
...
async function loadSavedKalkulation(projectId: string, projectCode?: string): Promise<LoadResult> {
  ...
  if (!base) {
    return { rows: [], sourceLabel: "Keine API", hasSavedCalculation: false, message: "API-URL nicht verfügbar." };
  }

  const ids = [projectId, projectCode].filter(Boolean).map((x) => encodeURIComponent(String(x)));
  const urls = ids.flatMap((id) => [
    `${base}/api/kalkulation/${id}/ki`,
...
export default function KalkulationScreen() {
  ...
  const openKi = () => navigation.navigate("KiCalculation", { projectId, projectCode, title });
  const openOutliers = () => navigation.navigate("KalkulationOutlier", { projectId, projectCode, title, filter: "OUTLIER" });
  const openCopilot = () => navigation.navigate("RlcCopilot", { projectId, projectCode, title, entryMode: "kalkulation" });
  ...
  const renderRow = ({ item }: { item: KalkRow }) => {
    const warning = rowWarning(item);
    const source = compact(String(item.source || ""));
    return (
      <Pressable style={styles.card} onPress={() => explainRow(item)}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {!!error && (
        <Pressable style={hasSavedCalculation ? styles.errorBox : styles.noticeBox} onPress={() => Alert.alert("Kalkulation", error)}>
          <Text style={hasSavedCalculation ? styles.errorText : styles.noticeText}>{error}</Text>
        </Pressable>
      )}

      {!hasSavedCalculation && !rows.length ? (
        ...
        <FlatList
          data={visibleRows}
          keyExtractor={(item, index) => String(item.id || item.positionId || `${rowPos(item)}-${index}`)}
          renderItem={renderRow}
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(true); }} />}
          contentContainerStyle={visibleRows.length ? styles.rows : styles.emptyRows}
          ListEmptyComponent={<Text style={styles.emptyText}>Keine Positionen gefunden.</Text>}
        />
      )}
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  statValue: { color: COLORS.text, fontWeight: "900", fontSize: 18, lineHeight: 23 },
  statLabel: { color: COLORS.sub, fontSize: 12, marginTop: 4, fontWeight: "800" },
  ...
  filterList: { paddingHorizontal: 16, gap: 8, paddingBottom: 8 },
