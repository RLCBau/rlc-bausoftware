...
import { exportLieferscheinPdfToProject, emailPdf } from "../lib/exporters/projectExport";
...
async function saveProjectMeta(projectFsKey: string, patch: Partial<ProjectMeta>) {
...
function isImage(item?: DateiMeta) {
  ...
  return type.startsWith("image/") || /\.(jpg|jpeg|png|webp|heic|heif)$/i.test(uri);
}
...
function forceHttpsIfSameHost(base: string, uri: string) {
  ...
  try {
    ...
    const x = new URL(u);
    if (b.hostname === x.hostname && b.protocol === "https:" && x.protocol === "http:") {
      x.protocol = "https:";
      return x.toString();
    }
  } catch {}
...
function buildKiTextFromRowLs(r: LieferscheinRow) {
  ...
  if (r.baustellenNummer) bits.push(`BaustellenNummer: ${String(r.baustellenNummer)}`);
  if (r.bauleiterEmail) bits.push(`BauleiterEmail: ${String(r.bauleiterEmail)}`);
  if (r.lieferscheinNummer) bits.push(`LieferscheinNr: ${String(r.lieferscheinNummer)}`);
  ...
  if (r.quantity != null && String(r.quantity).trim() !== "") bits.push(`Menge: ${String(r.quantity)}`);
...
function normalizeKiResultLs(raw: any) {
  ...
  const fallbackDirectObject = !firstSuggestion && root && typeof root === "object" && (root.material != null || root.quantity != null || root.qty != null || root.unit != null || root.fahrer != null || root.driver != null || root.baustelle != null || root.site != null || root.lieferant != null || root.supplier != null || root.comment != null || root.bemerkungen != null || root.text != null) ? root : null;
...
async function uploadVisionFilesIfPossible(projectFsKey: string, files: DateiMeta[]) {
  ...
  const uploadFn = (api as any)?.kiVisionFiles || (api as any)?.kiVisionFilesUpload || (api as any)?.kiUploadVisionFiles || (api as any)?.uploadVisionFiles || (api as any)?.visionFiles || null;
  ...
  let res: any;
  try {
    res = uploadFn.length >= 2 ? await uploadFn(projectFsKey, clean) : await uploadFn(clean);
  } catch {
    res = uploadFn.length >= 2 ? await uploadFn(projectFsKey, {
      files: clean
    }) : await uploadFn({
      files: clean
    });
  }
  const ids = res?.visionFileIds || res?.data?.visionFileIds || res?.result?.visionFileIds || res?.fileIds || res?.ids || res?.data?.fileIds || res?.data?.ids || res?.result?.fileIds || res?.result?.ids || [];
  return {
...
function requirePflichtfelderOrAlert(projectFsKey: string, r: LieferscheinRow) {
  ...
  if (!bl) missing.push("Bauleiter (E-Mail)");else if (!isValidEmail(bl)) missing.push("Bauleiter (E-Mail ungültig)");
  if (missing.length) {
...
export default function LieferscheinScreen({
  route,
  navigation
}: Props) {
  const {
  ...
  const projectFsKey = useMemo(() => normalizeProjectKey(projectCodeFs, projectIdOrFallback), [projectCodeFs, projectIdOrFallback]);
  ...
  const [row, setRow] = useState<LieferscheinRow>(() => {
    const fixed = inboxSnapshot ? normalizeInboxSnapshotLs(inboxSnapshot, String(editId || uid("ls"))) : null;
    return fixed || {
  ...
  const hydratePflichtfelder = useCallback(async () => {
    ...
    setRow(r => ({
      ...r,
      baustellenNummer: String(r.baustellenNummer || meta.baustellenNummer || "").trim(),
      bauleiterEmail: String(r.bauleiterEmail || meta.bauleiterEmail || "").trim()
    }));
  }, [projectFsKey]);
  async function persistToInbox(nextRow: LieferscheinRow) {
    const projectKeys = Array.from(new Set([projectFsKey, projectCodeFs, projectIdForServer, String((route.params as any)?.projectCode || ""), String((route.params as any)?.projectId || "")].map(x => String(x || "").trim()).filter(Boolean)));
    const storageKeys = Array.from(new Set(projectKeys.flatMap(k => lieferscheinInboxKeys(k))));
    ...
    if (idx >= 0) lst[idx] = nextRow;else lst.unshift(nextRow);
  ...
  const loadInboxList = useCallback(async () => {
    ...
    try {
      ...
      const normalized = await Promise.all(next.map(async r => {
        const hydratedRaw = await hydrateRowForPreview(r, projectFsKey);
        return {
          ...r,
          ...hydratedRaw,
          attachments: normalizeFiles(hydratedRaw?.attachments || hydratedRaw?.files || hydratedRaw?.photos || r.attachments || [])
        };
      }));
      ...
      Alert.alert("Lieferschein", e?.message || "Inbox konnte nicht geladen werden.");
    }
  ...
  const updatePflichtfeld = useCallback(async (patch: Partial<ProjectMeta>) => {
    setRow(r => ({
      ...r,
      ...patch,
      updatedAt: Date.now()
    ...
    try {
      ...
      if (mode === "SERVER_SYNC" && looksLikeProjectCode(projectFsKey)) {
        ...
        const payload = {
          ...
          quantity: next.quantity === "" || next.quantity == null ? undefined : Number(next.quantity),
        ...
        Alert.alert("Einreichen", "Lieferschein wurde in Eingang / Prüfung gespeichert. Sync läuft über Sync Queue.");
        navigation.goBack();
        return;
      }
  ...
  const onOpenPdf = useCallback(async () => {
    ...
    if (out.pdfUri) {
      navigation.navigate("PdfViewer", {
        uri: out.pdfUri,
        title: `Lieferschein ${safeDateOrNow(row.date)}`,
        projectId,
        projectCode,
        documentType: "LIEFERSCHEIN"
      });
    }
  }, [buildPdf, navigation, projectId, projectCode, row.date]);
  const onEmailPdf = useCallback(async () => {
    if (!requirePflichtfelderOrAlert(projectFsKey, row)) return;
    const out = await buildPdf();
    const attachments = Platform.OS === "web" ? [] : [out.pdfUri].filter(u => String(u || "").startsWith("file://"));
    await emailPdf({
      subject: out.fileName,
      body: `Lieferschein ${projectFsKey} (${out.date})\nBaustellenNummer: ${String(row.baustellenNummer || "")}`,
  ...
  const openFromHistory = useCallback((x: LieferscheinRow) => {
    navigation.setParams?.({
      editId: x.id,
      fromInbox: true
    } as any);
    normalizeAttachmentsForPreview(normalizeFiles(x.attachments || [])).then(atts => {
      setRow({
        ...x,
        attachments: atts
      });
  ...
  const onKiSuggest = useCallback(async () => {
    try {
      ...
      if (mode === "NUR_APP") {
        ...
        const hasText = !!String(row.material || "").trim() || !!String(row.lieferant || "").trim() || !!String(row.baustelle || "").trim() || !!String(row.bemerkungen || "").trim() || !!String(row.lieferscheinNummer || "").trim();
        setKiSuggestion(normalizeKiResultLs({
          suggestion: null,
      ...
      const fn = (api as any)?.kiLieferscheinSuggest || (api as any)?.kiSuggestLieferschein || null;
      if (typeof fn !== "function") {
        setKiSuggestion(normalizeKiResultLs({
          error: {
            message: "KI Endpoint nicht verbunden. (api.kiLieferscheinSuggest fehlt)"
          }
        }));
        return;
      }
      ...
      let rawRes: any;
      rawRes = typeof fn === "function" && fn.length >= 2 ? await fn(projectFsKey, payload) : await fn(payload);
      ...
      setKiSuggestion(normalizeKiResultLs({
        error: {
          message: String(e?.message || "KI Fehler")
        }
      }));
    } finally {
      setKiBusy(false);
  ...
  useEffect(() => {
    return registerRlcKiModuleHandler("Lieferschein", async (payload: any) => {
      ...
      const materialText = Array.isArray(parsed.material) ? parsed.material.map((m: any) => `${m.name} ${m.quantity} ${m.unit}`.trim()).join("\n") : "";
...
${materialText}` : ""}${warnings}`,
      ...
      return {
        ok: true,
        handled: true,
        message: "LIEFERSCHEIN_FIELDS_FILLED"
      };
    });
  }, []);
  const applyKiSuggestion = useCallback(() => {
    try {
      ...
      const isLabelLike = (v: any) => {
        ...
        return s === "datum" || s === "datum:" || s === "fahrer" || s === "fahrer:" || s === "kostenstelle" || s === "kostenstelle:";
      };
      ...
      const materialVal = getFirst(fp.material, fp.mat, fp.stoff, fp.materialName, fp.material_name, fp.artikel, fp.bezeichnung, fp.produkt, fp.product);
      const quantityVal = getFirst(fp.menge, fp.quantity, fp.qty, fp.amount, fp.anzahl, fp.volume, fp.masse);
      const unitVal = getFirst(fp.einheit, fp.unit, fp.me, fp.eh, fp.mengeneinheit);
      const supplierVal = getFirst(fp.lieferant, fp.supplier, fp.firma, fp.vendor, fp.haendler);
      ...
      const lvVal = getFirst(fp.lvPosition, fp.lvItemPos, fp.lvPos, fp.lv_pos, fp.lv_item_pos);
      const lsNumVal = getFirst(fp.lsNr, fp.lieferscheinNr, fp.lieferscheinNummer, fp.lieferschein_nummer, fp.bonNr, fp.bon_nr, fp.bon);
      const bemerkungVal = getFirst(fp.bemerkungen, fp.textBeschreibung, fp.beschreibung, fp.text, fp.comment, fp.note);
      ...
      setRow(r => {
        ...
        if (has(siteVal) && !isDateLike(siteVal) && !isLabelLike(siteVal) && !isProjectCode(siteVal)) {
          next.baustelle = sstr(siteVal);
        } else if (has(inferredFromBemerkung.site) && !isDateLike(inferredFromBemerkung.site) && !isLabelLike(inferredFromBemerkung.site) && !isProjectCode(inferredFromBemerkung.site)) {
        ...
        const finalSite = has(siteVal) && !isProjectCode(siteVal) ? sstr(siteVal) : sstr(inferredFromBemerkung.site);
        const finalDriver = has(driverVal) && !isClearlyWrongDriver(driverVal) ? sstr(driverVal) : sstr(inferredFromBemerkung.driver);
        if (has(materialVal) && qStr && has(unitVal) && finalSite && finalDriver) {
          next.bemerkungen = `${sstr(materialVal)} (${qStr} ${sstr(unitVal)}) zur Baustelle ${finalSite} geliefert (Fahrer: ${finalDriver}).`;
  ...
  const renderAttachment = useCallback(({
    ...
    return <View style={s.attCard}>
          {img ? <Image source={{
        uri
      }} style={s.attImg} /> : <View style={s.attFile}>
  ...
  return <SafeAreaView style={s.safe}>
      <ScrollView contentContainerStyle={s.wrap}>
        ...
        <View style={s.card}>
          ...
          <TextInput value={String(row.baustellenNummer || "")} onChangeText={v => updatePflichtfeld({
          baustellenNummer: v
        })} style={s.input} placeholder="z.B. BA-12345" placeholderTextColor={COLORS.sub} />

          <Text style={s.label}>Bauleiter (E-Mail)</Text>
          <TextInput value={String(row.bauleiterEmail || "")} onChangeText={v => updatePflichtfeld({
          bauleiterEmail: v
        })} style={s.input} autoCapitalize="none" keyboardType="email-address" placeholder="bauleiter@firma.de" placeholderTextColor={COLORS.sub} />
        ...
        <View style={s.card}>
          ...
          <View style={s.grid2}>
            <View style={s._inline3}>
              ...
              <TextInput value={String(row.lieferscheinNummer || "")} onChangeText={v => updateRow({
              lieferscheinNummer: v
            })} style={s.input} placeholder="z.B. LS-123" placeholderTextColor={COLORS.sub} />
            </View>
          ...
          <View style={s.grid2}>
            <View style={s._inline5}>
              ...
              <TextInput value={String(row.lieferant || "")} onChangeText={v => updateRow({
              lieferant: v
            })} style={s.input} placeholder="Firma" placeholderTextColor={COLORS.sub} />
            </View>
            <View style={s._inline6}>
              <Text style={s.label}>Fahrer / Mitarbeiter</Text>
              <TextInput value={String(row.fahrer || "")} onChangeText={v => updateRow({
              fahrer: v
          ...
          <TextInput value={String(row.baustelle || "")} onChangeText={v => updateRow({
          baustelle: v
        })} style={s.input} placeholder="Ort / Abschnitt" placeholderTextColor={COLORS.sub} />

          <Text style={s.label}>LV Position</Text>
          <TextInput value={String(row.lvItemPos || "")} onChangeText={v => updateRow({
          lvItemPos: v
        })} style={s.input} placeholder="z.B. 01.02.0001" placeholderTextColor={COLORS.sub} />

          <View style={s.grid2}>
            <View style={s._inline7}>
              <Text style={s.label}>Material</Text>
              <TextInput value={String(row.material || "")} onChangeText={v => updateRow({
              material: v
            ...
            <View style={s._inline8}>
              ...
              <TextInput value={String(row.unit || "")} onChangeText={v => updateRow({
              unit: v
          ...
          <TextInput value={String(row.quantity ?? "")} onChangeText={v => updateRow({
          quantity: v
        ...
        <View style={s.section}>
          ...
          {(row.attachments || []).length ? <FlatList data={row.attachments || []} keyExtractor={(x, i) => `${String(x?.id ?? x?.uri ?? "att")}_${row.id}_${i}`} renderItem={renderAttachment} scrollEnabled={false} contentContainerStyle={s._inline9} /> : <Text style={s.muted}>Keine Anhänge.</Text>}
        </View>
        ...
        <View style={s.section}>
          ...
          {list.length ? <FlatList data={list} keyExtractor={(x, i) => `${String(x.id)}_${i}`} renderItem={renderHistoryRow} scrollEnabled={false} contentContainerStyle={s._inline10} /> : <Text style={s.muted}>Keine Einträge.</Text>}
        </View>
      ...
      <Modal visible={kiOpen} transparent animationType="slide" onRequestClose={() => setKiOpen(false)}>
        <View style={s.modalWrap}>
          <View style={s.modalCard}>
            ...
            <Pressable onPress={() => Keyboard.dismiss()} style={s._inline12}>
              <Text style={s._inline13}>
                Tastatur schließen
              </Text>
            </Pressable>
{kiBusy ? <Text style={s.modalMuted}>KI läuft...</Text> : <>
                ...
                <ScrollView style={s._inline14} contentContainerStyle={s._inline15}>
                  <Text selectable style={s.modalBody}>
                    {kiSuggestion?.suggestion ? safeStringify(kiSuggestion.suggestion) : kiSuggestion?.raw ? safeStringify(kiSuggestion.raw) : "Kein Vorschlag verfügbar."}
                  </Text>
                </ScrollView>

                <View style={s._inline16}>
            ...
            {kiBusy ? <Pressable style={s.modalBtn} onPress={() => {
            Keyboard.dismiss();
            setKiOpen(false);
          }}>
                <Text style={s.modalBtnTxt}>Schließen</Text>
...
const s = createRlcStyles("LieferscheinScreen", {
  safe: {
    flex: 1,
    backgroundColor: COLORS.bg
  },
