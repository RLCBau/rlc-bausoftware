...
export default function EingangPruefungScreen({
  ...
  type InboxTab = "REGIE" | "LS" | "FOTOS" | "TAGESBERICHT" | "BAUTAGEBUCH" | "ANGEBOT" | "RECHNUNG" | "ABSCHLAGSRECHNUNG" | "MENGEN" | "KALKULATION";
  ...
  const pullServerInbox = useCallback(async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled([loadGeneric("ANGEBOT"), loadGeneric("RECHNUNG"), loadGeneric("ABSCHLAGSRECHNUNG"), loadGeneric("MENGENERMITTLUNG"), loadGeneric("KALKULATION")]);
  ...
  async function ensureGenericLocalPdf(t: InboxTab, item: any): Promise<string> {
    ...
    if (t === "ANGEBOT" || t === "RECHNUNG" || t === "ABSCHLAGSRECHNUNG" || t === "MENGEN" || t === "KALKULATION") {
  ...
  const shown = tab === "REGIE" ? regie : tab === "LS" ? ls : tab === "FOTOS" ? fotos : tab === "TAGESBERICHT" ? tagesberichte : tab === "BAUTAGEBUCH" ? bautagebuch : tab === "ANGEBOT" ? angebote : tab === "RECHNUNG" ? rechnungen : tab === "ABSCHLAGSRECHNUNG" ? abschlaege : tab === "MENGEN" ? mengen : kalkulationen;
