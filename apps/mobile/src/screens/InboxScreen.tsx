...
type Kind = "REGIE" | "LS" | "PHOTOS" | "TAGESBERICHT" | "BAUTAGEBUCH" | "ANGEBOT" | "RECHNUNG" | "ABSCHLAGSRECHNUNG" | "MENGEN" | "KALKULATION" | "ARBEITSZEIT";
type InboxItem = {
  kind: Kind;
  projectId: string;
  projectTitle: string;
...
function inferRowTitle(kind: Kind, r: any): string {
  ...
  const base = kind === "REGIE" ? "Regie" : kind === "LS" ? "Lieferschein" : kind === "TAGESBERICHT" ? "Tagesbericht" : kind === "BAUTAGEBUCH" ? "Bautagebuch" : kind === "ANGEBOT" ? "Angebot" : kind === "RECHNUNG" ? "Rechnung" : kind === "ABSCHLAGSRECHNUNG" ? "Abschlagsrechnung" : kind === "MENGEN" ? "Mengenermittlung" : kind === "KALKULATION" ? "Kalkulation" : kind === "ARBEITSZEIT" ? "Arbeitszeit" : "Fotos";
  const p1 = nr ? `#${nr}` : "";
...
function arbeitszeitInboxKeys(projectKey: string) {
  return [
    `rlc_mobile_inbox_arbeitszeiten:${projectKey}`,
    `rlc_mobile_inbox_arbeitszeit:${projectKey}`,
  ];
}
export default function InboxScreen({
  ...
  const loadInbox = useCallback(async () => {
    ...
    try {
      ...
      for (const p of proj) {
        ...
        const [regieInbox, lsInbox, photosInbox, tagesberichtInbox, bautagebuchInbox, angebotInbox, rechnungInbox, abschlagInbox, mengenInbox, kalkulationInbox, arbeitszeitInbox] = await Promise.all([loadArrayFromFirstKey(regieInboxKeys(projectKey)), loadArrayFromFirstKey(lsInboxKeys(projectKey)), loadArrayFromFirstKey(photosInboxKeys(projectKey)), loadArrayFromFirstKey(tagesberichtInboxKeys(projectKey)), loadArrayFromFirstKey(bautagebuchInboxKeys(projectKey)), loadArrayFromFirstKey(angebotInboxKeys(projectKey)), loadArrayFromFirstKey(rechnungInboxKeys(projectKey)), loadArrayFromFirstKey(abschlagInboxKeys(projectKey)), loadArrayFromFirstKey(mengenInboxKeys(projectKey)), loadArrayFromFirstKey(kalkulationInboxKeys(projectKey)), loadArrayFromFirstKey(arbeitszeitInboxKeys(projectKey))]);
        ...
        pushGeneric("ARBEITSZEIT", arbeitszeitInbox);
  ...
  const counts = useMemo(() => {
    ...
    const arbeitszeit = items.filter(x => x.kind === "ARBEITSZEIT").length;
    return {
      regie,
      ls,
      photos,
