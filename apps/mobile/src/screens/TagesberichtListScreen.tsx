...
import { View, Text, Pressable, SafeAreaView, FlatList, TextInput, Alert, Platform } from "react-native";
...
import { exportTagesberichtPdfToProject } from "../lib/exporters/projectExport";
...
function summarizeLines(lines?: TagesberichtLine[]) {
  ...
  return arr.map(x => String(x?.taetigkeit || x?.notiz || "").trim()).filter(Boolean).slice(0, 3).join(" | ");
}
...
export default function TagesberichtListScreen({
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => {
      ...
      const hay = [r.date, r.weather, r.temperature, r.workers, r.machines, r.workDone, r.issues, r.notes, lineText, ...(Array.isArray(r.lines) ? r.lines.flatMap(x => [x?.mitarbeiter, x?.maschine, x?.ort, x?.taetigkeit, x?.notiz]) : [])].map(x => String(x || "").toLowerCase()).join(" ");
  ...
  const summary = useMemo(() => {
    ...
    const withMachines = filtered.filter(x => totalMachines(x.lines) > 0 || String(x.machines || "").trim()).length;
    return {
      total,
      withIssues,
      withMachines
  ...
  const onDelete = useCallback((item: TagesberichtRow) => {
    Alert.alert("Tagesbericht löschen", `Soll der Tagesbericht vom ${String(item.date || "—")} wirklich gelöscht werden?`, [{
      text: "Abbrechen",
      style: "cancel"
    }, {
      text: "Löschen",
      ...
      onPress: async () => {
        try {
          ...
          Alert.alert("Fehler", e?.message || "Tagesbericht konnte nicht gelöscht werden.");
        }
      }
    }]);
  }, [projectKey]);
  const onSinglePdf = useCallback(async (item: TagesberichtRow) => {
    try {
      setBusy(true);
      ...
      navigation.navigate("PdfViewer", {
        uri: out.pdfUri,
        title: `Tagesbericht ${String(item.date || "").slice(0, 10)}`,
        projectId,
  ...
  }, [projectKey, title, navigation, projectId]);
  ...
  const renderItem = ({
    ...
    const desc = lineSummary || String(item.workDone || "").trim() || String(item.notes || "").trim() || "Kein Beschreibungstext";
    ...
    return <View style={s.rowCard}>
        ...
        <View style={s.rowActionBar}>
          ...
          <Pressable style={s.rowGhostBtn} onPress={() => onSinglePdf(item)} disabled={busy}>
            <Text style={s.rowGhostBtnTxt}>PDF</Text>
          </Pressable>

          <Pressable style={[s.rowGhostBtn, s.rowDangerBtn]} onPress={() => onDelete(item)}>
            <Text style={[s.rowGhostBtnTxt, s.rowDangerBtnTxt]}>Löschen</Text>
          </Pressable>
        </View>
      </View>;
  };
  const ListHeader = () => <View style={s.headerWrap}>
      ...
      <View style={s.filterCompact}>
        <View style={s.monthBox}>
          ...
          <TextInput value={monthFilter} onChangeText={setMonthFilter} style={s.input} placeholder="YYYY-MM" placeholderTextColor={COLORS.sub} />
        </View>

        <View style={s.searchBox}>
          <Text style={s.inputLabel}>Suche</Text>
          <TextInput value={search} onChangeText={setSearch} style={s.input} placeholder="Wetter, Tätigkeit, Mitarbeiter..." placeholderTextColor={COLORS.sub} />
        </View>
      </View>
  ...
  return <SafeAreaView style={s.safe}>
      <FlatList data={filtered} keyExtractor={item => String(item.id)} renderItem={renderItem} ListHeaderComponent={ListHeader} contentContainerStyle={s.listContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} ListEmptyComponent={<View style={s.emptyCard}>
...
const s = createRlcStyles("TagesberichtListScreen", {
  safe: {
    flex: 1,
    backgroundColor: COLORS.bg
  },
