...
import { View, Text, TextInput, Pressable, ScrollView, Alert, Platform, SafeAreaView, KeyboardAvoidingView } from "react-native";
...
function RoleInput({
  ...
  return <TextInput {...rest} editable={editable} selectTextOnFocus={editable} placeholderTextColor="rgba(11,23,32,0.45)" autoCorrect={false} blurOnSubmit={false} returnKeyType={multiline ? "default" : "done"} multiline={multiline} style={[s.input, !editable && s.inputDisabled, multiline && s.inputMultiline, style]} />;
}
export default function TeamRolesScreen({
  ...
  const effectiveProjectCode = useMemo(() => {
    ...
    const fromId = looksLikeProjectCode(String(projectId || "").trim()) ? String(projectId || "").trim() : "";
    return fromRoute || fromState || fromId;
  }, [routeProjectCode, projectCode, projectId]);
  const effectiveProjectKey = useMemo(() => {
    return looksLikeProjectCode(effectiveProjectCode) ? effectiveProjectCode : String(projectId || "").trim();
  ...
  useEffect(() => {
    (async () => {
      const s0 = (await getSession(effectiveProjectKey)) || (looksLikeProjectCode(String(routeProjectCode || "").trim()) ? await getSession(String(routeProjectCode).trim()) : null) || (await getSession(String(projectId).trim()));
      setSessionRole(s0?.role);
      const existing = (await getProjectRoles(effectiveProjectKey)) || (await getProjectRoles(projectId));
      if (existing && lastLoadedKeyRef.current !== String(effectiveProjectKey) && !isEditingRef.current) {
      ...
      if (!existing && lastLoadedKeyRef.current !== String(effectiveProjectKey)) {
        lastLoadedKeyRef.current = String(effectiveProjectKey);
        if (!isEditingRef.current) setModel((m: any) => ({
          ...(initialModel as any),
          ...m
        }));
      }
  ...
  const headerCode = looksLikeProjectCode(effectiveProjectCode) ? effectiveProjectCode : "-";
  const modeLabel = editable ? "Bearbeiten" : "Nur Ansicht";
  return <SafeAreaView style={s.safe}>
      <KeyboardAvoidingView style={s._inline1} behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <View style={s.bg}>
  ...
  ID: <Text style={s.badgeTxtStrong}>{String(projectId)}</Text>
              ...
              {bauleiterMissing ? <View style={s.warnBox}>
            ...
            <View style={s.body}>
              <Section title="Interne Rollen">
                <Row label="Bauleiter (Pflicht)">
                  <RoleInput editable={editable} value={model.bauleiter?.name || ""} onChangeText={(t: string) => patchModel(m => ({
                  ...m,
                  bauleiter: {
                    ...(m.bauleiter || {}),
                    name: t
                ...
                <Row label="Kalkulator">
                  <RoleInput editable={editable} value={(model as any).kalkulator?.name || ""} onChangeText={(t: string) => patchModel((m: any) => ({
                  ...m,
                  kalkulator: {
                    ...(m.kalkulator || {}),
                    name: t
                ...
                <Row label="Büro">
                  <RoleInput editable={editable} value={(model as any).buero?.name || ""} onChangeText={(t: string) => patchModel((m: any) => ({
                  ...m,
                  buero: {
                    ...(m.buero || {}),
                    name: t
                ...
                <Row label="Polier / Vorarbeiter">
                  <RoleInput editable={editable} value={model.polier?.name || ""} onChangeText={(t: string) => patchModel(m => ({
                  ...m,
                  polier: {
                    ...(m.polier || {}),
                    name: t
                ...
                <Row label="Fahrer">
                  <RoleInput editable={editable} value={(model as any).fahrer?.name || ""} onChangeText={(t: string) => patchModel((m: any) => ({
                  ...m,
                  fahrer: {
                    ...(m.fahrer || {}),
                    name: t
                ...
                <Row label="Mitarbeiter">
                  <RoleInput editable={editable} value={(model as any).mitarbeiter?.name || ""} onChangeText={(t: string) => patchModel((m: any) => ({
                  ...m,
                  mitarbeiter: {
                    ...(m.mitarbeiter || {}),
                    name: t
              ...
              <Section title="Auftraggeber / Ansprechpartner">
                <Row label="Firma / Kunde">
                  <RoleInput editable={editable} value={model.auftraggeber?.company || ""} onChangeText={(t: string) => patchModel(m => ({
                  ...m,
                  auftraggeber: {
                    ...(m.auftraggeber || {}),
                    company: t
                ...
                <Row label="Ansprechpartner">
                  <RoleInput editable={editable} value={model.auftraggeber?.contactName || ""} onChangeText={(t: string) => patchModel(m => ({
                  ...m,
                  auftraggeber: {
                    ...(m.auftraggeber || {}),
                    contactName: t
                ...
                <Row label="Telefon">
                  <RoleInput editable={editable} value={model.auftraggeber?.phone || ""} onChangeText={(t: string) => patchModel(m => ({
                  ...m,
                  auftraggeber: {
                    ...(m.auftraggeber || {}),
                    phone: t
                ...
                <Row label="E-Mail">
                  <RoleInput editable={editable} autoCapitalize="none" keyboardType="email-address" value={model.auftraggeber?.email || ""} onChangeText={(t: string) => patchModel(m => ({
                  ...m,
                  auftraggeber: {
                    ...(m.auftraggeber || {}),
                    email: t
                ...
                <Row label="Notiz">
                  <RoleInput editable={editable} value={model.auftraggeber?.note || ""} onChangeText={(t: string) => patchModel(m => ({
                  ...m,
                  auftraggeber: {
                    ...(m.auftraggeber || {}),
                    note: t
                  }
                }))} placeholder="Kurz notieren..." multiline style={s._inline3} />
                </Row>
              </Section>

              <Section title="Weitere interne Ansprechpartner (optional)">
                <Row label="Einkauf">
                  <RoleInput editable={editable} value={model.ansprechpartnerIntern?.einkauf?.name || ""} onChangeText={(t: string) => patchModel(m => ({
                  ...m,
                  ansprechpartnerIntern: {
                ...
                <Row label="Lager">
                  <RoleInput editable={editable} value={model.ansprechpartnerIntern?.lager?.name || ""} onChangeText={(t: string) => patchModel(m => ({
                  ...m,
                  ansprechpartnerIntern: {
                    ...(m.ansprechpartnerIntern || {}),
                    lager: {
                ...
                <Row label="Logistik">
                  <RoleInput editable={editable} value={model.ansprechpartnerIntern?.logistik?.name || ""} onChangeText={(t: string) => patchModel(m => ({
                  ...m,
                  ansprechpartnerIntern: {
                    ...(m.ansprechpartnerIntern || {}),
                    logistik: {
              ...
              <Section title="E-Mails (Versand / Ansprechpartner)">
                <Row label="Bauleiter – E-Mail">
                  <RoleInput editable={editable} autoCapitalize="none" keyboardType="email-address" value={(model as any).emails?.bauleiter || ""} onChangeText={(t: string) => patchModel((m: any) => ({
                  ...m,
                  emails: {
                    ...(m.emails || {}),
                    bauleiter: t
                ...
                <Row label="Büro – E-Mail">
                  <RoleInput editable={editable} autoCapitalize="none" keyboardType="email-address" value={(model as any).emails?.buero || ""} onChangeText={(t: string) => patchModel((m: any) => ({
                  ...m,
                  emails: {
                    ...(m.emails || {}),
                    buero: t
                ...
                <Row label="Extern / Prüfer – E-Mail">
                  <RoleInput editable={editable} autoCapitalize="none" keyboardType="email-address" value={(model as any).emails?.extern || ""} onChangeText={(t: string) => patchModel((m: any) => ({
                  ...m,
                  emails: {
                    ...(m.emails || {}),
                    extern: t
              ...
              {editable ? <Pressable style={s.primaryBtn} onPress={onSave}>
                  <Text style={s.primaryBtnTxt}>Speichern (offline)</Text>
                </Pressable> : <View style={s.readOnlyBox}>
...
function normalize(x: ProjectRoles): ProjectRoles {
  ...
  const out: any = {
    bauleiter: trim((x as any).bauleiter?.name) ? {
      ...((x as any).bauleiter || {}),
      name: trim((x as any).bauleiter?.name)
    } : undefined,
    kalkulator: trim((x as any).kalkulator?.name) ? {
      ...((x as any).kalkulator || {}),
      name: trim((x as any).kalkulator?.name)
    } : undefined,
    buero: trim((x as any).buero?.name) ? {
      ...((x as any).buero || {}),
      name: trim((x as any).buero?.name)
    } : undefined,
    polier: trim((x as any).polier?.name) ? {
      ...((x as any).polier || {}),
      name: trim((x as any).polier?.name)
    } : undefined,
    fahrer: trim((x as any).fahrer?.name) ? {
      ...((x as any).fahrer || {}),
      name: trim((x as any).fahrer?.name)
    } : undefined,
    mitarbeiter: trim((x as any).mitarbeiter?.name) ? {
      ...((x as any).mitarbeiter || {}),
      name: trim((x as any).mitarbeiter?.name)
    } : undefined,
    auftraggeber: trim((x as any).auftraggeber?.company) || trim((x as any).auftraggeber?.contactName) || trim((x as any).auftraggeber?.phone) || trim((x as any).auftraggeber?.email) || trim((x as any).auftraggeber?.note) ? {
    ...
    emails: trim((x as any).emails?.bauleiter) || trim((x as any).emails?.buero) || trim((x as any).emails?.extern) ? {
...
const s = createRlcStyles("TeamRolesScreen", {
  safe: {
    flex: 1,
