...
function sanitizeFileBase(v: string) {
  return String(v || "schlussrechnung").trim().replace(/[\\/:*?"<>|]/g, "_").replace(/\s+/g, "_");
}
...
function buildAbschlagLines(abschlaege: AbschlagItem[]) {
  ...
  return safe.map(a => `${a.nummer}. Abschlag Nr. ${a.nummer} (${a.datum}): ${money(a.betrag)} €${a.note ? ` • ${a.note}` : ""}`);
}
...
export default function SchlussrechnungsScreen({
    ...
    try {
      ...
      if (mode === "SERVER_SYNC") {
        try {
          ...
          found = safeServerList.find((x: any) => String(x?.id) === String(rechnungId)) || null;
          if (safeServerList.length) {
            await AsyncStorage.setItem(RECHNUNG_KEY + projectCode, JSON.stringify(safeServerList));
          }
        } catch (e) {
      ...
      if (!found) {
        ...
        found = safe.find((x: any) => String(x?.id) === String(rechnungId)) || null;
      }
      if (!found) {
  ...
  const brutto = useMemo(() => Number(rechnung?.brutto || 0), [rechnung?.brutto]);
  const abschlaege: AbschlagItem[] = useMemo(() => Array.isArray(rechnung?.abschlaege) ? rechnung.abschlaege.map((a: any, i: number) => normalizeAbschlag(a, i)) : [], [rechnung?.abschlaege]);
  const bezahlt = useMemo(() => abschlaege.reduce((sum: number, a: AbschlagItem) => sum + Number(a?.betrag || 0), 0), [abschlaege]);
  const rest = useMemo(() => Math.max(0, brutto - bezahlt), [brutto, bezahlt]);
  async function onExportPdf() {
    try {
      if (!rechnung) return;
      setBusy(true);
      const fileNameBase = sanitizeFileBase(`${rechnung?.rechnungNr || "schlussrechnung"}_schlussrechnung`);
      const out: any = await buildSchlussrechnungPdf({
        projectCode,
        fileName: `${fileNameBase}.pdf`,
        title: "SCHLUSSRECHNUNG",
        subTitle: abschlaege.length > 0 ? "Restbetrag nach Abschlägen" : "Schlussbetrag",
  ...
  return <SafeAreaView style={s.safe}>
      <ScrollView contentContainerStyle={s.wrap}>
        ...
        <View style={s.card}>
          ...
          {abschlaege.length ? abschlaege.map(a => <View key={a.id} style={s.absRow}>
              ...
              </View>) : <Text style={s.muted}>Keine Abschläge vorhanden.</Text>}
        </View>
        ...
        <Pressable style={s.secondaryBtn} onPress={() => navigation.navigate("RechnungEditor", {
        projectId,
        projectCode,
        rechnungId
      })}>
...
const s = createRlcStyles("SchlussrechnungScreen", {
  safe: {
    flex: 1,
