...
import { View, Text, Pressable, ActivityIndicator, Alert, ScrollView, Platform, SafeAreaView } from "react-native";
import { NativeStackScreenProps } from "@react-navigation/native-stack";
import { RootStackParamList } from "../navigation/types";
import { api } from "../lib/api";
import * as DocumentPicker from "expo-document-picker";
import * as FileSystem from "expo-file-system/legacy";
import { downloadPdf, importLocalPdf, getLocalUri, deletePdf, listDownloadedPdfs, PdfMetaItem } from "../lib/pdfStorage";
...
export default function ProjectPdfsScreen({
  route,
  navigation
}: Props) {
  const {
  ...
  function getFsRootOrThrow(): string {
    const doc = typeof FileSystem.documentDirectory === "string" ? FileSystem.documentDirectory : "";
    const cache = typeof FileSystem.cacheDirectory === "string" ? FileSystem.cacheDirectory : "";
    const root = doc || cache;
    if (!root) {
      throw new Error("FileSystem directory fehlt (document/cache).\n\n" + "Bitte Expo Go komplett schließen und neu öffnen. " + "Danach `npx expo start --lan` neu starten.");
    }
  ...
  async function uniqueTargetUri(dir: string, desiredName: string) {
    ...
    return {
      name,
      uri: `${dir}${baseStem}_${Date.now()}.pdf`
    };
  }
  async function copyPickedPdfToTarget(fromUri: string, toUri: string) {
    ...
    throw new Error("PDF konnte nicht kopiert werden. Bitte Datei erneut wählen.");
  }
  const loadOffline = useCallback(async () => {
    try {
      await ensureAllPdfDirs();
      const locals: PdfMetaItem[] = await listDownloadedPdfs(projectFsKey);
      const mapped: Row[] = locals.map(x => ({
  ...
  const onOpen = useCallback(async (r: Row) => {
    try {
  ...
  const onDelete = useCallback(async (r: Row) => {
    try {
  ...
  const onPickOfflinePdf = useCallback(async () => {
    try {
      ...
      try {
        ...
        imported = {
          uri: targetPreferred.uri,
          name: targetPreferred.name
        };
      }
      await loadOffline();
      navigation.navigate("PdfViewer", {
  ...
  function BackButton({
    ...
    return <Pressable onPress={() => nav.goBack()} style={s.backBtn}>
        <Text style={s.backTxt}>Zurück</Text>
      </Pressable>;
  }
  const header = useMemo(() => {
    return <View style={s.headerCard}>
        ...
        <View style={s.actionsRow}>
          <Pressable style={s.actionBtnPrimary} onPress={() => setRefreshTick(x => x + 1)} disabled={loading}>
            <Text style={s.actionBtnPrimaryTxt}>
              {loading ? "Lade..." : "Aktualisieren"}
            </Text>
          </Pressable>

          <Pressable style={s.actionBtnSecondary} onPress={onPickOfflinePdf} disabled={loading}>
        ...
        {offlineMode ? <View style={s.infoBox}>
  ...
  return <SafeAreaView style={s.safe}>
      <View style={s.bg}>
        ...
        {loading ? <View style={s.center}>
            <ActivityIndicator />
            <Text style={s.centerTxt}>Lade PDFs…</Text>
          </View> : <ScrollView contentContainerStyle={s.list} showsVerticalScrollIndicator={false}>
            {rows.map(r => <View key={`${r.name}:${r.url || "offline"}`} style={s.card}>
                <Pressable onPress={() => onOpen(r)} style={s.cardMain}>
                  <View style={s.cardTop}>
                    <View style={[s.dot, {
                backgroundColor: r.offline ? COLORS.accent : COLORS.accentDark
              }]} />
                ...
                <View style={s.actions}>
                  {r.offline ? <>
                      ...
                      <Pressable style={[s.btn, s.btnPrimary, r.busy ? s.btnDisabled : null]} onPress={() => onDownload(r)} disabled={r.busy}>
            ...
            {!rows.length ? <View style={s.empty}>
                <Text style={s.emptyTitle}>Keine PDFs</Text>
                <Text style={s.emptyText}>
                  {offlineMode ? "Du bist offline und hast noch keine PDFs lokal gespeichert. Gehe online und lade PDFs herunter." : "Server liefert aktuell keine PDFs für dieses Projekt."}
                </Text>
              </View> : null}
...
const s = createRlcStyles("ProjectPdfsScreen", {
  safe: {
    flex: 1,
