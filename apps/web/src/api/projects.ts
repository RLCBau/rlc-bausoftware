...
// ==================== LV des Projekts laden (lv.json aus FS) ====================
export async function fetchProjectLv(projectId: string) {
  const res = await fetch(`/api/projects/${projectId}/lv`, {
    method: "GET",
  });
  ...
  if (!res.ok || !data) {
    throw new Error("Fehler beim Laden des Projekt-LV");
  }

  if (data.ok === false) {
    throw new Error(data.error || "Fehler beim Laden des Projekt-LV");
  }

  return data; // z.B. { ok: true, items: [...] }
}
