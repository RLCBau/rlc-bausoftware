...
function toRPN(tokens) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (tk.t === "comma") {
            ...
            if (!fnFrames.length)
                throw new Error("Separatore ',' fuori funzione");
    ...
    for (const tk of rpn) {
        ...
        if (tk.t === "op") {
            if (tk.v === "u+") {
                const a = stack.pop();
                if (a == null)
                    throw new Error("Argomento mancante");
                stack.push(+a);
                continue;
            }
            if (tk.v === "u-") {
...
export function evaluateExpression(expr, vars = {}) {
    if (!expr || !expr.trim())
        return 0;
    const normalizedVars = Object.fromEntries(Object.entries(vars).map(([k, v]) => [k.toUpperCase(), Number(v)]));
