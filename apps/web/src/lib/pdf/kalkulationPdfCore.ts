...
export type KalkulationPdfCoreInput = {
  projectKey: string;
  projectTitle?: string;
  rows: any[];
...
function unitPrice(row: any): number {
  return (
    numberValue(row?.finalUnitPrice) ||
...
function breakdownText(row: any): string {
...
export async function buildKalkulationPdfCoreBlob(
  input: KalkulationPdfCoreInput
): Promise<Blob> {
  ...
  const mwst =
    numberValue(summary.mwst) ||
    numberValue(summary.tax) ||
    netto * (taxRate / 100);

  const brutto =
    numberValue(summary.brutto) ||
    numberValue(summary.totalGross) ||
    netto + mwst;

  const summaryY = (doc as any).lastAutoTable?.finalY + 5 || 63;
  ...
  const tableY = (doc as any).lastAutoTable?.finalY + 6 || 82;
