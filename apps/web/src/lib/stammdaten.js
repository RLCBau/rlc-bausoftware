...
/* =========================================================
   MAIN FUNCTION
   ========================================================= */
export function getStandardFormel(einheit) {
    if (!einheit)
        return "=N";
    const norm = normalizeEinheit(einheit);
    // 1. custom rules (utente)
    const regeln = loadRegeln();
    const match = regeln.find((r) => normalizeEinheit(r.einheit) === norm);
    if (match?.standardFormel)
        return match.standardFormel;
    // 2. default rules (sistema)
    if (DEFAULT_FORMELN[norm])
        return DEFAULT_FORMELN[norm];
