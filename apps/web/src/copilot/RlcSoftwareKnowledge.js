...
export const RLC_SOFTWARE_KNOWLEDGE = [
    {
        id: "projects",
        module: "Projekt",
...
export function searchRlcSoftwareKnowledge(query, pathname = "", limit = 8) {
    ...
    return RLC_SOFTWARE_KNOWLEDGE
        ...
        if (entry.route && q.includes(normalizeKnowledgeText(entry.route)))
            score += 8;
        return { entry, score };
    })
        .filter((x) => x.score > 0)
...
export function buildRlcKnowledgeContext(query, pathname = "") {
