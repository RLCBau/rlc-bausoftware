...
function toRpn(tokens) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (token.t === "comma") {
            ...
            if (!ops.length)
                throw new Error("Separator ',' an falscher Stelle");
            if (!fnFrames.length)
                throw new Error("Separator ',' außerhalb Funktion");
...
export function evaluate(expr, ctx = {}) {
    if (!expr || !expr.trim())
        return 0;
    const normalizedCtx = Object.fromEntries(Object.entries(ctx).map(([k, v]) => [k.toUpperCase(), Number(v)]));
