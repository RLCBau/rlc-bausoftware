...
function unitTextFromEinheit(e) {
    switch (e) {
        case "m2":
            return "m²";
        case "m3":
...
function calcMenge(item, einheit) {
    switch (einheit) {
        case "m2":
...
function ruleScore(rule, item) {
    let score = 0;
    if (rule.match.kind) {
        if (rule.match.kind !== item.kind)
            return -1;
        score += 3;
    }
    if (rule.match.layer) {
        if (!rule.match.layer.test(item.layer || ""))
            return -1;
        score += 2;
    }
    if (rule.match.text) {
        if (!rule.match.text.test(item.label || ""))
            return -1;
        score += 2;
    }
    return score;
...
// ============================================================
export function extractPosFromText(t) {
    if (!t)
        return null;
