...
import { LV } from "../pages/kalkulation/store.lv";
import { KalkulationsDatenbank } from "../pages/kalkulation/kalkulationsDatenbank";
const RLC_COPILOT_KI_AVATAR_SRC = "/assets/rlc-copilot-cartoon.webp";
function rlcNum(value) {
...
function collectPageRuntimeSnapshot() {
    ...
    const unique = (values, limit) => Array.from(new Set(values.map((x) => compactText(x)).filter(Boolean))).slice(0, limit);
    const headings = unique(Array.from(document.querySelectorAll("main h1, main h2, main h3, .content h1, .content h2, .content h3")).
        filter(isVisible).
        map((el) => el.textContent || ""), 24);
    const buttons = unique(Array.from(document.querySelectorAll("main button, .content button, main [role='button'], .content [role='button']")).
        filter(isVisible).
        map((el) => el.textContent ||
        el.getAttribute("aria-label") ||
        el.getAttribute("title") ||
        ""), 36);
    const fields = Array.from(document.querySelectorAll("main input, main select, main textarea, .content input, .content select, .content textarea")).
        filter(isVisible).
        slice(0, 40).
        map((el) => {
        const input = el;
        ...
        const nearbyLabel = input.parentElement?.previousElementSibling?.textContent || "";
        const label = compactText(explicitLabel ||
            wrappingLabel ||
            nearbyLabel ||
            input.getAttribute("aria-label") ||
            input.name || ("placeholder" in input ? input.placeholder : "") ||
            input.type, 90);
        const value = input instanceof HTMLSelectElement ?
            compactText(input.selectedOptions?.[0]?.textContent || input.value, 140) :
            input.type === "password" ?
                "***" :
    ...
    const visibleText = unique(Array.from(document.querySelectorAll("main .card, main section, .content .card, .content section")).
        filter(isVisible).
        map((el) => el.textContent || "").
        filter((text) => text.trim().length >= 20), 18);
    return {
...
function hasValidKiCalculation(row) {
    const validSources = [
        "company-calibration",
        "technical-parser",
        "recipe",
    ...
    return (rlcNum(row?.rlcKiUnitPrice) > 0 ||
...
function rowLabel(r) {
    ...
    if (pos && text)
        return `Pos. ${pos} – ${text.slice(0, 70)}`;
    if (pos)
        return `Pos. ${pos}`;
    if (text)
...
function dbEntryPrice(e) {
    return n(e.kosten?.epNetto);
}
function dbDuplicateKey(e) {
...
function getDbDuplicateGroups(rows) {
    const map = new Map();
    for (const row of rows) {
...
function getModuleKey(pathname) {
    if (pathname.startsWith("/kalkulation"))
        return "kalkulation";
    if (pathname.startsWith("/mengenermittlung"))
        return "mengenermittlung";
    if (pathname.startsWith("/cad"))
        return "cad";
    if (pathname.startsWith("/buro"))
        return "buro";
    if (pathname.startsWith("/ki"))
        return "ki";
    if (pathname.startsWith("/buchhaltung"))
        return "buchhaltung";
...
function getPageKey(pathname) {
    if (pathname === "/start" || pathname === "/start/")
        return "start-projekt";
    if (pathname.startsWith("/projekt/uebersicht") ||
    ...
    if (pathname.startsWith("/kalkulation/crm"))
        return "kalkulation-crm";
    if (pathname.startsWith("/kalkulation/rezepte") ||
    ...
    if (pathname.startsWith("/kalkulation/lv-import") ||
        pathname.startsWith("/kalkulation/lv")) {
        return "kalkulation-lv";
    }
    if (pathname.startsWith("/kalkulation/mit-ki"))
        return "kalkulation-mit-ki";
    if (pathname.startsWith("/kalkulation/datenbank/preise") ||
...
function routeLabel(pathname) {
    ...
    if (pageKey === "kalkulation-lv")
        return "Kalkulation · LV / Positionen";
    if (pageKey === "kalkulation-mit-ki")
        return "Kalkulation · KI-Kalkulation";
    ...
    if (pageKey === "start-projekt")
        return "Start · Projekt auswählen";
    if (pathname.startsWith("/kalkulation"))
        return "Kalkulation";
    if (pathname.startsWith("/mengenermittlung"))
        return "Mengenermittlung";
...
function lvScore(row) {
    return ((row.posNr ? 10 : 0) + (row.kurztext ? 10 : 0) + (row.langtext ? 6 : 0) + (row.einheit ? 5 : 0) + (n(row.menge) > 0 ? 10 : 0) + (rowPrice(row) > 0 ? 10 : 0) + (row.priceBreakdown?.length ? 8 : 0));
}
function dbScore(row) {
    return ((row.posNr ? 8 : 0) + (row.kurztext ? 10 : 0) + (row.langtext ? 6 : 0) + (row.einheit ? 6 : 0) + (n(row.menge) > 0 ? 6 : 0) + (dbEntryPrice(row) > 0 ? 12 : 0) + (row.ressourcen?.length ? 10 : 0) + (n(row.verwendungen) > 0 ? 5 : 0) +
        Math.round(n(row.confidence) * 10));
}
function getProjectCodeFromPage() {
    try {
        ...
        if (urlCode)
            return urlCode;
        const globals = [
            window.__RLC_CURRENT_PROJECT__?.code,
            window.__RLC_CURRENT_PROJECT__?.projectCode,
            window.__RLC_ACTIVE_PROJECT__?.code,
            window.__RLC_ACTIVE_PROJECT__?.projectCode,
            window.__RLC_KALKULATION_RUNTIME_SUMMARY__?.projectCode,
            window.__RLC_KALKULATION_RUNTIME_SUMMARY__?.code,
            window.__RLC_PROJECT_CODE__,
            window.rlcProjectCode
        ];
        ...
        const localStorageCodeKey = Object.keys(localStorage).find((key) => /project.*code|code.*project|baustelle|ba-\d{4}/i.test(key));
...
function getProjectTitleFromPage() {
    try {
        const globals = [
            window.__RLC_CURRENT_PROJECT__?.name,
            window.__RLC_CURRENT_PROJECT__?.title,
            window.__RLC_CURRENT_PROJECT__?.projectName,
            window.__RLC_ACTIVE_PROJECT__?.name,
            window.__RLC_ACTIVE_PROJECT__?.title,
            window.__RLC_ACTIVE_PROJECT__?.projectName,
            window.__RLC_KALKULATION_RUNTIME_SUMMARY__?.projectName,
            window.__RLC_KALKULATION_RUNTIME_SUMMARY__?.projectTitle
        ];
        for (const value of globals) {
...
function compactKiText(value) {
    ...
    if (lower.includes("bereits vollständig") || lower.includes("übersprungen")) {
        return oneLine.replace("Bereits vollständig / übersprungen:", "Bereits vollständig:");
    }
    return oneLine;
}
function uniqueCompactList(values, max = 10) {
    const seen = new Set();
    const out = [];
...
function copilotModeLabel(mode) {
    if (mode === "listening")
        return "hört zu";
    if (mode === "speaking")
        return "spricht";
...
export default function RlcKiAssistant() {
    ...
    const [tab, setTab] = React.useState("steuerung");
    ...
    const [kiLog, setKiLog] = React.useState(null);
    ...
    const [activeKiSuggestion, setActiveKiSuggestion] = React.useState(null);
    ...
    const recognitionAvailable = typeof window !== "undefined" && (!!window.SpeechRecognition ||
    ...
    React.useEffect(() => {
        try {
            ...
            if (Array.isArray(parsed) && parsed.length) {
                const clean = parsed.filter((m) => m && (m.role === "user" || m.role === "assistant") &&
    ...
    React.useEffect(() => {
        function onActiveKiSuggestion(event) {
            const detail = event.detail;
            if (!detail?.title)
                return;
    ...
    const lvDuplicateCount = React.useMemo(() => lvDuplicateGroups.reduce((sum, g) => sum + Math.max(0, g.length - 1), 0), [lvDuplicateGroups]);
    const dbDuplicateCount = React.useMemo(() => dbDuplicateGroups.reduce((sum, g) => sum + Math.max(0, g.length - 1), 0), [dbDuplicateGroups]);
    const [runtimeKalkulationSummary, setRuntimeKalkulationSummary] = React.useState(() => {
        try {
            return window.__RLC_KALKULATION_RUNTIME_SUMMARY__ || null;
        }
        catch {
            return null;
        }
    ...
    React.useEffect(() => {
        function onRuntimeSummary(e) {
            const detail = e.detail;
            if (detail)
                setRuntimeKalkulationSummary(detail);
        }
        window.addEventListener("rlc:kalkulation-runtime-summary", onRuntimeSummary);
        const currentSummary = window.__RLC_KALKULATION_RUNTIME_SUMMARY__;
        if (currentSummary)
            setRuntimeKalkulationSummary(currentSummary);
        return () => {
    ...
    const lvStats = React.useMemo(() => {
        ...
        const safeRows = lvRows.filter((r) => String(r.kurztext || "").trim() &&
            String(r.einheit || "").trim() &&
            n(r.menge) > 0 &&
            rowPrice(r) > 0).length;
        const projectCode = getProjectCodeFromPage();
        function readKiRowsForAssistant() {
            const directKeys = [
        ...
        function readServerKiSummaryForAssistant() {
            const directKeys = [
            ...
            for (const key of [...directKeys, ...fallbackKeys]) {
                try {
                    const raw = localStorage.getItem(key);
                    const parsed = raw ? JSON.parse(raw) : null;
                    if (parsed && Number(parsed.totalNet || 0) > 0)
                        return parsed;
                }
                catch {
                    // ignore broken localStorage entries
        ...
        const missingText = lvRows.filter((r) => !String(r.kurztext || "").trim() && !String(r.langtext || "").trim()).length;
        const runtime = pathname.includes("/kalkulation") && runtimeKalkulationSummary ?
            runtimeKalkulationSummary :
            null;
        const runtimeKiNet = rlcNum(runtime?.rlcKiNet) ||
            rlcNum(runtime?.rlcKiTotal) ||
            rlcNum(runtime?.kiNet) ||
            rlcNum(runtime?.kiTotal);
    ...
    const dbStats = React.useMemo(() => {
        ...
        const avgConfidence = dbRows.length > 0 ?
    ...
    function buildCopilotContext(userMessage) {
        ...
        return {
            ...
            pageRuntime,
            softwareKnowledge,
            aufmasseditor: pageKey === "mengenermittlung-aufmasseditor" ?
                {
                    purpose: "LV-bezogenes Aufmaß mit Orte/Unterorte, Aufmaßzeilen, Formeln, Mengen, Summen und Export",
    ...
    function buildCopilotPromptMessage(userMessage) {
    ...
    function startLocalProgress(titleText) {
        setOpen(true);
        setTab("steuerung");
        setKiWorking(true);
        setKiProgress(18);
    ...
    function finishLocalProgress(log) {
        setKiWorking(false);
        setKiProgress(100);
        setKiProgressText("Aktion abgeschlossen.");
        setKiLog(compactKiLog(log));
    }
    function sendPageCommand(eventName, detail) {
        window.dispatchEvent(new CustomEvent(eventName, { detail }));
        setTab("steuerung");
    }
    function runActiveKiSuggestion(activeKi) {
        if (!activeKi)
            return;
        setKiSignalPulse(false);
    ...
    const COPILOT_ROUTE_TARGETS = [
        {
            ...
            answer: "Ich öffne das Mahnwesen in der Buchhaltung. Dort prüfst du offene Posten, Mahnungen und Zahlungsstatus."
        },
        {
            keys: [
                "kostenuebersicht",
        ...
        },
        {
            ...
            answer: "Ich öffne den Aufmaß-Editor. Dort erfasst du Aufmaße direkt mit Maßen, Formeln und Positionen."
        },
        {
            keys: ["auftragsliste"],
    ...
    function resolveCopilotRouteTarget(userText) {
        ...
        if (!wantsNavigation && !asksLocation)
            return null;
    ...
    function sendSimpleCommand(eventName, action, label) {
        sendPageCommand(eventName, { action });
        setStatus(label);
    }
    function sendSimpleFilter(eventName, filter, label) {
        sendPageCommand(eventName, { filter });
        setStatus(label);
    }
    function sendRezepteCommand(action) {
    ...
    function sendVersionsvergleichCommand(action) {
    ...
    function sendCrmCommand(action) {
    ...
    function sendGaebCommand(action, extra) {
    ...
    function sendGaebFilter(filter) {
    ...
    function sendKalkulationFilter(filter) {
        sendSimpleFilter("rlc:kalkulation-filter", filter, `Filter aktiviert: ${filter}`);
    }
    function sendKalkulationAction(action) {
        const label = action === "runKi" ?
    ...
    function showGaebLocalProblems() {
        ...
        if (lvStats.missingPosNr)
            warnings.push(`${lvStats.missingPosNr} Position(en) ohne Positionsnummer.`);
        if (lvStats.missingText)
            warnings.push(`${lvStats.missingText} Position(en) ohne Kurztext/Langtext.`);
        if (lvStats.missingUnits)
            warnings.push(`${lvStats.missingUnits} Position(en) ohne Einheit.`);
        if (lvStats.missingQty)
            warnings.push(`${lvStats.missingQty} Position(en) ohne Menge oder mit Menge 0.`);
        if (lvStats.duplicateCount)
            warnings.push(`${lvStats.duplicateCount} doppelte LV-Position(en).`);
        if (lvStats.unitFixable)
            warnings.push(`${lvStats.unitFixable} Einheit(en) können automatisch normalisiert werden.`);
        finishLocalProgress({
    ...
    function makeSupportPrompt(kind) {
    ...
    function buildSupportReport() {
        const lvProblems = lvStats.missingQty +
            lvStats.missingUnits +
            lvStats.missingPrice +
            lvStats.missingBreakdown +
            lvStats.duplicateCount;
        const dbProblems = dbStats.missingPrice +
            dbStats.missingUnit +
    ...
    function getRlcCopilotBrainAnswer(userText) {
        ...
        const openSoon = (path, label) => {
            if (wantsOpen || wantsCreate) {
                setStatus(label);
                window.setTimeout(() => go(path), 450);
            }
        };
        const answer = (lines, path, label) => {
            if (path && label)
                openSoon(path, label);
        ...
        if (wantsOpen &&
            ...
            ], "/mengenermittlung/aufmasseditor", "Aufmaß-Editor wird geöffnet.");
        }
        if (wantsOpen &&
            ...
            ], "/kalkulation/mit-ki", "KI-Kalkulation wird geöffnet.");
        }
        if (wantsOpen &&
            ...
            ], "/kalkulation", "Kalkulation wird geöffnet.");
        }
        ...
        if (has([
            ...
            ], "/kalkulation/lv-import", "LV / Positionen wird geöffnet.");
        }
        ...
        if (has([
            ...
            ], "/kalkulation/rezepte", "Urkalkulation / Rezepte wird geöffnet.");
        }
        if (has([
            "datenbank",
            "kalkulationsdatenbank",
            ...
            ], "/kalkulation/datenbank", "Kalkulationsdatenbank wird geöffnet.");
        }
        if (has([
            "preise",
            "preisliste",
            ...
            ], "/kalkulation/preise", "Preise & Ressourcen wird geöffnet.");
        }
        ...
        if (has([
            ...
            ], "/kalkulation/gaeb", "GAEB-Seite wird geöffnet.");
        }
        if (has([
            "angebot",
            "pdf",
            ...
            ], "/kalkulation/angebot", "Angebot / Export wird geöffnet.");
        }
        if (has([
            "nachtrag",
            "nachträge",
            ...
            ], "/kalkulation/nachtraege", "Nachträge werden geöffnet.");
        }
        if (has([
            "crm",
            "angebot verfolgen",
            ...
            ], "/kalkulation/crm", "CRM / Angebotsverfolgung wird geöffnet.");
        }
        ...
        if (has([
            ...
            return answer([
                ...
                "/mengenermittlung", has(["aufmaß", "aufmass", "aufmasseditor", "aufmaßeditor"]) ?
        ...
        if (has([
            ...
            ], "/cad/viewer", "CAD Viewer wird geöffnet.");
        }
        if (has([
            "büro",
            "buero",
            ...
            ], "/buro", "Büro / Verwaltung wird geöffnet.");
        }
        if (has([
            "buchhaltung",
            "rechnung",
            ...
            ], "/buchhaltung", "Buchhaltung wird geöffnet.");
        }
        if (has([
            "hilfe",
            "support",
            ...
            ], "/info/support", "Support / Hilfe wird geöffnet.");
        }
        return null;
    }
    function getDirectRlcActionAnswer(userText) {
        const q = String(userText || "").toLowerCase();
        const projectCode = getProjectCodeFromPage() || "nicht erkannt";
        const projectTitle = getProjectTitleFromPage() || "Neues Projekt";
        const projectLine = `Projekt ${projectCode} – ${projectTitle}`;
    ...
    function getLocalRlcWideAnswer(userText) {
        const q = String(userText || "").toLowerCase();
        const projectCode = getProjectCodeFromPage() || "nicht erkannt";
        const projectTitle = getProjectTitleFromPage() || "Neues Projekt";
    ...
    function getHumanSmallTalkAnswer(userText) {
        ...
        const onlyGreeting = /^(ciao|buongiorno|buonasera|salve|hallo|hi|hey)\b/.test(q) &&
            q.length <= 40;
    ...
    function setGeneralConversationMode(value) {
        try {
    ...
    function isGeneralConversationModeActive() {
        try {
    ...
    function getNaturalConversationAnswer(userText) {
        const q = String(userText || "").
            toLowerCase().
            trim();
        if (!q)
        ...
        const asksGeneralTopic = (q.includes("können wir über") ||
        ...
        const clearNonProjectEverydayQuestion = generalTopicWords.some((w) => q.includes(w)) &&
            ![
                "kalkulation",
                "projekt",
                "rlc",
    ...
    function getLocalProjectAwareAnswer(userText) {
        if (pageKey !== "kalkulation-mit-ki" &&
        ...
        const projectCode = getProjectCodeFromPage() || "nicht erkannt";
        const projectTitle = getProjectTitleFromPage() || "Neues Projekt";
        const asksProject = q.includes("projekt") ||
            q.includes("code") ||
            q.includes("kontroll") ||
            q.includes("kontrol") ||
        ...
        const asksNachtrag = q.includes("nachtrag") ||
        ...
        const asksOffer = q.includes("angebot") || q.includes("pdf") || q.includes("export");
        const asksGaeb = q.includes("gaeb") ||
            q.includes("x83") ||
            q.includes("x84") ||
            q.includes("d83") ||
            q.includes("p83") ||
        ...
        const asksRisk = q.includes("risiko") ||
            q.includes("outlier") ||
    ...
    async function sendSupportMessage(customText) {
    ...
    React.useEffect(() => {
        ...
        if (!activePages.includes(pageKey))
            return;
        const projectCode = getProjectCodeFromPage();
        if (!projectCode && pageKey === "start-projekt")
            return;
        ...
        const totalProblems = problems.duplicates +
        ...
        const nextStep = problems.missingPrice > 0 ?
    ...
    function openKalkulationOutlierReportFromAssistant() {
        const fn = window.rlcOpenKalkulationOutlierReport;
    ...
    function renderActions(actions) {
        return actions.slice(0, 3).map((action) => _jsx("button", { type: "button", className: rlcClass(null, getButtonStyle(action)), disabled: !!action.disabled, onClick: action.onClick, children: action.label }, action.label));
    }
    function renderStats() {
        if (pageKey === "kalkulation-uebersicht" ||
            ...
            pageKey === "projekt-uebersicht" ||
            pageKey === "start-projekt") {
            return (_jsxs("div", { className: rlcClass(null, statsList), children: [pageKey === "kalkulation-mit-ki" && lvStats.activeKi ?
                        _jsxs("div", { className: rlcClass(null, statsRow), children: [_jsx("span", { children: "KI aktiv" }), _jsx("b", { children: safeReactText(lvStats.activeKi) })] }) :
                        null, _jsxs("div", { className: rlcClass(null, statsRow), children: [_jsx("span", { children: "Netto RLC-KI" }), _jsx("b", { children: lvStats.net > 0 ?
                                    money(lvStats.net) :
                                    runtimeKalkulationSummary ?
                                        "Wird berechnet…" :
                                        "0,00 €" })] }), _jsxs("div", { className: rlcClass(null, statsRow), children: [_jsx("span", { children: "Doppelte LV-Positionen" }), _jsx("b", { children: lvStats.duplicateCount })] }), _jsxs("div", { className: rlcClass(null, statsRow), children: [_jsx("span", { children: "Einheit fehlt" }), _jsx("b", { children: lvStats.missingUnits })] }), _jsxs("div", { className: rlcClass(null, statsRow), children: [_jsx("span", { children: "Menge fehlt / 0" }), _jsx("b", { children: lvStats.missingQty })] }), _jsxs("div", { className: rlcClass(null, statsRow), children: [_jsx("span", { children: "EP fehlt" }), _jsx("b", { children: lvStats.missingPrice })] }), _jsxs("div", { className: rlcClass(null, statsRow), children: [_jsx("span", { children: "Urkalkulation fehlt" }), _jsx("b", { children: lvStats.missingBreakdown })] })] }));
        }
        if (pageKey === "kalkulation-datenbank") {
            return (_jsxs("div", { className: rlcClass(null, statsGrid), children: [_jsx("span", { children: "Eintr\u00E4ge" }), _jsx("b", { children: dbStats.count }), _jsx("span", { children: "Dubletten" }), _jsx("b", { children: dbStats.duplicateCount }), _jsx("span", { children: "EP fehlt" }), _jsx("b", { children: dbStats.missingPrice }), _jsx("span", { children: "Einheit fehlt" }), _jsx("b", { children: dbStats.missingUnit }), _jsx("span", { children: "Ressourcen fehlen" }), _jsx("b", { children: dbStats.missingResources }), _jsx("span", { children: "Risiko hoch/kritisch" }), _jsx("b", { children: dbStats.highRisk }), _jsx("span", { children: "Confidence niedrig" }), _jsx("b", { children: dbStats.lowConfidence }), _jsx("span", { children: "\u25C9 Confidence" }), _jsx("b", { children: pct(dbStats.avgConfidence) }), _jsx("span", { children: "Wiederverwendet" }), _jsx("b", { children: dbStats.used })] }));
        }
        return null;
    }
    function getActions() {
    ...
    function renderSupportChips() {
        ...
        return chips.map((chip) => _jsx("button", { type: "button", className: rlcClass(null, chipBtn), onClick: () => void sendSupportMessage(makeSupportPrompt(chip.kind)), children: chip.label }, chip.kind));
    }
    return (_jsxs(_Fragment, { children: [_jsx("style", { children: `
      ...
      ` }), open ?
                _jsx("div", { className: rlcClass("rlc-copilot-layer", overlay), children: _jsxs("aside", { className: rlcClass("rlc-copilot-popover", drawer), role: "dialog", "aria-modal": "false", "aria-label": "RLC Copilot", children: [_jsxs("div", { className: rlcClass(null, head), children: [_jsxs("div", { className: rlcClass(null, headLeft), children: [_jsxs("div", { className: rlcClass(null, avatarShell(copilotMode)), "aria-label": `RLC Copilot ${copilotModeLabel(copilotMode)}`, children: [_jsx("div", { className: rlcClass(null, avatarAura(copilotMode)) }), _jsx("div", { className: rlcClass(null, avatarRealPhotoIcon(copilotMode)) })] }), _jsxs("div", { children: [_jsx("div", { className: rlcClass(null, title), children: "RLC Copilot" }), _jsx("div", { className: rlcClass(null, sub), children: current }), _jsx("div", { className: rlcClass(null, modePill(copilotMode)), children: copilotModeLabel(copilotMode) })] })] }), _jsxs("div", { className: rlcClass(null, headActions), children: [_jsx("button", { type: "button", className: rlcClass(null, isListening ? iconBtnActive : iconBtn), onClick: startVoiceInput, title: "Mit RLC Copilot sprechen", children: _jsxs("svg", { width: "20", height: "20", viewBox: "0 0 24 24", fill: "none", "aria-hidden": "true", children: [_jsx("path", { d: "M12 14c1.7 0 3-1.3 3-3V6c0-1.7-1.3-3-3-3S9 4.3 9 6v5c0 1.7 1.3 3 3 3Z", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), _jsx("path", { d: "M5 11c0 3.9 3.1 7 7 7s7-3.1 7-7", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), _jsx("path", { d: "M12 18v3", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" })] }) }), _jsx("button", { type: "button", className: rlcClass(null, voiceEnabled ? iconBtnActive : iconBtn), onClick: toggleVoice, title: "Sprachausgabe ein/aus", children: _jsxs("svg", { width: "20", height: "20", viewBox: "0 0 24 24", fill: "none", "aria-hidden": "true", children: [_jsx("path", { d: "M4 9v6h4l5 4V5L8 9H4Z", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round" }), _jsx("path", { d: "M17 9.5c.8.8 1.2 1.6 1.2 2.5S17.8 13.7 17 14.5", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), _jsx("path", { d: "M19.5 7c1.4 1.4 2.2 3.1 2.2 5s-.8 3.6-2.2 5", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" })] }) }), _jsx("button", { type: "button", className: rlcClass(null, closeBtn), onClick: () => setOpen(false), children: _jsx("svg", { width: "22", height: "22", viewBox: "0 0 24 24", fill: "none", "aria-hidden": "true", children: _jsx("path", { d: "M6 6l12 12M18 6 6 18", stroke: "currentColor", strokeWidth: "2.5", strokeLinecap: "round" }) }) })] })] }), _jsxs("div", { className: rlcClass(null, tabs), children: [_jsx("button", { type: "button", className: rlcClass(null, tab === "steuerung" ? tabActive : tabBtn), onClick: () => setTab("steuerung"), children: "Analyse" }), _jsx("button", { type: "button", className: rlcClass(null, tab === "support" ? tabActive : tabBtn), onClick: () => setTab("support"), children: "Chat" })] }), _jsxs("div", { className: rlcClass(null, liveStatusBox(copilotMode)), children: [_jsx("span", { className: rlcClass(null, liveDot(copilotMode)) }), _jsx("span", { children: copilotMode === "listening" ?
                                            ...
                                            copilotMode === "speaking" ?
                                                ...
                                                copilotMode === "analyzing" ?
                                                    ...
                                                    "Bereit für Analyse, Chat oder Sprachbefehl." })] }), tab === "steuerung" ?
                                _jsxs("div", { className: rlcClass(null, body), children: [pageKey !== "kalkulation-mit-ki" ?
                                            _jsx("div", { className: rlcClass(null, speech), children: pageIntro(pageKey) }) :
                                            null, secretaryAlert ?
                                            _jsx("div", { className: rlcClass(null, {
                                                    ...
                                                    animation: kiSignalPulse ?
                                                        ...
                                                        undefined
                                                }), children: secretaryAlert }) :
                                            null, status ? _jsx("div", { className: rlcClass(null, successBox), children: status }) : null, kiWorking || kiProgress > 0 || kiLog ?
                                            _jsxs("div", { className: rlcClass(null, progressBox), children: [_jsxs("div", { className: rlcClass(null, progressHead), children: [_jsx("b", { children: kiWorking ? "KI arbeitet" : "KI-Protokoll" }), _jsxs("span", { children: [kiProgress, "%"] })] }), _jsx("div", { className: rlcClass(null, progressTrack), children: _jsx("div", { className: rlcClass(null, { ...progressFill, width: `${kiProgress}%` }) }) }), kiProgressText ?
                                                        _jsx("div", { className: rlcClass(null, progressText), children: kiProgressText }) :
                                                        null, kiLog ?
                                                        _jsxs("div", { className: rlcClass(null, changeLogBox), children: [_jsx("div", { className: rlcClass(null, changeLogTitle), children: kiLog.title }), kiLog.changes.length ?
                                                                    _jsxs("div", { className: rlcClass(null, changeList), children: [_jsx("div", { className: rlcClass(null, changeSectionTitle), children: "\u00C4nderungen" }), kiLog.changes.map((x, idx) => _jsxs("div", { className: rlcClass(null, changeItem), children: ["\u2013 ", x] }, `c-${idx}`))] }) :
                                                                    null, kiLog.warnings?.length ?
                                                                    _jsxs("div", { className: rlcClass(null, warningList), children: [_jsx("div", { className: rlcClass(null, warningSectionTitle), children: "Warnungen" }), kiLog.warnings.map((x, idx) => _jsxs("div", { className: rlcClass(null, warningItem), children: ["\u2013 ", x] }, `w-${idx}`))] }) :
                                                                    null, kiLog.unchanged?.length ?
                                                                    _jsxs("div", { className: rlcClass(null, unchangedList), children: [_jsx("div", { className: rlcClass(null, unchangedSectionTitle), children: "\u00FCbersprungen / unver\u00E4ndert" }), kiLog.unchanged.map((x, idx) => _jsxs("div", { className: rlcClass(null, unchangedItem), children: ["\u2013 ", x] }, `u-${idx}`))] }) :
                                                                    null, !kiLog.changes.length &&
                                                                    !kiLog.warnings?.length &&
                                                                    !kiLog.unchanged?.length ?
                                                                    _jsx("div", { className: rlcClass(null, unchangedItem), children: "Keine Detailmeldungen vorhanden." }) :
                                                                    null] }) :
                                                        null] }) :
                                            null, _jsxs("div", { className: rlcClass(null, moduleBox), children: [_jsx("div", { className: rlcClass(null, boxTitle), children: pageKey === "kalkulation-mit-ki" ?
                                                        "Copilot Analyse" :
                                                        current }), renderStats(), renderActions(getActions())] })] }) :
                                _jsxs("div", { className: rlcClass(null, chatBody), children: [_jsxs("div", { ref: chatListRef, className: rlcClass(null, chatList), children: [_jsxs("div", { className: rlcClass(null, speechSmall), children: ["Copilot-Kontext: ", _jsx("b", { children: current }), _jsx("br", {}), pathname] }), _jsx("div", { className: rlcClass(null, supportActions), children: renderSupportChips() }), messages.map((m, idx) => _jsx("div", { className: rlcClass(null, m.role === "assistant" ? assistantMsg : userMsg), children: m.text }, `${m.role}-${idx}`))] }), _jsxs("div", { className: rlcClass(null, composer), children: [_jsx("button", { type: "button", className: rlcClass(null, isListening ? micBtnActive : micBtn), onClick: startVoiceInput, title: "Frage diktieren", disabled: !recognitionAvailable, children: isListening ?
                                                        "Stop" :
                                                        _jsxs("svg", { width: "21", height: "21", viewBox: "0 0 24 24", fill: "none", "aria-hidden": "true", children: [_jsx("path", { d: "M12 14c1.7 0 3-1.3 3-3V6c0-1.7-1.3-3-3-3S9 4.3 9 6v5c0 1.7 1.3 3 3 3Z", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), _jsx("path", { d: "M5 11c0 3.9 3.1 7 7 7s7-3.1 7-7", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), _jsx("path", { d: "M12 18v3", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" })] }) }), _jsx("textarea", { className: rlcClass(null, textarea), value: input, onChange: (e) => setInput(e.target.value), placeholder: isListening ?
                                                        "Ich höre zu…" :
                                                        "Frag RLC Copilot zum Softwareziel…", onKeyDown: (e) => {
                                                        if (e.key === "Enter" && !e.shiftKey) {
                                                            e.preventDefault();
                                                            void sendSupportMessage();
                                                        }
                                                    } }), _jsx("button", { type: "button", className: rlcClass(null, voiceEnabled ? voiceBtnActive : voiceBtn), onClick: toggleVoice, title: "Antwort vorlesen", disabled: !voiceAvailable, children: _jsxs("svg", { width: "20", height: "20", viewBox: "0 0 24 24", fill: "none", "aria-hidden": "true", children: [_jsx("path", { d: "M4 9v6h4l5 4V5L8 9H4Z", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round" }), _jsx("path", { d: "M17 9.5c.8.8 1.2 1.6 1.2 2.5S17.8 13.7 17 14.5", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" }), _jsx("path", { d: "M19.5 7c1.4 1.4 2.2 3.1 2.2 5s-.8 3.6-2.2 5", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round" })] }) }), _jsx("button", { type: "button", className: rlcClass(null, sendBtn), disabled: busy || !input.trim(), onClick: () => void sendSupportMessage(), children: busy ? "..." : "Senden" })] })] })] }) }) :
                null, _jsx("button", { type: "button", className: rlcClass(null, kiSignalPulse ? floatBtnPulse : floatBtn), onClick: () => {
                    setOpen(true);
                    setKiSignalPulse(false);
                }, "aria-label": "RLC Copilot \u00F6ffnen", title: "RLC Copilot \u00F6ffnen", children: _jsx("span", { className: rlcClass(null, floatAvatarRealPhoto), "aria-hidden": "true" }) })] }));
}
const overlay = {
    position: "fixed",
    inset: 0,
...
function avatarRealPhotoIcon(mode) {
    return {
        position: "relative",
        zIndex: 2,
        width: 50,
...
function avatarWomanPortrait(mode) {
    return {
        width: 44,
        height: 44,
        borderRadius: 18,
...
function avatarEye(mode) {
    return {
        width: 4,
        height: 4,
        borderRadius: 999,
        background: "#0F172A",
        animation: mode === "speaking" || mode === "listening" ?
            "rlcCopilotBlink 3s ease-in-out infinite" :
            undefined
    };
}
function avatarMouth(mode) {
    return {
        position: "absolute",
        bottom: 8,
        ...
        animation: mode === "speaking" ?
            "rlcCopilotTalk 0.55s ease-in-out infinite" :
            undefined
    };
}
function avatarMicRing(mode) {
    return {
        position: "absolute",
        bottom: 0,
        right: 0,
...
const floatBtnPulse = {
    ...
    boxShadow: "0 0 0 8px rgba(20,110,245,0.18), 0 0 0 16px rgba(20,110,245,0.10), 0 18px 44px rgba(20,110,245,0.48)",
    animation: "rlcKiPulse 1s ease-in-out infinite"
};
const floatAvatarRealPhoto = {
    width: 50,
    height: 50,
    borderRadius: 14,
    backgroundImage: `url(${RLC_COPILOT_KI_AVATAR_SRC})`,
...
const floatAvatarEyes = {
    display: "flex",
    gap: 9,
    alignItems: "center"
};
const floatAvatarEye = {
    width: 4,
    height: 4,
    borderRadius: 999,
    background: "#111827"
};
const floatAvatarMouth = {
    width: 9,
    height: 4,
