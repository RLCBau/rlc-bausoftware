...
export const CalendarDB = {
    list() {
        return load().sort((a, b) => new Date(a.start).getTime() - new Date(b.start).getTime());
    },
    blank() {
        const now = new Date();
    ...
    exportICS(evts) {
        ...
        for (const e of evts) {
            ...
            if (e.notes)
                lines.push("DESCRIPTION:" + escapeIcsText(e.notes));
            if (e.projectId)
                lines.push("X-RLC-PROJECTID:" + escapeIcsText(e.projectId));
