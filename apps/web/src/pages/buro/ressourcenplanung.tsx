...
export default function Ressourcenplanung() {
  ...
  const resources = React.useMemo<ResourceRow[]>(() => {
  ...
  const projects = React.useMemo(
    () => Array.from(new Set(assign.map((a) => a.projectId).filter(Boolean))).sort(),
    [assign]
  );

  const cellData = React.useCallback(
  ...
  const sumDay = React.useCallback(
    (rId: string, day: string) => {
  ...
  const newAssign = React.useCallback(
    (rId: string, date: string) => {
      const a: ResAssign = {
  ...
  const upd = React.useCallback((patch: ResAssign) => {
    ResDB.upsert(patch);
    setAssign(ResDB.list());
  }, []);
  ...
  const clearWeek = React.useCallback(() => {
    if (!confirm("Diese Woche wirklich komplett leeren?")) return;
