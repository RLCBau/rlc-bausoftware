...
const load = (): RlcEmployee[] => {
  try {
    const raw = localStorage.getItem(KEY);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? (parsed as RlcEmployee[]) : [];
  } catch {
    return [];
  }
};

const save = (rows: RlcEmployee[]) => {
...
function normalizeEmployee(e: RlcEmployee): RlcEmployee {
  const raw = e as RlcEmployee & { projectId?: string };

  const normalized: RlcEmployee & { projectId?: string } = {
...
export const PersonalDB = {
  ...
  create(projectId?: string): RlcEmployee {
    const base: RlcEmployee & { projectId?: string } = {
  ...
  async attach(empId: string, file: File): Promise<EmpAttachment | null> {
    const all = load();
    ...
    const attachment: EmpAttachment = {
      id: crypto.randomUUID(),
      name: file.name,
      mime: file.type,
      size: file.size,
  ...
  exportCSV(rows: RlcEmployee[]) {
    const header =
      "id;name;role;email;phone;hourlyRate;costCenter;projects;employmentType;contractStart;contractEnd;vacationTotal;vacationTaken";
  ...
  importCSV(txt: string) {
    ...
    for (const r of rows) {
      try {
        const employee: RlcEmployee = normalizeEmployee({
          ...
          employmentType: (r[8] as RlcEmployee["employmentType"]) || "Vollzeit",
  ...
  importJSON(txt: string) {
    try {
      const data: RlcEmployee[] = JSON.parse(txt || "[]");
