// apps/web/src/pages/kalkulation/preise.tsx
...
function autoCorrectRow(row: CatalogPos): CatalogPos {
  ...
  if (suggestion !== null && suggestion !== undefined) {
    if (ep <= 0) nextEp = suggestion;
    else if (range && (ep < range.min || ep > range.max)) nextEp = suggestion;
  }
  ...
  return (
    <div style={page}>
      {busy ? (
        <div style={busyOverlay}>
          <div style={busyBox}>
            <div style={spinner} />
            <div>
              <div style={busyTitle}>Bitte warten</div>
              <div style={busySub}>{busyText || "Vorgang läuft…"}</div>
            </div>
          </div>
        </div>
      ) : null}
      ...
      <section style={heroCard}>
        <div>
          <div style={eyebrow}>RLC Kalkulationsdatenbank</div>
          <h1 style={title}>Preise einfügen</h1>
          <p style={subtitle}>
        ...
        <div style={heroActions}>
          ...
          <button style={btnSecondary} disabled={busy} onClick={exportCSV}>
            CSV-Export
          </button>
      ...
      {editor ? (
        <section style={editCard}>
          <div style={sectionHead}>
            <div>
              <h2 style={sectionTitle}>Preisposition bearbeiten</h2>
          ...
          <div style={manualGrid}>
            <Field label="PosNr / Ref">
              <input
                value={editor.posNr}
                onChange={(e) => setEditor({ ...editor, posNr: e.target.value })}
                style={input}
              />
            </Field>
            ...
            <Field label="EP netto">
              <input
                type="number"
                value={editor.ep}
                onChange={(e) => setEditor({ ...editor, ep: e.target.value })}
                style={inputStrong}
              />
            </Field>
      ...
      <section style={card}>
        <div style={sectionHead}>
          <div>
            <h2 style={sectionTitle}>Neue Preisposition</h2>
            <div style={sectionText}>
        ...
        <div style={manualGrid}>
          <Field label="PosNr / Ref">
            <input
              value={manual.posNr}
              onChange={(e) => setManual({ ...manual, posNr: e.target.value })}
              style={input}
          ...
          <Field label="EP netto">
            <input
              type="number"
              value={manual.ep}
              onChange={(e) => setManual({ ...manual, ep: e.target.value })}
              style={input}
            />
          </Field>
      ...
      <section style={card}>
        <div style={sectionHead}>
          ...
          <div style={buttonRow}>
            <button
              style={viewMode === "alle" ? btnPrimary : btnSecondary}
              disabled={busy}
            ...
            <button
              style={viewMode === "pruefen" ? btnWarning : btnSecondary}
              disabled={busy || !pruefungDone}
            ...
            <button
              style={viewMode === "fehler" ? btnDanger : btnSecondary}
              disabled={busy || !pruefungDone}
            ...
            <button
              style={viewMode === "doppelte" ? btnWarning : btnSecondary}
              disabled={busy || !pruefungDone}
        ...
        <div style={chipRow}>
          {gruppen.map((g) => (
            <button
              ...
              disabled={busy}
              onClick={() => setGruppe(g)}
              style={gruppe === g ? chipActive : chip}
            >
              {g}
        ...
        <div style={actionBox}>
          <div style={{ display: "grid", gap: 10 }}>
            <div>
              <h2 style={sectionTitle}>KI-Learning / Quality Gate</h2>
              <div style={sectionText}>
                  ...
                  return (
                    ...
                    >
                      ...
                      {entry.parameter?.warning ? (
                        <div style={{ color: "#92400E", fontSize: 13, fontWeight: 800 }}>
                          ⚠ {entry.parameter.warning}
                        </div>
                      ) : null}
                      ...
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        {QUALITY_GATE_STATUSES.map((s) => (
                          <button
                            key={s}
                            type="button"
                            style={s === status ? btnPrimary : btnSecondary}
        ...
        {stat ? <div style={alertSuccess}>{stat}</div> : null}
      </section>

      <section style={card}>
        <div style={sectionHead}>
          <div>
            <h2 style={sectionTitle}>Preispositionen</h2>
            <div style={sectionText}>
              Maximal 700 Zeilen sichtbar. Für große Preislisten Suche/Filter verwenden.
            </div>
          </div>

          <label style={selectAllBox}>
            <input
              type="checkbox"
              disabled={busy}
              ...
              {view.map((row, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={td}>
                      {!pruefungDone ? (
                        <span style={pillNeutral}>—</span>
                      ) : meta?.status === "error" ? (
                        <span style={pillError}>Fehler</span>
                      ) : meta?.status === "warning" ? (
                        <span style={pillWarning}>Prüfen</span>
                      ) : meta?.status === "duplicate" ? (
                        <span style={pillWarning}>
                          {meta.keepBestDuplicate ? "Duplikat behalten" : "Duplikat"}
                        </span>
                      ) : (
                        <span style={pillOk}>OK</span>
                      )}
                    </td>

                    <td style={tdMono}>{normalized.posNr}</td>
                    <td style={td}>{normalized.kurztext}</td>
                    <td style={tdMuted}>
                      {String((normalized as any).langtext || "").trim() || "—"}
                    </td>
                    <td style={td}>{normalizeUnit(normalized.einheit)}</td>
                    <td style={tdRight}>{money((normalized as any).ep)}</td>
                    <td style={td}>
                      <span style={groupBadge((normalized as any).gruppe)}>
                        {(normalized as any).gruppe || "—"}
                      </span>
                    </td>
                    <td style={tdMono}>{pruefungDone ? meta?.score ?? "—" : "—"}</td>
                    <td style={tdMono}>{toRefKey(normalized)}</td>
                    <td style={td}>
