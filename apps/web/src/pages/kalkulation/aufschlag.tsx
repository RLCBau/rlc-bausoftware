  ...
  return (
    <div className={rlcClass(null, page)}>
      <section className={rlcClass("rlc-page-hero", heroCard)}>
        <div>
          <div className={rlcClass(null, eyebrow)}>RLC Elite Preissteuerung</div>
          <h1 className={rlcClass(null, title)}>Preisaufschlag / Rabatt</h1>
        ...
        <div className={rlcClass(null, heroActions)}>
          <button className={rlcClass(null, btnSecondary)} onClick={importFromLv}>
            LV neu laden
          </button>
          ...
          <button className={rlcClass(null, btnSecondary)} onClick={downloadCsv} disabled={!rows.length}>
            CSV Export
          </button>
          ...
          <button className={rlcClass(null, btnSecondary)} onClick={() => nav("/kalkulation/manuell")}>
            Manuell
          </button>

          <button className={rlcClass(null, btnSecondary)} onClick={() => nav("/kalkulation/mit-ki")}>
            KI
          </button>

          <button className={rlcClass(null, btnSecondary)} onClick={() => nav("/kalkulation/angebot")}>
            Angebot
          </button>
        </div>
        ...
        <input
          ...
          }} className="rlc-migrated-pages-kalkulation-aufschlag-tsx-853" />
      ...
      <section className={rlcClass(null, card)}>
        <div className={rlcClass(null, sectionHead)}>
          <div>
            <h2 className={rlcClass(null, sectionTitle)}>Regeln & Filter</h2>
        ...
        <div className={rlcClass(null, ruleGrid)}>
          <Field label="Modus">
            <select
              value={params.mode}
              onChange={(e) =>
          ...
          <Field label={params.mode === "ziel_ep" ? "Ziel-EP €" : "Wert %"}>
            <input
              ...
              step="0.01"
              value={params.value}
              onChange={(e) => setParams({ ...params, value: n(e.target.value) })} className={rlcClass(null,
              input)} />
        ...
        <div className={rlcClass(null, buttonRow)}>
          ...
          <button className={rlcClass(null, btnSecondary)} onClick={() => toggleAll(true)}>
            Alle markieren
          </button>

          <button className={rlcClass(null, btnSecondary)} onClick={() => toggleAll(false)}>
            Markierung löschen
          </button>

          <button className={rlcClass(null, btnSecondary)} onClick={resetManualOverrides}>
      ...
      <section className={rlcClass(null, card)}>
        <div className={rlcClass(null, sectionHead)}>
          <div>
            <h2 className={rlcClass(null, sectionTitle)}>Preisvorschau</h2>
            <div className={rlcClass(null, sectionText)}>
          ...
          <table className={rlcClass(null, table)}>
            <thead>
              <tr>
                ...
                <th className={rlcClass(null, thRight)}>EP alt</th>
                <th className={rlcClass(null, thRight)}>EP neu</th>
                <th className={rlcClass(null, thRight)}>GP alt</th>
                <th className={rlcClass(null, thRight)}>GP neu</th>
                <th className={rlcClass(null, thRight)}>Diff.</th>
                <th className={rlcClass(null, thRight)}>%</th>
              </tr>
            </thead>
              ...
              {visibleRows.map((row, idx) => {
                ...
                return (
                  <tr
                    ...
                    <td className={rlcClass(null, tdText)} title={`${row.kurztext}\n${row.langtext}`}>
                      <b>{row.kurztext || "—"}</b>
                      {row.langtext ? <div className={rlcClass(null, tiny)}>{row.langtext}</div> : null}
                    </td>

                    <td className={rlcClass(null, td)}>{row.einheit || "—"}</td>
                    <td className={rlcClass(null, tdRight)}>{num(row.menge)}</td>
                    <td className={rlcClass(null, tdRight)}>{money(row.epAlt)}</td>
                    ...
                    <td className={rlcClass(null, tdRight)}>{money(row.gpAlt)}</td>
                    <td className={rlcClass(null, tdRight)}>{money(row.gpNeu)}</td>
...
function Field({
  ...
  return (
    <label className="rlc-migrated-pages-kalkulation-aufschlag-tsx-854">
