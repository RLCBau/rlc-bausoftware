...
function exactOrShortGeneric(kurz, values) {
    const k = norm(kurz);
...
export function getWorkTypeProfile(key) {
    return WORK_TYPE_PROFILES.find((p) => p.key === key) || null;
}
export function shouldForceLocalCalculation(key) {
...
export function isForbiddenForWorkType(args) {
    ...
    if (profile.forbiddenGroups.includes(group))
        return true;
    if (profile.forbiddenResourceIds.includes(resourceId))
        return true;
    if (profile.forbiddenWords.some((w) => text.includes(norm(w))))
        return true;
    return false;
}
function ambiguous(title, message, alternatives) {
    return {
        key: "unknown",
        confidence: 0.2,
        ambiguous: true,
...
export function detectWorkType(input) {
    ...
    if (exactOrShortGeneric(kurz, ["asphalt", "asphaltarbeiten"])) {
        return ambiguous("Asphalt-Leistung unklar", "Asphalt ist zu ungenau. Es muss klar sein, ob gefräst, aufgenommen, eingebaut oder wiederhergestellt wird.", [
            "Asphalt fräsen",
            "Asphalttragschicht einbauen",
            "Asphaltdeckschicht einbauen",
    ...
    if (exactOrShortGeneric(kurz, ["pflaster", "pflasterarbeiten"])) {
        return ambiguous("Pflaster-Leistung unklar", "Pflaster ist zu ungenau. Es muss klar sein, ob aufgenommen, neu verlegt oder nur die Bettung/Fuge bearbeitet wird.", [
            "Pflaster aufnehmen",
            "Pflaster neu verlegen",
            "Pflasterfläche wiederherstellen",
            "Splittbett herstellen",
    ...
    if (exactOrShortGeneric(kurz, ["kies", "schotter", "tragschicht", "material"])) {
        return ambiguous("Tragschicht / Material unklar", "Der Begriff ist zu allgemein. Es muss klar sein, ob Material geliefert, eingebaut, verdichtet oder entsorgt wird.", [
    ...
    if (exactOrShortGeneric(kurz, ["rohr", "leitung", "kabel", "speedpipe"])) {
        return ambiguous("Leitungsleistung unklar", "Die Leitungsleistung ist zu ungenau. Es muss klar sein, ob ein Graben hergestellt oder nur Rohr/Speedpipe/Kabel verlegt wird.", [
            "Leitungsgraben herstellen",
            "Rohrleitung verlegen",
            "Speedpipe verlegen",
            "Kabelschutzrohr verlegen",
    ...
    if (exactOrShortGeneric(kurz, ["graben"])) {
        return ambiguous("Graben-Leistung unklar", "Graben ist zu allgemein. Bitte unterscheiden zwischen Aushub, Leitungsgraben, Verfüllung oder kompletter Leitungsbauleistung.", [
            "Leitungsgraben herstellen",
    ...
    return {
        ...
        message: "Die Leistungsart konnte nicht sicher erkannt werden. Bitte genauer beschreiben, z. B. Planie herstellen, Asphalt fräsen, Frostschutzschicht herstellen, Leitungsgraben herstellen, Speedpipe verlegen, Schacht setzen oder Bordstein setzen.",
    };
}
