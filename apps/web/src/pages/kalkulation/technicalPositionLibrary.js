...
export const TECHNICAL_POSITIONS = Array.from(new Map(positions.map((p) => [p.id, p])).values());
export function getTechnicalPositions() {
    return TECHNICAL_POSITIONS;
}
export function getTechnicalPositionCount() {
    ...
    for (const p of TECHNICAL_POSITIONS) {
        ...
        for (const s of p.synonyms) {
            ...
            if (!ss)
                continue;
            if (text.includes(ss))
                score += Math.min(30, ss.length);
        }
        if (input.einheit && p.unit === input.einheit)
            score += 8;
