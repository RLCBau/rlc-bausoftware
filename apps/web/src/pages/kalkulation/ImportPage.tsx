// apps/web/src/pages/kalkulation/ImportPage.tsx
    ...
    <div style={page}>
      <section style={hero}>
        <div>
          ...
          <h1 style={title}>Import-Zentrale</h1>
          <p style={subtitle}>
        ...
        <div style={heroActions}>
          <button style={btnPrimary} onClick={() => fileRef.current?.click()} disabled={busy}>
            {busy ? "Import läuft…" : "Dateien importieren"}
          </button>

          <button style={btnSecondary} onClick={() => nav("/kalkulation/gaeb")}>
            GAEB Spezialimport
          </button>

          <button style={btnSecondary} onClick={() => nav("/kalkulation/lv-upload")}>
            LV hochladen / erstellen
          </button>

          <button style={btnSecondary} onClick={() => nav("/kalkulation/preise")}>
            Preise einfügen
          </button>
        </div>
      ...
      <section style={card}>
        <div style={sectionHead}>
          <div>
            <h2 style={sectionTitle}>Import-Protokoll</h2>
            <div style={sectionText}>
              Übersicht der letzten Importvorgänge dieser Sitzung.
            </div>
          </div>

          <button style={btnSecondary} onClick={exportLogCsv} disabled={!logs.length}>
            Protokoll CSV
          </button>
        </div>
        ...
        <div style={tableWrap}>
          <table style={table}>
            ...
            <tbody>
              {logs.map((l) => (
                <tr key={l.id}>
                  <td style={td}>{l.createdAt}</td>
                  <td style={td}>{l.type}</td>
                  <td style={td}>{l.fileName}</td>
                  <td style={tdRight}>{l.count}</td>
                  <td style={td}>
                    <span
                      style={
                        l.status === "ok"
...
function ImportCard({
  ...
  return (
    <div style={card}>
      ...
      <button style={{ ...btnSecondary, marginTop: 14 }} onClick={onClick}>
        {action}
      </button>
    </div>
  );
