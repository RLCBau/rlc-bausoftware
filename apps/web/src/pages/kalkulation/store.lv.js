...
function currentProjectStorageKey() {
    try {
        const raw = localStorage.getItem(PROJECT_KEY_STORAGE) ||
...
function sanitizeAuftragType(value) {
...
function sanitizeCalcStatus(value) {
    const v = String(value ?? "").trim().toLowerCase();
...
function sanitizeBreakdownGroup(value) {
...
function normalizePriceBreakdownLine(line) {
    const qty = toSafeNumber(line.qty, 1);
    const price = toSafeNumber(line.price, 0);
    const total = typeof toNumber(line.total) === "number"
...
function normalizePriceBreakdown(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .map((x) => normalizePriceBreakdownLine(x))
        .filter((x) => x.total > 0 || x.price > 0 || x.qty > 0);
}
function sumPriceBreakdown(lines) {
    if (!Array.isArray(lines))
        return 0;
    return round2(lines.reduce((sum, line) => sum + toSafeNumber(line.total), 0));
}
function computeGesamt(menge, preis, gesamt) {
...
function comparePosNr(a, b) {
    return a.localeCompare(b, "de", { numeric: true, sensitivity: "base" });
}
function sortAuftraege(rows) {
...
function makeRow(row) {
    ...
    const finalUnitPrice = toNumber(row.finalUnitPrice) ??
        toNumber(row.preis) ??
    ...
    return {
        ...
        auftragType: sanitizeAuftragType(row.auftragType) ?? linkedAuftrag?.type ?? undefined,
        ...
        rlcPreisMin: toNumber(row.rlcPreisMin),
        rlcPreisAvg: toNumber(row.rlcPreisAvg),
        rlcPreisMax: toNumber(row.rlcPreisMax),
        rlcPreisSource: normalizeText(row.rlcPreisSource),
        rlcPreisGroup: normalizeText(row.rlcPreisGroup),
...
function parseStoredRow(input) {
    return makeRow({
        ...
        rlcPreisMin: input?.rlcPreisMin,
        rlcPreisAvg: input?.rlcPreisAvg,
        rlcPreisMax: input?.rlcPreisMax,
        rlcPreisSource: input?.rlcPreisSource,
        rlcPreisGroup: input?.rlcPreisGroup,
...
function sortRows(rows) {
    return [...rows].sort((a, b) => {
        ...
        if (aa !== ba)
            return aa.localeCompare(ba, "de", { sensitivity: "base" });
        const ai = typeof a.sortIndex === "number" ? a.sortIndex : Number.MAX_SAFE_INTEGER;
        const bi = typeof b.sortIndex === "number" ? b.sortIndex : Number.MAX_SAFE_INTEGER;
        if (ai !== bi)
            return ai - bi;
...
export const AuftragStore = {
    ...
    createUnterauftrag(name, parentId, projectId) {
...
export const LV = {
    ...
    assignAuftrag(rowIds, auftragId) {
    ...
    renumber(prefix = "01", start = 1, digits = 4) {
        const rows = readAll();
        const next = rows.map((row, idx) => makeRow({
            ...row,
    ...
    importCSV(text) {
        ...
        const rows = body
            ...
            return makeRow({
                ...
                auftragType: iAuftragType >= 0 ? parts[iAuftragType] : undefined,
                ...
                materialCost: iMaterialCost >= 0 ? toNumber(parts[iMaterialCost]) : undefined,
                laborCost: iLaborCost >= 0 ? toNumber(parts[iLaborCost]) : undefined,
                machineCost: iMachineCost >= 0 ? toNumber(parts[iMachineCost]) : undefined,
                subcontractorCost: iSubcontractorCost >= 0
                    ? toNumber(parts[iSubcontractorCost])
                    : undefined,
                disposalCost: iDisposalCost >= 0 ? toNumber(parts[iDisposalCost]) : undefined,
                overheadCost: iOverheadCost >= 0 ? toNumber(parts[iOverheadCost]) : undefined,
                riskCost: iRiskCost >= 0 ? toNumber(parts[iRiskCost]) : undefined,
                profitCost: iProfitCost >= 0 ? toNumber(parts[iProfitCost]) : undefined,
                baseUnitPrice: iBaseUnitPrice >= 0 ? toNumber(parts[iBaseUnitPrice]) : undefined,
                suggestedUnitPrice: iSuggestedUnitPrice >= 0
                    ? toNumber(parts[iSuggestedUnitPrice])
                    : undefined,
                finalUnitPrice: iFinalUnitPrice >= 0 ? toNumber(parts[iFinalUnitPrice]) : undefined,
                riskLevel: iRiskLevel >= 0 ? parts[iRiskLevel] : undefined,
                calculationStatus: iCalculationStatus >= 0 ? parts[iCalculationStatus] : undefined,
                ...
                priceBreakdown: iPriceBreakdownJson >= 0
                    ? parseJsonField(parts[iPriceBreakdownJson], [])
                    : [],
...
export const LV_CAD = {
    addFromCad(payload) {
        const row = makeRow({
            ...
            rlcPreisMin: payload.rlcPreisMin,
            rlcPreisAvg: payload.rlcPreisAvg,
            rlcPreisMax: payload.rlcPreisMax,
            rlcPreisSource: payload.rlcPreisSource,
            rlcPreisGroup: payload.rlcPreisGroup,
            meta: payload.meta,
        });
        const all = readAll();
        all.unshift(row);
    ...
    addManyFromCad(list) {
        const existing = readAll();
        const mapped = list.map((payload) => makeRow({
            id: safeId(),
            auftragId: payload.auftragId,
            auftragName: payload.auftragName,
            auftragType: payload.auftragType,
