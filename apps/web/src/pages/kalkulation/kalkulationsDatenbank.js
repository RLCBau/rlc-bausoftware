...
function tokenize(value) {
    return Array.from(new Set(norm(value)
...
function normUnit(value) {
    ...
    if (["m2", "m²", "qm", "m^2"].includes(u))
        return "m²";
    if (["m3", "m³", "cbm", "m^3"].includes(u))
        return "m³";
    if (["st", "stk", "stuck", "stück"].includes(u))
        return "stk";
    if (["psch", "ps", "pausch", "pauschal"].includes(u))
        return "psch";
    if (["lfm", "mtr", "meter"].includes(u))
        return "m";
    if (["to", "tonne", "tonnen"].includes(u))
        return "t";
    return u;
}
function classifyWork(value) {
    const t = norm(value);
    if (t.includes("bauzaun"))
        return "bauzaun";
    if (t.includes("verkehrssicherung") || t.includes("baustellenmarkierung"))
        return "verkehrssicherung";
    if (t.includes("baustelleneinricht"))
        return "baustelleneinrichtung";
    if (t.includes("baustelle") && (t.includes("räumen") || t.includes("raeumen")))
        return "baustelleraeumen";
    if (t.includes("fss") || t.includes("frostschutz"))
        return "fss";
    if (t.includes("granitbord") || t.includes("bordstein") || t.includes("bord"))
        return "bord";
    if (t.includes("kunststoffrohr") || t.includes("rohrleitung") || t.includes("leitung"))
        return "rohr";
    if (t.includes("speedpipe") || t.includes("mikroroh"))
        return "speedpipe";
    if (t.includes("aushub") || t.includes("boden") || t.includes("graben"))
        return "erdarbeit";
    if (t.includes("asphalt"))
        return "asphalt";
    if (t.includes("pflaster"))
        return "pflaster";
    if (t.includes("hoehenfestpunkt") || t.includes("höhenfestpunkt") || t.includes("vermessung"))
        return "vermessung";
    return "";
}
function priceUsable(value) {
    const v = n(value);
...
function unitPriceFromResources(resources) {
    return round2(resources.reduce((sum, r) => {
        /*
         * Wichtig:
...
function buildKosten(resources, menge, old) {
     ...
     */
    const epLooksWrong = resourceEp > 0 &&
...
function businessOverwriteKey(row) {
...
function epOf(row) {
    const v = Number(row?.kosten?.epNetto ?? 0);
    return Number.isFinite(v) ? Math.round(v * 100) / 100 : 0;
}
function gpOf(row) {
    const v = Number(row?.kosten?.gpNetto ?? 0);
    return Number.isFinite(v) ? Math.round(v * 100) / 100 : 0;
}
function mergeWithPriceHistory(oldRow, nextRow) {
    const oldEp = epOf(oldRow);
    const newEp = epOf(nextRow);
    const oldGp = gpOf(oldRow);
    const newGp = gpOf(nextRow);
    const changed = oldEp !== newEp ||
        oldGp !== newGp ||
        String(oldRow.quelle || "") !== String(nextRow.quelle || "");
    const history = Array.isArray(oldRow.priceHistory)
        ? [...oldRow.priceHistory]
        : [];
    if (changed) {
        history.unshift({
...
function dedupeDbRows(rows) {
    ...
    for (const raw of rows) {
        ...
        const oldSource = String(old.quelle || "");
        const newSource = String(row.quelle || "");
        const newWins = newSource === "x84-company-baseline" ||
...
function scoreEntry(entry, query) {
    let score = 0;
    const gruende = [];
    const qText = `${query.posNr || ""} ${query.kurztext || ""} ${query.langtext || ""}`;
    ...
    const qPrice = n(query?.kosten?.epNetto);
    ...
    if (typeof qp.schichtdickeCm === "number" &&
    ...
    if (typeof qp.grabentiefeM === "number" &&
...
function localSearch(query, limit = 10) {
    return readDb()
...
function localUpsert(row) {
    ...
    const idxByBusiness = rows.findIndex((x) => businessOverwriteKey(x) === businessOverwriteKey(nextRow));
...
async function serverBulkUpsert(items) {
    const clean = items.map(normalizeEntry);
    const json = await fetchJson(`${API_PATH}/datenbank/bulk-upsert`, {
        method: "POST",
...
function riskFromInput(riskLevel) {
    if (riskLevel === "critical" || riskLevel === "kritisch")
        return "kritisch";
    if (riskLevel === "high" || riskLevel === "hoch")
        return "hoch";
    if (riskLevel === "low" || riskLevel === "niedrig")
        return "niedrig";
    return "mittel";
}
/* ================= EXPORT API ================= */
export const KalkulationsDatenbank = {
    ...
    async countServer() {
        const json = await fetchJson(`${API_PATH}/datenbank/count`, {
            method: "GET",
            headers: authJsonHeaders(),
        });
        return typeof json?.count === "number" ? json.count : this.count();
    },
    async listServerPage(limit = 200, offset = 0) {
        const safeLimit = Math.min(Math.max(Number(limit) || 200, 1), 1000);
        const safeOffset = Math.max(Number(offset) || 0, 0);
        const json = await fetchJson(`${API_PATH}/datenbank?limit=${safeLimit}&offset=${safeOffset}`, {
            method: "GET",
            headers: authJsonHeaders(),
        });
        ...
        return {
            ...
            hasNext: typeof json?.hasNext === "boolean"
                ? json.hasNext
                : responseOffset + clean.length < total,
            hasPrev: typeof json?.hasPrev === "boolean" ? json.hasPrev : responseOffset > 0,
        };
    },
    async listServer() {
    ...
    async searchServer(query, limit = 10) {
        ...
        if (query.posNr)
            params.set("posNr", query.posNr);
        if (query.kurztext)
            params.set("q", query.kurztext);
        if (query.einheit)
            params.set("einheit", query.einheit);
        if (query.parameter?.gewerk)
            params.set("gewerk", query.parameter.gewerk);
        ...
        const json = await fetchJson(`${API_PATH}/datenbank?${params.toString()}`, {
            method: "GET",
            headers: authJsonHeaders(),
        });
        const raw = json?.rows || json?.items || json?.data || null;
    ...
    fromCalculatedPosition(input) {
        ...
        const epFromParts = round2(materialCost +
            laborCost +
            machineCost +
            subcontractorCost +
            disposalCost +
        ...
        const epNetto = round2(explicitEp > 0 ? explicitEp : epFromParts > 0 ? epFromParts : 0);
        const gpNetto = round2(explicitGp > 0 ? explicitGp : epNetto > 0 ? epNetto * qty : 0);
        /*
         * Wichtig:
