...
function makeRow(input) {
    ...
    const preis = input.preis === undefined || input.preis === null
        ? undefined
...
export const Changes = {
    ...
    remove(projectId, id) {
        const key = normalizeProjectId(projectId);
        const db = load();
        db[key] = (Array.isArray(db[key]) ? db[key] : []).filter((row) => row.id !== id);
        save(db);
    },
    clear(projectId) {
        const key = normalizeProjectId(projectId);
    ...
    importCSV(projectId, text) {
        ...
        const rows = body
            ...
            return makeRow({
                ...
                preis: iPreis >= 0 && p[iPreis] !== ""
                    ? toNumber(p[iPreis])
                    : undefined,
                status: iStatus >= 0 ? p[iStatus] : "Entwurf",
    ...
    totals(projectId, mwst) {
        const rows = Changes.list(projectId);
        const netto = round2(rows.reduce((s, r) => s + calcLine(r), 0));
