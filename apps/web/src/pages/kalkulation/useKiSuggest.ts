...
export type EliteKalkulationInputRow = {
  id?: string;
  posNr?: string;
  kurztext?: string;
  langtext?: string;
...
export type EliteKalkulationResultRow = {
  id?: string;
  posNr: string;
  kurztext: string;
  langtext?: string;
...
export type EliteKalkulationSummary = {
  totalNet: number;
...
export type EliteKalkulationResponse = {
  ok: boolean;
  source: "server" | "local-rule-engine";
  rows: EliteKalkulationResultRow[];
  summary: EliteKalkulationSummary;
  error?: string;
};
...
async function postKiSuggestBatchChunked(
  projectCode: string,
  inputRows: EliteKalkulationInputRow[],
  ...
  const chunks: EliteKalkulationInputRow[][] = [];
  ...
  async function requestChunk(
    chunk: EliteKalkulationInputRow[]
  ): Promise<any[] | null> {
    try {
        ...
        body: JSON.stringify({
          ...
          options: {
            ...
            maxOpenAiRowsPerBatch:
              options?.maxOpenAiRowsPerBatch ?? (forceRecalculate ? 20 : 8),
            forceRecalculate,
          },
        }),
      });
  ...
  async function processChunk(chunk: EliteKalkulationInputRow[]) {
  ...
  await Promise.all(
    Array.from(
      { length: Math.min(maxParallelChunks, chunks.length) },
      () => worker()
    )
  );
  ...
  for (const r of [...completedNormalized, ...serverRows]) {
    ...
    if (idKey) resultByKey.set(`id:${idKey}`, r);
    if (posKey) resultByKey.set(`pos:${posKey}`, r);
  }
  ...
  const allRows = inputRows
    ...
    .map((inputRow) => {
      ...
      return (
        (idKey ? resultByKey.get(`id:${idKey}`) : null) ||
...
function isStructuralInputRow(row: EliteKalkulationInputRow, projectCode = ""): boolean {
  ...
  if (pc && pos.toLowerCase() === pc) return true;
  if (/^ba-\d{4}/i.test(pos)) return true;
  if (/^ba-\d{4}/i.test(kurz)) return true;

  if (/^titel\s*\d*$/i.test(kurz)) return true;
  if (/^abschnitt\s*\d*$/i.test(kurz)) return true;
  if (/^kapitel\s*\d*$/i.test(kurz)) return true;

  if (gewerk.includes("gliederung")) return true;
  if (leistungsart.includes("struktur")) return true;
  if (text.includes("keine kalkulatorische leistungsposition")) return true;
...
function hasExistingCalculationData(row: EliteKalkulationInputRow): boolean {
  const breakdown =
    Array.isArray(row.priceBreakdown) && row.priceBreakdown.length > 0;

  const costs =
    n(row.materialCost) > 0 ||
    n(row.laborCost) > 0 ||
    n(row.machineCost) > 0 ||
...
function normalizePriceBreakdown(lines: unknown): PriceBreakdownLine[] {
  if (!Array.isArray(lines)) return [];

  return lines
    .map((x: any) => ({
      id: String(x?.id || safeId()),
      group: String(x?.group || "Material") as PriceBreakdownLine["group"],
...
function buildPriceBreakdownFromCosts(row: {
  ...
  function add(
    group: PriceBreakdownLine["group"],
    name: string,
    value: unknown,
    note = ""
  ) {
...
function detectLeistungsart(text: string): string {
  ...
  if (t.includes("schalung")) return "Schalarbeiten";
  if (t.includes("bewehrung")) return "Bewehrungsarbeiten";
  if (t.includes("beton")) return "Betonarbeiten";
...
function detectBauverfahren(text: string, unit: string): string {
  ...
  if (t.includes("abfuhr")) return "LKW-Transport inklusive Lade-/Kippvorgang";
  if (t.includes("verfüll")) return "Einbau lagenweise mit Verdichtung";
  if (t.includes("speedpipe")) return "Rohr-/Speedpipe-Verlegung im Leitungsgraben";
  if (t.includes("kabelschutz")) return "Kabelschutzrohr liefern und verlegen";
  if (t.includes("schalung")) return "Schalung stellen, vorhalten und entfernen";
  if (t.includes("bewehrung")) return "Bewehrung schneiden, biegen, verlegen";
  if (t.includes("beton")) return "Beton liefern, einbauen und verdichten";
...
function riskFromText(text: string, unit: string, menge: number): RiskLevel {
  ...
  if (menge > 1000) return "medium";

  return "low";
}

function confidenceFrom(row: EliteKalkulationInputRow, risk: RiskLevel): number {
  let score = 0.88;
  ...
  if (!unit) score -= 0.2;
  if (menge <= 0) score -= 0.18;
  if (risk === "medium") score -= 0.1;
  if (risk === "high") score -= 0.22;

  return Math.max(0.25, Math.min(0.98, round2(score)));
}

function normUnit(value: string): string {
...
function getPlausibleUnitRange(text: string, unitRaw: string): {
  min: number;
  avg: number;
  max: number;
  label: string;
...
function clampToPlausibleRange(price: number, text: string, unitRaw: string): number {
  const range = getPlausibleUnitRange(text, unitRaw);
  const p = n(price);

  if (p <= 0) return 0;
  if (range.min <= 0 || range.max <= 0 || range.avg <= 0) return round2(p);

  if (p < range.min || p > range.max) return round2(range.avg);
  return round2(p);
}

function unitBasePrice(text: string, unit: string): number {
  const t = text.toLowerCase();
  const u = normUnit(unit);

  const range = getPlausibleUnitRange(t, u);
  if (range.avg > 0) return range.avg;

  if (t.includes("aushub") && u === "m³") return 18.5;
  if (t.includes("abfuhr") && (u === "t" || u === "m³")) return 22;
  if (t.includes("verfüll") && u === "m³") return 26;
  if (t.includes("kies") && u === "m³") return 38;
  if (t.includes("speedpipe") && u === "m") return 7.8;
  if (t.includes("kabelschutzrohr") && u === "m") return 18.5;
  if (t.includes("rohr") && u === "m") return 24;
  if (t.includes("schalung") && u === "m²") return 42;
  if (t.includes("bewehrung") && u === "kg") return 2.15;
  if (t.includes("beton") && u === "m³") return 165;
...
function localEliteCalculateRow(
  row: EliteKalkulationInputRow
): EliteKalkulationResultRow {
  ...
  const trustedExistingRisk =
    hasExistingCalc ||
    row.calculationStatus === "manual" ||
    row.calculationStatus === "critical";

  const riskLevel: RiskLevel =
    trustedExistingRisk && row.riskLevel ? row.riskLevel : detectedRisk;
  ...
  if (!posNr) warningParts.push("Positionsnummer fehlt");
  if (!kurztext) warningParts.push("Kurztext fehlt");
  if (!einheit) warningParts.push("Einheit fehlt");
  if (menge <= 0) warningParts.push("Menge fehlt oder ist 0");
  ...
  if (riskLevel === "high") warningParts.push("Erhöhtes Kalkulations-/Nachtragsrisiko");
  if (confidence < 0.65) warningParts.push("Niedrige Kalkulationssicherheit");
  ...
  const bauverfahren =
    cleanText(row.bauverfahren) || detectBauverfahren(text, einheit);
...
function sanitizeEliteResultPrice(
  row: EliteKalkulationResultRow,
  fallback?: EliteKalkulationInputRow
): EliteKalkulationResultRow {
  const text = `${row.kurztext || ""} ${row.langtext || ""}`;
  const range = getPlausibleUnitRange(text, row.einheit);

  const offerEp =
    n((fallback as any)?.angebotUnitPrice) ||
    n((fallback as any)?.originalPreKiPrice) ||
    n(fallback?.preis) ||
...
function normalizeServerRow(
  ...
  const suggestedUnitPrice = n(
    r?.suggestedUnitPrice ?? r?.unitPrice ?? r?.ep ?? r?.preis,
    breakdownEp || n(fallback?.suggestedUnitPrice ?? fallback?.preis)
  );

  const finalUnitPrice = n(
    r?.finalUnitPrice ?? r?.suggestedUnitPrice ?? r?.unitPrice ?? r?.preis,
    suggestedUnitPrice || breakdownEp || n(fallback?.finalUnitPrice ?? fallback?.preis)
  );
  ...
  const isStructureRow =
    cleanText(r?.leistungsart ?? fallback?.leistungsart).toLowerCase().includes("struktur") ||
...
return {
    ...
    confidence: Math.max(
      0,
      Math.min(1, n(r?.confidence ?? fallback?.confidence, 0.78))
    ),
    riskLevel,
    calculationStatus,
    ...
    leistungsart:
      cleanText(r?.leistungsart ?? fallback?.leistungsart) ||
      detectLeistungsart(text),
    bauverfahren:
      cleanText(r?.bauverfahren ?? fallback?.bauverfahren) ||
      detectBauverfahren(text, einheit),

    warning:
      cleanText(r?.warning ?? r?.hinweis) ||
      (riskLevel === "high"
        ? "KI-Kalkulation vorhanden, aber mit erhöhtem Prüfbedarf"
        : "KI-Kalkulation vom Server/OpenAI übernommen"),

    aiReason:
      cleanText(r?.aiReason ?? r?.explanation ?? r?.begruendung) ||
      "Server/OpenAI-Kalkulation: Preisansatz wurde über KI bzw. Backend-Kalkulationslogik erzeugt.",


    rlcPreisMin: n(r?.rlcPreisMin ?? (fallback as any)?.rlcPreisMin),
    rlcPreisAvg: n(r?.rlcPreisAvg ?? (fallback as any)?.rlcPreisAvg),
    rlcPreisMax: n(r?.rlcPreisMax ?? (fallback as any)?.rlcPreisMax),
    rlcPreisSource: cleanText(r?.rlcPreisSource ?? (fallback as any)?.rlcPreisSource),
    rlcPreisGroup: cleanText(r?.rlcPreisGroup ?? (fallback as any)?.rlcPreisGroup),
...
function toInputRows(rows: LVPos[]): EliteKalkulationInputRow[] {
  return rows.map((r) => ({
    id: r.id,
    posNr: r.posNr,
    kurztext: r.kurztext,
...
export function useKiSuggest() {
  ...
  async function eliteCalculateRows(
    projectCode: string,
    rows: EliteKalkulationInputRow[] | LVPos[],
    optionsOverride: Record<string, any> = {}
  ): Promise<EliteKalkulationResponse> {
    setLoading(true);

    try {
      const inputRows: EliteKalkulationInputRow[] = Array.isArray(rows)
        ? rows.map((r: any) => ({
            id: r.id,
            posNr: r.posNr,
            kurztext: r.kurztext,
      ...
      if ((mode === "openai" || mode === "server") && inputRows.length) {
        ...
        const json = await postKiSuggestBatchChunked(
        ...
        if (Array.isArray(rawRows) && rawRows.length) {
          ...
          const rawByPos = new Map<string, any>();

          for (const raw of rawRows) {
            const idKey = String(raw?.id || "").trim();
            const posKey = String(raw?.posNr || raw?.positionNumber || "").trim();

            if (idKey) rawById.set(idKey, raw);
            if (posKey) rawByPos.set(posKey, raw);
          }
