...
export default function Lieferscheine() {
  ...
  const deepLinkOpenedRef = React.useRef<string>("");
  ...
  React.useEffect(() => {
    ...
    const desiredTab: Tab =
      stageParam === "freigegeben" || stageParam === "approved"
        ? "FREIGEGEBEN"
        : "INBOX";

    const sourceRows =
      desiredTab === "FREIGEGEBEN" ? freigegebenRows : inboxRows;
    ...
    if (!row || deepLinkOpenedRef.current === key) return;
  ...
  async function selectRow(r: LsRow, stageOverride?: Tab) {
    const docId = String(r.id || "");
    if (docId && projectId) {
      try {
        const effectiveTab = stageOverride || tab;
        const endpoint =
          effectiveTab === "FREIGEGEBEN"
