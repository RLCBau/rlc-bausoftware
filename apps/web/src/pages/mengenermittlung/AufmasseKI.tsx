...
export default function AufmasseKI() {
  ...
  const [unitScale, setUnitScale] = React.useState<number>(1);
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const nextDocs: DocInfo[] = Array.isArray(j?.docs) ? j.docs : [];
        setDocs(nextDocs);
        if (nextDocs.length && !docId) setDocId(nextDocs[0].id);
      } catch (err) {
        console.error(err);
      }
    })();
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const j = await r.json();
      const nextItems: MeasItem[] = Array.isArray(j?.items) ? j.items : [];
      setItems(nextItems);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  return (
    <div className="card" style={{ padding: 10 }}>
      ...
      >
        <label style={{ fontSize: 13, opacity: 0.8 }}>Dokument</label>
        <select
          value={docId ?? ""}
        ...
        <label style={{ fontSize: 13, opacity: 0.8 }}>Skalierungsfaktor</label>
        <input
          type="number"
          step="0.0001"
          value={unitScale}
      ...
      >
        ...
        >
          {current?.previewUrl ? (
            ...
            <div style={{ opacity: 0.7, padding: 12, fontSize: 13 }}>
              Keine Vorschau verfügbar. (Die Dateien werden in der nächsten Sektion{" "}
              <b>Import PDF / CAD / LandXML / GSI / CSV</b> geladen.)
            </div>
          )}
        </div>
      ...
      <div style={{ marginTop: 10, opacity: 0.7, fontSize: 13 }}>
        Hinweis: Diese Seite nutzt aktuell Demo-Ergebnisse. Die echten Erkennungen
        werden aus den in <b>Import PDF / CAD / LandXML / GSI / CSV</b>{" "}
        hochgeladenen Dateien berechnet.
      </div>
    </div>
  );
}
