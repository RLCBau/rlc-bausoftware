...
/* ------------------ CONFIG SIDE + MENU ------------------ */
type Section = { key: string; title: string; items: { key: string; label: string }[] };
  ...
  return (
    <div className="card">
      ...
      <div className="s-sub" style={{ paddingBottom: 8 }}>
        {topItems.map((it) => (
          <Link key={it.to} to={it.to} className={`s-link ${pathname === it.to ? "active" : ""}`}>
            {it.label}
          </Link>
        ))}
      </div>
        ...
        {SECTIONS.map((s) => {
          ...
          return (
            <li key={s.key} className={`s-sec ${isOpen ? "open" : ""}`}>
              ...
              {isOpen && (
                <div className="s-sub">
                  <Link className={`s-link ${pathname === `/${s.key}` ? "active" : ""}`} to={`/${s.key}`}>
                    Übersicht
                  </Link>
                  {s.items.map((it) => {
                    const active = pathname === `/${s.key}/${it.key}`;
                    return (
                      <Link key={it.key} className={`s-link ${active ? "active" : ""}`} to={`/${s.key}/${it.key}`}>
                        {it.label}
                      </Link>
                    );
                  })}
...
function SubsectionEmpty() {
  ...
  return (
    <div className="card">
      <div className="breadcrumbs">
        <Link className="link" to="/">RLC</Link>
        <span className="sep">/</span>
        {s ? <Link className="link" to={`/${s.key}`}>{s.title}</Link> : <span>Unbekannt</span>}
        <span className="sep">/</span>
          ...
          <div className="layout">
            ...
            <div className="content">
              <Routes>
                ...
                {SECTIONS.map((s) => (
                  <Route key={s.key} path={`/${s.key}`} element={OVERVIEW[s.key] ?? <SectionList sectionKey={s.key} />} />
                ))}
                ...
                {/* Büro */}
                <Route path="/buro/projekte" element={<BuroLayout><Projekte /></BuroLayout>} />
                <Route path="/buro/dokumente" element={<BuroLayout><Dokumente /></BuroLayout>} />
                <Route path="/buro/kommunikation" element={<BuroLayout><Kommunikation /></BuroLayout>} />
                <Route path="/buro/vertraege" element={<BuroLayout><Vertraege /></BuroLayout>} />
                ...
                <Route path="/buro/personalverwaltung" element={<Personalverwaltung />} />
                <Route path="/buro/maschinenverwaltung" element={<Maschinenverwaltung />} />
                <Route path="/buro/materialverwaltung" element={<Materialverwaltung />} />
                <Route path="/buro/ressourcenplanung" element={<Ressourcenplanung />} />
                ...
                <Route path="/buro/tasks" element={<BuroLayout><Tasks /></BuroLayout>} />
                ...
                <Route path="/kalkulation/versionsvergleich" element={<VersionsvergleichPage />} />
                <Route path="/kalkulation/aufschlag" element={<AufschlagPage />} />
                <Route path="/kalkulation/lv-export" element={<LVExportOhnePreisePage />} />
