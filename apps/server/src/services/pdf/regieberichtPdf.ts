...
export type CreateRegieberichtPdfInput = {
  /**
   * Percorso COMPLETO del file PDF da scrivere.
   * Viene calcolato dalla route (con PROJECTS_ROOT) e passato qui.
   * Esempio: C:/.../data/projects/<projectId>/regieberichte/Regiebericht_2025-12-06_001.pdf
   */
  pdfPath: string;
...
/**
 * Crea il PDF del Regiebericht LKS-Style nel percorso indicato da pdfPath.
 *
 * Ritorna:
 *  - filePath: percorso completo sul disco (uguale a pdfPath)
 *  - pdfUrl: URL relativo da usare nel FE (es. "/files/PRJ-1/regieberichte/…")
 *  - fileName: solo il nome file (Regiebericht_YYYY-MM-DD_xxx.pdf)
 */
export async function createRegieberichtPdf(
  ...
  const aufmass = aufmassRaw as RegieItem[];
  ...
  doc.rect(left, topY, leftBlockW, 60).lineWidth(0.7).stroke();
  ...
  days.forEach((d, idx) => {
    ...
    doc.moveTo(x, daysY).lineTo(x, daysY + 16).stroke();
  ...
  const colWetterW =
    right -
    left -
    (colArbeitsbeginnW + 2 * colPauseW + colArbeitsendeW + colBlattW);
  ...
  doc.moveTo(xPause1, headY).lineTo(xPause1, headY + rowH).stroke();
  doc.moveTo(xPause2, headY).lineTo(xPause2, headY + rowH).stroke();
  doc.moveTo(xArbeitsende, headY).lineTo(xArbeitsende, headY + rowH).stroke();
  doc.moveTo(xBlatt, headY).lineTo(xBlatt, headY + rowH).stroke();
  doc.moveTo(xWetter, headY).lineTo(xWetter, headY + rowH).stroke();
  ...
  doc.moveTo(xPause1, timeY).lineTo(xPause1, timeY + rowH).stroke();
  doc.moveTo(xPause2, timeY).lineTo(xPause2, timeY + rowH).stroke();
  doc.moveTo(xArbeitsende, timeY).lineTo(xArbeitsende, timeY + rowH).stroke();
  doc.moveTo(xBlatt, timeY).lineTo(xBlatt, timeY + rowH).stroke();
  doc.moveTo(xWetter, timeY).lineTo(xWetter, timeY + rowH).stroke();

  // --------------------------------------------------
  // Tabelle: Geräte / Mitarbeiter / Besondere Leistungen (max 6 Zeilen)
  // --------------------------------------------------
  ...
  doc.moveTo(xName, tableHeadY).lineTo(xName, tableHeadY + tableRowH).stroke();
  doc.moveTo(xBes, tableHeadY).lineTo(xBes, tableHeadY + tableRowH).stroke();
  ...
  for (let i = 0; i < maxRows; i++) {
    ...
    doc.moveTo(xName, y).lineTo(xName, y + tableRowH).stroke();
    doc.moveTo(xBes, y).lineTo(xBes, y + tableRowH).stroke();
  ...
  const signLeftW = (right - left - 140) / 2;
  ...
  doc.fontSize(8).text("Geprüft – Bauleiter", left + 4, signTop + 4);
  doc
  ...
  doc.fontSize(8).text("Aufgestellt – Polier", left + 4, sign2Top + 4);
  doc
  ...
  if (photos && photos.length) {
    ...
    for (const ph of photos) {
      try {
        ...
        doc.rect(left, y - 4, maxW + 8, maxH + 8).stroke();
  ...
  return { filePath: pdfPath, pdfUrl: relPath, fileName };
}
