...
export function rebaseCadPoint(
  point: CadPoint,
  origin: CadPoint
): CadPoint {
  return {
...
export function restoreCadPoint(
  point: CadPoint,
  origin: CadPoint
): CadPoint {
  return {
...
export function rebaseCadViewBox(
  viewBox: CadViewBox,
  origin: CadPoint
): CadViewBox {
  return {
...
export function zoomCadViewBox(
  previous: CadViewBox,
  factor: number,
  anchorRatio: CadPoint
): CadViewBox {
  const safeFactor =
  ...
  const nextWidth = Math.max(
    0.001,
    Math.min(1e12, previous.width * safeFactor)
  );
  const nextHeight = Math.max(
    0.001,
    Math.min(1e12, previous.height * safeFactor)
  );
...
export type SnapKind = "endpoint" | "vertex" | "midpoint" | "center" | "surveyPoint";
...
function distance(a: CadPoint, b: CadPoint) {
  return Math.hypot(b.x - a.x, b.y - a.y);
}

function featureLength(points: CadPoint[], closed: boolean) {
  let total = 0;
...
function featureArea(points: CadPoint[]) {
...
function normalizePoint(point: unknown): CadPoint | null {
  const candidate = point as Partial<CadPoint> | null | undefined;
...
export function cloneCadFeatures(features: CadFeature[]) {
...
export function recalculateCadFeature(
  feature: CadFeature,
  fallbackIndex = 0
): CadFeature {
  const points = Array.isArray(feature.pts)
    ? feature.pts.map(normalizePoint).filter((point): point is CadPoint => Boolean(point))
    : [];
  const kind: CadFeatureKind =
    feature.kind ||
...
export function normalizeCadFeatures(payload: unknown): CadFeature[] {
  ...
  return candidates.map((rawFeature: any, index: number) => {
    const rawPoints =
      rawFeature?.pts ||
      rawFeature?.points ||
      rawFeature?.vertices ||
    ...
    const pts = Array.isArray(rawPoints)
      ? rawPoints
          ...
          .filter((point: CadPoint | null): point is CadPoint => Boolean(point))
      : [];
    ...
    const kind: CadFeatureKind =
      rawType.includes("circle") || rawType.includes("kreis")
        ? "circle"
...
export function translateCadFeatures(
  features: CadFeature[],
  selectedIds: string[],
  dx: number,
  dy: number
  ...
  return features.map((feature, index) => {
    if (!selected.has(String(feature.id || ""))) return feature;
    return recalculateCadFeature(
      {
        ...feature,
...
export function setCadVertex(
  ...
  return features.map((feature, index) => {
    ...
    const points = (feature.pts || []).map((candidate, candidateIndex) =>
      candidateIndex === vertexIndex ? { ...point } : candidate
    );
    return recalculateCadFeature({ ...feature, pts: points }, index);
  });
}

export function duplicateCadFeatures(
  features: CadFeature[],
  selectedIds: string[],
  dx = 0,
  dy = 0
  ...
  const selected = new Set(selectedIds);
  const copies: CadFeature[] = [];
  const copiedIds: string[] = [];

  features.forEach((feature, index) => {
    if (!selected.has(String(feature.id || ""))) return;
...
export function rotateCadFeatures(
  features: CadFeature[],
  selectedIds: string[],
  center: CadPoint,
  angleDegrees: number
...
export function scaleCadFeatures(
  features: CadFeature[],
  selectedIds: string[],
  center: CadPoint,
  factor: number
) {
  if (!Number.isFinite(factor) || Math.abs(factor) < EPSILON) return features;
  const selected = new Set(selectedIds);

  return features.map((feature, index) => {
    if (!selected.has(String(feature.id || ""))) return feature;
    const height = Number((feature.meta as any)?.height || 0);
    return recalculateCadFeature(
      {
        ...feature,
        radius:
...
function unitNormal(a: CadPoint, b: CadPoint) {
  const dx = b.x - a.x;
  const dy = b.y - a.y;
...
function offsetPolylinePoints(
  ...
  const normals = Array.from({ length: segmentCount }, (_, index) =>
    unitNormal(points[index], points[(index + 1) % points.length])
  );
  ...
  return points.map((point, index) => {
    ...
    const denominator = Math.max(
      0.25,
      Math.abs(miter.x * next.x + miter.y * next.y)
    );
...
export function offsetCadFeatures(
  features: CadFeature[],
  selectedIds: string[],
  amount: number
) {
  const selected = new Set(selectedIds);
  const created: CadFeature[] = [];
...
export function findCadSnap(
  ...
  for (const feature of features) {
    ...
    if (!featureId || excluded.has(featureId)) continue;
    const points = feature.pts || [];
...
function parseDxfPairs(source: string): DxfPair[] {
...
function firstValue(pairs: DxfPair[], code: number, fallback = "") {
  return pairs.find((pair) => pair.code === code)?.value ?? fallback;
}

function numericValue(pairs: DxfPair[], code: number, fallback = 0) {
  return finite(firstValue(pairs, code), fallback);
}

function decodeDxfText(pairs: DxfPair[]) {
  const raw = pairs
  ...
  return raw
    .replace(/\\U\+([0-9a-f]{4,8})/gi, (_match, hex: string) => {
...
function repeatedVertices(pairs: DxfPair[], scale: number) {
  const vertices: DxfVertex[] = [];
  let current:
    | { x: number; y: number | null; bulge: number }
    | null = null;
...
function repeatedPoints(
  ...
  let current: { x: number; y: number | null } | null = null;
  const flush = () => {
    if (current && current.y !== null) {
...
function dxfUnitScale(pairs: DxfPair[]) {
  ...
  for (let index = 0; index < pairs.length - 1; index += 1) {
    if (pairs[index].code !== 9 || pairs[index].value !== "$INSUNITS") continue;
...
function dxfAciColor(value: number) {
...
function dxfTrueColor(value: number) {
  const color = Math.max(0, Math.trunc(value));
  if (!color) return undefined;
  return `#${(color & 0xffffff).toString(16).padStart(6, "0")}`;
}

function dxfLayerStyles(pairs: DxfPair[]) {
...
function extractEntityBlocks(pairs: DxfPair[]) {
  const entitiesMarker = pairs.findIndex(
    (pair, index) =>
      pair.code === 2 &&
...
function extractBlockDefinitions(
  pairs: DxfPair[],
  scale: number
): Map<string, DxfBlockDefinition> {
  const definitions = new Map<string, DxfBlockDefinition>();
  const blocksMarker = pairs.findIndex(
    (pair, index) =>
      pair.code === 2 &&
  ...
  if (blocksMarker < 0) return definitions;
  ...
  while (index < pairs.length) {
    ...
    const basePoint =
      indexedPoint(header, 10, 20, scale) || { x: 0, y: 0 };
    const entities: DxfEntityBlock[] = [];
...
function baseDxfMeta(
  block: DxfEntityBlock,
  fileName: string,
  unitCode: number,
  ...
  const paperSpace =
    Math.trunc(numericValue(block.pairs, 67)) === 1 ||
  ...
  return {
    ...
    paperSpace,
    color:
      dxfTrueColor(entityTrueColor) ||
      dxfAciColor(entityAciColor) ||
      layerStyle?.color,
    lineType: firstValue(block.pairs, 6) || layerStyle?.lineType,
    lineWeight:
      numericValue(block.pairs, 370) || layerStyle?.lineWeight || undefined,
  };
}
...
function bulgeArcPoints(
  ...
  const segments = Math.max(
    4,
    Math.min(180, Math.ceil(Math.abs(sweep) / (Math.PI / 36)))
  );
...
function flattenBulgedPolyline(vertices: DxfVertex[], closed: boolean) {
  if (vertices.length < 2) return vertices.map((vertex) => vertex.point);
  const points: CadPoint[] = [{ ...vertices[0].point }];
...
const IDENTITY_MATRIX: DxfMatrix = {
  a: 1,
  b: 0,
  c: 0,
  d: 1,
...
function applyDxfMatrix(matrix: DxfMatrix, point: CadPoint): CadPoint {
  return {
...
function multiplyDxfMatrices(parent: DxfMatrix, local: DxfMatrix): DxfMatrix {
  return {
...
function transformDxfFeature(
  ...
  if (feature.kind === "circle" && points[0] && Number(feature.radius || 0) > 0) {
    ...
    const xRadius = distance(
      transformedCenter,
      applyDxfMatrix(matrix, { x: center.x + radius, y: center.y })
    );
    const yRadius = distance(
      transformedCenter,
      applyDxfMatrix(matrix, { x: center.x, y: center.y + radius })
    );
...
function ellipsePoints(
  ...
  const segments = Math.max(
    24,
    Math.min(240, Math.ceil(Math.abs(sweep) / (Math.PI / 72)))
  );
...
function clampedUniformKnots(controlPointCount: number, degree: number) {
  ...
  return Array.from({ length: knotCount }, (_, index) => {
    ...
    if (index >= controlPointCount) return finalValue;
    return index - degree;
  });
}
...
function splinePoints(pairs: DxfPair[], scale: number) {
  const controlPoints = repeatedVertices(pairs, scale).map(
    (vertex) => vertex.point
  );
  ...
  const degree = Math.max(
    1,
    Math.min(requestedDegree, controlPoints.length - 1)
  );
  ...
  const knots =
    rawKnots.length >= requiredKnotCount
...
export function parseAsciiDxf(
  source: string,
  fileName = "drawing.dxf"
): CadDocument {
  if (!source.trim()) throw new Error("Die DXF-Datei ist leer.");
  ...
  const emitBlocks = (
    entityBlocks: DxfEntityBlock[],
    inheritedLayer = "",
    matrix: DxfMatrix = IDENTITY_MATRIX,
    idPrefix = "DXF",
    depth = 0,
    inheritedPaperSpace = false,
    inheritedLayout = "Model"
  ) => {
    if (depth > 8) return;
    ...
    for (let index = 0; index < entityBlocks.length; index += 1) {
      ...
      const layer =
        inheritedLayer && rawLayer === "0" ? inheritedLayer : rawLayer;
      const handle = firstValue(block.pairs, 5, `${index + 1}`);
      const id = `${idPrefix}_${handle}_${generatedIndex + 1}`;
      const meta = baseDxfMeta(
        block,
        fileName,
        unitCode,
        layerStyles.get(layer)
      );
      if (depth > 0) {
      ...
      if (depth === 0) {
        if (meta.paperSpace) paperSpaceCount += 1;
        else modelSpaceCount += 1;
      }

      const push = (feature: CadFeature) => {
        generatedIndex += 1;
        features.push(
          transformDxfFeature(
            {
      ...
      if (block.type === "LINE") {
        ...
        if (start && end) push({ kind: "line", pts: [start, end] });
        continue;
      }

      if (block.type === "LWPOLYLINE") {
        const closed =
          (Math.trunc(numericValue(block.pairs, 70)) & 1) === 1;
        ...
        if (points.length) {
          push({
            ...
            closed,
            meta: {
              bulgeSegments: vertices.filter(
                (vertex) => Math.abs(vertex.bulge) > EPSILON
              ).length,
            },
          });
        }
      ...
      if (block.type === "ELLIPSE") {
        ...
        if (center && majorAxis) {
          ...
          const endParameter = numericValue(
            block.pairs,
            42,
            Math.PI * 2
          );
          const points = ellipsePoints(
            center,
            majorAxis,
            Math.abs(numericValue(block.pairs, 40, 1)),
            startParameter,
            endParameter
          );
          const closed =
            Math.abs(endParameter - startParameter) >= Math.PI * 2 - 1e-6;
          if (points.length) {
            if (
              closed &&
              distance(points[0], points[points.length - 1]) < EPSILON
      ...
      if (block.type === "INSERT" || block.type === "DIMENSION") {
        ...
        const insertion =
          indexedPoint(block.pairs, 10, 20, scale) || { x: 0, y: 0 };
        if (definition) {
          ...
          const columns = Math.max(
            1,
            Math.trunc(numericValue(block.pairs, 70, 1))
          );
          const rows = Math.max(
            1,
            Math.trunc(numericValue(block.pairs, 71, 1))
          );
          ...
          for (let row = 0; row < rows; row += 1) {
            for (let column = 0; column < columns; column += 1) {
              ...
              emitBlocks(
                definition.entities,
                layer,
                multiplyDxfMatrices(matrix, local),
                `${id}_${blockName}_${row}_${column}`,
                depth + 1,
                Boolean(meta.paperSpace),
                String(meta.layout || "Model")
              );
            }
          }
        } else {
      ...
      if (block.type === "POINT") {
        ...
        if (point) push({ kind: "point", name: "Punkt", pts: [point] });
        continue;
      }
      ...
      if (
        ...
        const horizontalJustification = Math.trunc(
          numericValue(block.pairs, 72)
        );
        const verticalJustification = Math.trunc(
          numericValue(block.pairs, 73)
        );
        ...
        if (point) {
          push({
            ...
            meta: {
              ...
              attachmentPoint: Math.trunc(
                numericValue(block.pairs, 71, 1)
              ),
      ...
      if (block.type === "HATCH" || block.type === "LEADER") {
        const points = repeatedVertices(block.pairs, scale).map(
          (vertex) => vertex.point
        );
        if (points.length > 1) {
      ...
      if (
        ...
        const points = [10, 11, 12, 13]
          .map((xCode) =>
            indexedPoint(block.pairs, xCode, xCode + 10, scale)
          )
          .filter((point): point is CadPoint => Boolean(point));
  ...
  if (!normalized.length) {
    throw new Error(
      "Die DXF-Datei enthält keine unterstützte 2D-Geometrie (LINE, POLYLINE, CIRCLE, ARC, ELLIPSE, INSERT, POINT, TEXT, SPLINE, MLINE)."
    );
  }
