...
function fotoPdfInputFromSource(opts: {
  pdfPath: string;
  projectId: string;
  source: any;
  company?: any;
}) {
  const source = decorateInboxItem(opts.projectId, opts.source || {});
  const main = source.main
    ? {
        ...source.main,
        name: source.main.name || source.main.file,
        url: source.main.publicUrl || source.main.url,
        publicUrl: source.main.publicUrl,
      }
    : source.imageUri
    ? { name: "Hauptfoto", url: source.imageUri }
    : null;
  const files = [
    ...
    ...file,
    name: file?.name || file?.file || "Datei",
    url: file?.url || file?.publicUrl || file?.uri,
  ...
  return {
    ...
    projectName: String(source.projectName || source.projectTitle || source.baustelle || opts.projectId),
    date: String(source.date || source.createdAt || new Date().toISOString()).slice(0, 10),
    kostenstelle: String(source.kostenstelle || source.costCenter || ""),
    lvItemPos: source.lvItemPos || source.lvPos || null,
    comment: String(source.comment || source.note || ""),
    bemerkungen: String(source.bemerkungen || source.notes || ""),
    main,
    files,
    extras: Array.isArray(source.extras) ? source.extras : [],
    boxes: Array.isArray(source.boxes) ? source.boxes : [],
...
router.post("/inbox/submit", express.json(), (req, res) => {
  ...
  const entry = {
    ...
    mitarbeiter: String(req.body?.mitarbeiter || req.body?.worker || "").trim(),
    worker: String(req.body?.worker || req.body?.mitarbeiter || "").trim(),
    regieId: String(req.body?.regieId || req.body?.regieberichtId || "").trim(),
    lieferscheinId: String(req.body?.lieferscheinId || req.body?.lsId || "").trim(),
...
router.post("/inbox/update", express.json({ limit: "25mb" }), (req, res) => {
  ...
  const updated = {
    ...current,
    id: String(current.id || current.docId || docId),
    docId: String(current.docId || current.id || docId),
    projectId,
    projectCode: projectId,
    date: String(req.body?.date || current.date || new Date().toISOString()).slice(0, 10),
    mitarbeiter: String(req.body?.mitarbeiter ?? req.body?.worker ?? current.mitarbeiter ?? current.worker ?? "").trim(),
    worker: String(req.body?.worker ?? req.body?.mitarbeiter ?? current.worker ?? current.mitarbeiter ?? "").trim(),
    kostenstelle: String(req.body?.kostenstelle ?? current.kostenstelle ?? "").trim(),
    lvItemPos: String(req.body?.lvItemPos ?? req.body?.lvPos ?? current.lvItemPos ?? "").trim(),
    regieId: String(req.body?.regieId ?? req.body?.regieberichtId ?? current.regieId ?? "").trim(),
    lieferscheinId: String(req.body?.lieferscheinId ?? req.body?.lsId ?? current.lieferscheinId ?? "").trim(),
    comment: String(req.body?.comment ?? req.body?.note ?? current.comment ?? current.note ?? "").trim(),
    note: String(req.body?.note ?? req.body?.comment ?? current.note ?? current.comment ?? "").trim(),
    bemerkungen: String(req.body?.bemerkungen ?? current.bemerkungen ?? "").trim(),
    extras: req.body?.extras ?? current.extras,
    boxes: req.body?.boxes ?? current.boxes,
    main: current.main || null,
    files: Array.isArray(current.files) ? current.files : [],
    status: current.status || "INBOX",
    workflowStatus: current.workflowStatus || "INBOX",
    updatedAt: new Date().toISOString(),
  };
  ...
  return res.json({
    ...
    projectId,
    docId: updated.docId,
    item: decorateInboxItem(projectId, updated),
  });
});

// APPROVE INBOX DOC -> FINAL
async function commitInboxDoc(projectId: string, docIdRaw: string, req?: any) {
  ...
  const safeDate = String(finalEntry.date || finalEntry.createdAt || new Date().toISOString()).slice(0, 10);
  const pdfName = `Fotodokumentation_${safeDate}_${safeName(realDocId)}.pdf`;
  const pdfPath = path.join(notesDir(projectId), pdfName);
  const company = req ? await loadRlcPdfCompanyFromRequest(req) : undefined;
  const pdfResult = await createFotoDokumentationPdf(
    fotoPdfInputFromSource({ pdfPath, projectId, source: finalEntry, company })
  );
...
router.post("/inbox/approve", express.json(), async (req, res) => {
  try {
    ...
    const item = await commitInboxDoc(projectId, docId, req);
...
router.post("/preview", express.json({ limit: "25mb" }), async (req, res) => {
  try {
    ...
    const previewDir = path.join(notesDir(projectId), "preview");
    ...
    const pdfPath = path.join(previewDir, `Fotodokumentation_Vorschau_${date}_${Date.now()}.pdf`);
    const company = await loadRlcPdfCompanyFromRequest(req);
    const result = await createFotoDokumentationPdf(
      fotoPdfInputFromSource({ pdfPath, projectId, source: req.body, company })
    );
    return res.json({ ok: true, ...result });
  } catch (e: any) {
    console.error("POST /api/fotos/preview failed:", e);
