...
function visionPrompt(note: string) {
  return `
Du bist ein präziser Bauabrechner für Tiefbau, Straßenbau, Entwässerung und Außenanlagen.
Analysiere ausschließlich den sichtbaren Ausschnitt eines Plans oder Baustellenfotos.

ZIEL:
Erkenne nur tatsächlich ausführbare und abrechenbare Bauleistungen mit einer eindeutig lesbaren
Menge, Länge, Fläche, Volumen oder Stückzahl.

NICHT als Position ausgeben:
- Höhenkoten, Geländehöhen, Deckel-/Sohlhöhen und Werte wie D 687.56, S 685.40, OK/UK ±0.00.
- Maßstab, Plan-Nr., Blattgröße, Datum, Index, Projektname, Bauherr, Planer, Phase und Legende.
- Koordinaten, Achsen, Stationierungen, Gefällewerte oder reine Maßketten ohne zuordenbare Leistung.
- Unsichtbare, geschätzte oder frei erfundene Leistungen.
- Allgemeine Begriffe wie "Boden", "Kabel", "Rohre", "Zufahrt" ohne eindeutigen Planbezug.
- Positionen ohne sicher lesbare Menge. In Zweifelsfällen nicht ausgeben.

MENGENREGELN:
- Bereits eingetragene Abrechnungsmengen dürfen übernommen werden.
- Eine Zahl darf nur qty sein, wenn sie eindeutig zur beschriebenen Bauleistung gehört.
- Durchmesser, Nennweiten, Höhen und Bauteilabmessungen sind keine qty.
- Keine Mengen aus Maßstab oder Geometrie schätzen.
- Pro realer Leistung nur eine Position ausgeben.
...
Gib ausschließlich JSON zurück:
{
  ...
  "positions": [
    {
      "id": "",
      "kurztext": "konkrete Bauleistung",
      "einheit": "m|m²|m³|St|kg|t|h|Psch",
      "qty": 12.34,
      "confidence": 0.82,
      "evidence": "kurzer Hinweis, welcher lesbare Plantext die Position belegt"
    }
  ]
}
...
- 0 bis 30 Positionen sind erlaubt.
- confidence liegt zwischen 0 und 1.
- Nur Positionen mit qty > 0 ausgeben.
- Optionaler Nutzerhinweis: ${note || "(keine Notiz)"}.
`.trim();
}
...
function clampConfidence(v: any, fallback = 0.62) {
  const n = Number(v);
...
function isAdministrativeOrNonBillable(textRaw: string) {
  const text = normalizeTechnicalText(textRaw);
  if (!text) return true;
...
function looksLikeElevationOrDimension(textRaw: string, qty: number | null) {
  const raw = String(textRaw || "").trim();
  const text = normalizeTechnicalText(raw);

  if (/\b(?:ok|uk|d|s)\s*[+-]?\d{2,4}[.,]\d{1,3}\b/i.test(raw)) return true;
  if (/\bhohe\b|\bhohenkote\b|\bdeckel\b|\bsohle\b/.test(text)) return true;
  if (qty != null && qty >= 400 && /\b(?:d|s|ok|uk)\b/.test(text)) return true;
  return false;
}

function technicalPositionKey(p: PhotoPosition) {
  const text = normalizeTechnicalText(p.kurztext)
    .replace(/\b\d+(?:[.,]\d+)?\b/g, "")
    .replace(/\s+/g, " ")
    .trim();
  const unit = String(p.einheit || "").trim().toLowerCase();
  const qty = p.qty == null ? "" : Number(p.qty).toFixed(3);
  return `${text}|${unit}|${qty}`;
}

function filterAndDedupePositions(input: PhotoPosition[]) {
  const out: PhotoPosition[] = [];
  const seen = new Set<string>();

  for (const p of input || []) {
    const kurztext = String(p?.kurztext || "").trim();
    const qty = coerceQty(p?.qty);
    const einheit = String(p?.einheit || "").trim();

    if (!kurztext || !einheit || qty == null || qty <= 0) continue;
    if (isAdministrativeOrNonBillable(kurztext)) continue;
    if (looksLikeElevationOrDimension(kurztext, qty)) continue;

    const normalized: PhotoPosition = {
      ...p,
      kurztext,
      einheit,
      qty,
      confidence: clampConfidence(p?.confidence),
      evidence: String(p?.evidence || "").trim() || undefined,
    };

    const key = technicalPositionKey(normalized);
...
function typeFromUnit(unitRaw: string): "LINE" | "AREA" | "COUNT" {
  const unit = String(unitRaw || "").toLowerCase().replace(/\s+/g, "");
  if (unit === "m2" || unit === "m²") return "AREA";
  if (unit === "m" || unit === "lfm") return "LINE";
  return "COUNT";
}
...
async function analyzeWithOpenAI(imageDataUrl: string, note: string) {
  ...
  console.log("[auto-ki] OpenAI model:", OPENAI_MODEL);
  console.log("[auto-ki] OpenAI raw:", text.slice(0, 8000));
  ...
  if (!parsed || !Array.isArray(parsed.positions)) {
    return { summary: "KI-Antwort unlesbar.", positions: [] as PhotoPosition[] };
  }

  const mapped: PhotoPosition[] = parsed.positions
    .map((p: any) => ({
      id: p?.id ? String(p.id) : "",
    ...
    .filter((p: PhotoPosition) => p.kurztext);

  const positions = filterAndDedupePositions(mapped);

  console.log(
    "[auto-ki] parsed positions:",
    mapped.length,
    "accepted:",
    positions.length,
    "filtered:",
    mapped.length - positions.length
  );
    ...
    if (KI_VISION_ON && preview) {
      try {
        ...
        boxes = positions.map((pp: any, idx: number) => ({
          ...
          score: clampConfidence(pp.confidence),
          qty: Number(pp.qty),
        ...
        items = positions.map((pp: any, idx: number) => ({
          ...
          type: typeFromUnit(pp.einheit),
          descr: String(pp.kurztext || ""),
          unit: String(pp.einheit || ""),
          qty: Number(pp.qty),
          layer: "",
          source: "image+openai",
          confidence: clampConfidence(pp.confidence),
          matchReason:
            String(pp.evidence || "").trim() ||
            "Aus lesbarer Planangabe erkannt; technische Prüfung erforderlich",
        }));
      } catch (e: any) {
        console.error("[auto-ki] OpenAI failed:", e?.message || e);
        summary = `OpenAI-Analyse fehlgeschlagen: ${e?.message || "unknown"}`;
        boxes = [];
        extras = [];
        items = [];
        positions = [];
      ...
      summary = isPdf
        ? "PDF importiert, aber die Vorschau konnte serverseitig nicht gerendert werden."
        : "Datei importiert. Keine KI-Analyse, weil die Vorschau fehlt oder KI_VISION deaktiviert ist.";
      boxes = [];
      extras = [];
      items = [];
      positions = [];
